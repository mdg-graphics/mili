		...
		unsigned int xsize, unsigned int ysize, unsigned int zsize)
{
	...
	if (rw)
	    ...
	    image->flags = _IOREAD;
	if(ISRLE(image->type)) {
	    tablesize = image->ysize*image->zsize*sizeof(SIGNED_4BYTE);
	    image->rowstart = (UNSIGNED_4BYTE *)malloc(tablesize);
	    image->rowsize = (SIGNED_4BYTE *)malloc(tablesize);
