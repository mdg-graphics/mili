...
SIGNED_4BYTE img_getrowsize(IMAGE *image)
{
    SIGNED_4BYTE rval;
    switch(image->dim) {
	case 1:
	    rval = image->rowsize[0];
	case 2:
	    rval = image->rowsize[image->y];
	case 3:
	    rval = image->rowsize[image->y+image->z*image->ysize];
    }
...
void img_setrowsize(IMAGE *image, long cnt, int y, int z)
{
    SIGNED_4BYTE *sizeptr;
