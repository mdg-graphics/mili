...
extern	char	*hallocate( unsigned int );
extern	char	*getenv();


/*
 * hmove
 *      For OpenGL, takes the place of the GL command "move".
 */
void
hmove( float x, float y, float z )
{
        h_curr_pos[0] = x;
        h_curr_pos[1] = y;
        h_curr_pos[2] = z;
...
/*
 * hmove2
 *      For OpenGL, takes the place of the GL command "move2".
 */
void
hmove2( float x, float y )
{
        hmove( x, y, 0.0 );
}
...
/*
 * hrmv
 *      For OpenGL, takes the place of the GL command "rmv", which
 *      performs a relative move.
 */
void
hrmv( float x, float y, float z )
{
        h_curr_pos[0] += x;
        h_curr_pos[1] += y;
        h_curr_pos[2] += z;
...
/*
 * hrdr
 *      For OpenGL, takes the place of the GL command "rdr", which
 *      performs a relative draw.
 */
void
hrdr( float x, float y, float z )
{
        glBegin( GL_LINES );
        glVertex3fv( h_curr_pos );
        h_curr_pos[0] += x;
        h_curr_pos[1] += y;
        h_curr_pos[2] += z;
        glVertex3fv( h_curr_pos );
 ...
 */
void
hgetcharsize( char c, float *width, float *height )
{
 ...
 */
void
htextsize( float width, float height )
{
	float	a;
 ...
 */
void
hgetfontsize( float *cw, float *ch )
{
 ...
 */
void
hboxtext( float x, float y, float l, float h, char *s )
{
 ...
 */
void
hboxfit( float l, float h, int nchars )
{
