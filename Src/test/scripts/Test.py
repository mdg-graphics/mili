...
class Test:
	...
	def start(self,params,suites):
		...
		### Disabled the checking of the platform checking ####
		#if params["baseline"] and not self.dyna_env.default_platform == self.dyna_env.osname:
		#	os.system("echo Cannot install baseline for %s from %s tests" % (self.dyna_env.default_platform, self.dyna_env.osname))
	 	#sys.exit(0)
		##########################################
		os.system("echo Log File: %s" % default_log)
		if params["quiet"]:
			os.system("echo Run File: %s" %  run_log)
			f = open(run_log,'a')
			f.close()
	...
	def runCode(self, params):
		...
		for template in execs:
			...
			index = index +1
			if index == len(execs):
				if "run_validation_suffix" in self.dyna_env.mapping[params["suite"]].keys():
	...
	def runATest(self, params):
		...
		if params["parallel"] and self.cannotRunParallel(params["test"],params["suite"]):
			...
			if params["mode"] == "smoke":
				os.system("echo Starting Smoke Test of %s %s" % (params["suite"],params["test"]))
			elif params["evaluate"]:
				os.system("echo Starting Evaluation of %s %s" % (params["suite"],params["test"]))
			else:
				os.system("echo Starting %s %s" % (params["suite"],params["test"]))
			...
			os.chdir(home)
			os.system("echo Finished with status: %s" % output["status"])
