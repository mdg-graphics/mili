...
class CurveParser(parser.parser):
	...
	def compare(	self,analyzer,curves2,results):
		...
		for key in curves2.keys():
			if not key in curves1.keys():
				test_passed = False
				m= {"message":"No matching baseline curve","baseline":"None","test_value":None}
				...
				for timestep in curves1[key].keys():
					if key in curves2.keys() and not timestep in curves2[key].keys():
						test_passed = False
						m= {"message":"Missing timestep in test output.","baseline":"None","test_value":None,"validtimestep":False}
						if not key in results.keys():
							results[key] = {timestep:m}
						else:
							results[key][timestep] = m
						
				for timestep in curves2[key].keys():
					if key in curves1.keys() and not timestep in curves1[key].keys():
						test_passed = False
						m= {"message":"Unkown timestep to baseline.","baseline":"None","test_value":None,"validtimestep":False}
						if not key in results.keys():
							results[key] = {timestep:m}
						else:
							results[key][timestep] = m
			...
			for key in info.keys():
				...
				if not info[key] == None:
					if "unkowncurve" in info[key].keys():
						message = "%s%s" % (message,"%6s%s \n" %("",info[key]["unkowncurve"]["message"]))
					else:
						...
						for step in sorted(info[key].keys()):
							...
							if d == None:
								...
								message = "%s%s" % (message,"%6s%-10s %s\n" % ("",fpformat.sci(step,1),d["message"]))
							else:
	...
	def readCurve(self,pos,lines):
		...
		while not lines[i].startswith("end") and i >= 0:
			...
			if testline.find("rel ") > 0 or  testline.find("abs ") > 0 or testline == "#":
				...
				vals = testline.strip().split()
				try:
					temp_val = float(vals[0])
					temp_val = float(vals[1])
				except:
					print "There seems to be formatting issue with your curve: "
					print "ignoring line:",i,testline
					pass 
		...
		while not lines[i].strip().startswith("end") and not lines[i].strip().startswith("#") and len(lines[i].strip()) >1:
			...
			try:
				...
				print "Invalid number conversion in readCurve: ",time,value
