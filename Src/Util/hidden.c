...
/* 
 * hidden.c - Create a hidden line drawing of a mesh.
 * 
 * 	Donald J. Dovey
...
static void read_hid_file( FILE *infile );
static void create_poly_table();
static void create_edge_table();
static void bbox_polys();
static void gen_hidden();
static void output_hidden( FILE *outfile );
static void vec_norm( Real_type *vec );
static void point_transform( Real_type *res, Real_type *vec, Transf_mat *mat);
...
*/
    ...
    while ( strcmp( token, "v" ) == 0 )
    {
        ...
        point_transform( npt, pt, &view_mat );
...
create_poly_table()
{
    ...
    for ( i = 0; i < 4; i++ )
        poly[i] = NEW_N( int, poly_cnt, "Polygons" );
...
create_edge_table()
{
    ...
    for ( i = 0; i < poly_cnt; i++ )
        for ( j = 0; j < 4; j++ )
        {
            or_edge[0][or_edge_cnt] = poly[j][i];
            or_edge[1][or_edge_cnt] = poly[(j+1)%4][i];
            or_edge_cnt++;
        }
...
bbox_polys()
{
    ...
    for ( i = 0; i < poly_cnt; i++ )
    {
        poly_min[0][i] = proj_nodes[0][poly[0][i]];
        poly_max[0][i] = proj_nodes[0][poly[0][i]];
        poly_min[1][i] = proj_nodes[1][poly[0][i]];
        poly_max[1][i] = proj_nodes[1][poly[0][i]];

        for ( j = 1; j < 4; j++ )
        {
            x = proj_nodes[0][poly[j][i]];
            y = proj_nodes[1][poly[j][i]];
 ...
 */
static void
point_transform( Real_type *res, Real_type *vec, Transf_mat *mat)
{
...
/*    if ( !DEF_LT( t1, 0.0 ) && !DEF_LT( t2, 0.0 ) ) */
    if ( !DEF_LT2( t1, 0.0 ) && !DEF_LT2( t2, 0.0 ) )
        return( 0 );
    ...
    if ( t1 * t2 < 0.0 )
    {
        ...
        if ( APX_EQ2( VEC_DOT(line_dir, plane_norm), 0.0 ) )
            return( 1 );
...
shadow_edge_poly( int npoly, Edge_frag *frag, Edge_frag **nfrag2 )
{
    ...
    for ( i = 0; i < 4; i++ )
        for ( j = 0; j < 3; j++ )
            verts[i][j] = nodes[j][poly[i][npoly]];

    /* Clip shadow edge against the four side planes. */
    for ( i = 0; i < 4; i++ )
    {
        /* Check for degenerate edge. */
        if ( poly[i][npoly] == poly[(i+1)%4][npoly] )
            continue;
        ...
        if ( !clip_line_seg_plane( verts[i], v3, frag->line_pt,
            ...
            return 1;
/*
fprintf( stderr, "Edge after clipping to poly side %d: %f %f\n", i,
...
/*
fprintf( stderr, "Edge after clipping to front face : %f %f\n",
shadow_bounds[0], shadow_bounds[1] );
*/
    /*
    ...
    if ( !DEF_GT2( shadow_bounds[1], shadow_bounds[0] ) )
    {
        /* Just return original fragment. */
        return 1;
    }

    if ( APX_EQ2( shadow_bounds[0], frag->bounds[0] ) )
    {
        if ( DEF_LT2( shadow_bounds[1], frag->bounds[1] ) )
        {
    ...
    else if ( APX_EQ2( shadow_bounds[1], frag->bounds[1] ) )
    {
...
/*
fprintf( stderr, "     Two frags, frag 1            : %f %f\n",
frag->bounds[0], frag->bounds[1] );
fprintf( stderr, "     Two frags, frag 2            : %f %f\n",
(*nfrag2)->bounds[0], (*nfrag2)->bounds[1] );
*/
        return 2;
    }
}
    ...
    for ( i = 0; i < edge_cnt; i++ )
    {
        ...
        for ( j = 0; j < poly_cnt; j++ )
        {
            ...
            for ( done = FALSE, k = 0; k < 4; k++ )
                if ( ( edge[0][i] == poly[k][j] &&
                       edge[1][i] == poly[(k+1)%4][j] ) ||
                     ( edge[1][i] == poly[k][j] &&
                       edge[0][i] == poly[(k+1)%4][j] ) )
                    done = TRUE;
            if ( done )
                continue;
