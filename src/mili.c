...
#define COMMIT_NS(f) \
    (f->access_mode != 'r' && (f->directory[f->file_count - 1].commit_count == 0 || f->non_state_ready))

#define COMMIT_TI(f) \
    (f->access_mode != 'r' && (f->ti_directory[f->ti_file_count - 1].commit_count == 0 || f->non_state_ready))
 ...
 */
int internal_sizes[QTY_PD_ENTRY_TYPES + 1] = {0,           sizeof(char), sizeof(float),   sizeof(float), sizeof(double),
                                              sizeof(int), sizeof(int),  sizeof(LONGLONG)};
                                 ...
                                 Bool_type *create_db, Famid *fam_id)
{
    ...
    else
    {
        ...
        fam_list = RENEW_N(Mili_family *, fam_list, pos, 1, "Big Kahuna pointers");
        if ( fam_list == NULL )
        {
            return ALLOC_FAILED;
        }
 ...
 */
Return_value mc_quick_open(char *root_name, char *path, char *control_string, Famid *p_fam_id)
{
#if TIMER
    clock_t start, stop;
 ...
 * family identifier to caller.
 */
Return_value mc_open(char *root_name, char *path, char *control_string, Famid *p_fam_id)
{
#if TIMER
    clock_t start, stop;
    double cumalative = 0.0;
...
Return_value parse_control_string(char *ctl_str, Mili_family *fam, Bool_type *p_create)
{
    ...
    for ( p_c = ctl_str; p_c != NULL && *p_c != '\0' && rval == OK; p_c++ )
    {
        switch ( *p_c )
        {
            case 'A':
                ...
                switch ( *p_c )
                {
                    ...
                    case 'a':
                        ...
                        else if ( rval == OK || (rval == MAPPED_OLD_HEADER && *p_c == 'r') )
                        {
    ...
    while ( (func_specified_count < QTY_CONTROL_FUNCTIONS) && (rval == OK) )
    {
        switch ( func_idx )
        {
            ...
            case CTL_ENDIAN:
                 ...
                 * set as well.
                 */
                if ( !func_requested[CTL_ACCESS_MODE] || !func_requested[CTL_PRECISION_LIMIT] )
                {
                    break;
                }
                ...
                if ( endian_request != '\0' )
                {
                    ...
                    {
                        if ( (BIG_ENDIAN_HOST && fam->char_header[ENDIAN_IDX] == MILI_BIG_ENDIAN) ||
                             (LITTLE_ENDIAN_HOST && fam->char_header[ENDIAN_IDX] == MILI_LITTLE_ENDIAN) )
                        {
                    ...
                    {
                        if ( endian_request == 'n' || (endian_request == 'b' && BIG_ENDIAN_HOST) ||
...
Return_value open_family(Famid fam_id)
{
    ...
    if ( fam->access_mode == 'a' && fam->file_count > 0 )
    {
        ...
        if ( lkstat != OK )
        {
            ...
            {
                fprintf(stderr,
                        "Mili - Unable to place name lock for "
                        "non-state init.\n");
            }
            return lkstat;
        }
        else
        {
            ...
            if ( filelock_enable )
            {
                write_ct = write(fam->lock_file_descriptor, (void *)p_fname, M_MAX_NAME_LEN);
...
#endif
    status = mc_read_scalar(fam_id, name_file_size_limit, &fam->bytes_per_file_limit);
    if ( status != OK )
    {
    ...
    status = OK;
    status = mc_read_scalar(fam_id, "post_modified", &fam->post_modified);
    if ( status != OK )
    {
        fam->post_modified = 0;
    }
    ...
    if ( !fam->hide_states )
    {
        status = mc_read_scalar(fam_id, name_states_per_file, &fam->states_per_file);
        if ( status != OK )
        {
            return status;
        }
 ...
 */
static Return_value create_family(Mili_family *fam)
{
    Return_value rval;
 ...
 *                           writing process.
 */
static Return_value create_write_lock(Mili_family *fam)
{
    int fd, rval = 0;
    char fname[M_MAX_NAME_LEN];
    char *null_buffer;
 ...
 */
Return_value mc_delete_family(char *root, char *path)
{
    char *lpath;
    char fname[M_MAX_NAME_LEN];
    int fd;
    ...
    for ( i = 0; i < fam_array_length; i++ )
    {
        if ( fam_list[i] != NULL )
        {
            if ( strcmp(fam_list[i]->path, lpath) == 0 && strcmp(fam_list[i]->file_root, root) == 0 &&
 ...
 * FAMILY_NOT_FOUND.
 */
static Return_value delete_family(char *root, char *path)
{
    int qty, rootlen;
    char fname[M_MAX_NAME_LEN];
    int i;
    ...
    for ( i = 0; i < qty; i++ )
    {
        ...
        if ( is_numeric(p_fname + rootlen) || is_all_upper(p_fname + rootlen) )
        {
...
static Return_value init_header(Mili_family *fam)
{
    int write_ct;
 ...
 */
Return_value mc_init_metadata(Famid fam_id)
{
    Return_value rval;
    time_t t;
    char char_t[25];
 ...
 */
Return_value mc_get_metadata(Famid fam_id, char *mili_version, char *host, char *arch, char *timestamp,
                             char *xmilics_version)
{
    mili_version[0] = '\0';
    host[0] = '\0';
    arch[0] = '\0';
 ...
 */
Return_value mc_close(Famid fam_id)
{
    Mili_family *fam;
    Return_value rval;
    char fname[M_MAX_NAME_LEN];
    ...
    if ( filelock_enable && (fam->access_mode == 'w' || fam->access_mode == 'a') )
    {
#if defined(_WIN32) || defined(WIN32)
#else
 ...
 * file names.
 */
Return_value mc_suffix_width(Famid fam_id, int suffix_width)
{
    Mili_family *fam;
    Return_value rval;
 ...
 */
Return_value mc_limit_states(Famid fam_id, int states_per_file)
{
    Mili_family *fam;
    Return_value rval;
    ...
    if ( fam->partition_scheme == STATE_COUNT )
    {
        ...
        rval = mc_wrt_scalar(fam_id, M_INT, name_states_per_file, (void *)&states_per_file);
 ...
 */
Return_value mc_limit_filesize(Famid fam_id, LONGLONG filesize)
{
    Mili_family *fam;
    ...
    rval = mc_wrt_scalar(fam_id, M_INT, name_file_size_limit, (void *)&filesize);
 ...
 */
Return_value mc_flush(Famid fam_id, int data_type)
{
    Mili_family *fam;
    Return_value rval;
    ...
    if ( data_type == STATE_DATA )
    {
        if ( fam->cur_st_file != NULL )
        {
            ...
            make_fnam(STATE_DATA, fam, ST_FILE_SUFFIX(fam, fam->cur_st_index), fname);
...
{
    if ( fam_id < 0 || fam_id >= fam_array_length || fam_list[fam_id] == NULL )
    {
        return BAD_FAMILY;
    }
...
static Return_value test_open_next(Mili_family *fam, Bool_type *open_next)
{
    ...
    *open_next = FALSE;
    ...
    {
        if ( fam->cur_st_file_size >= fam->bytes_per_file_limit || fam->file_st_qty >= fam->states_per_file )
        {
            *open_next = TRUE;
        }
    }
    else if ( fam->bytes_per_file_limit > 0 && fam->cur_st_file_size >= fam->bytes_per_file_limit )
    {
        *open_next = TRUE;
    }
    else if ( (fam->states_per_file > 0 && fam->file_st_qty >= fam->states_per_file) ||
...
Return_value prep_for_new_data(Mili_family *fam, int ftype)
{
    ...
    else if ( ftype == STATE_DATA )
    {
        ...
        if ( fam->partition_scheme == STATE_COUNT )
        {
            ...
            {
                fam->file_map =
                    RENEWC_N(State_file_descriptor, fam->file_map, fam->st_file_count, 1, "New file map entry");
                if ( fam->file_map == NULL )
                {
                    return ALLOC_FAILED;
                }
                /* Open a new file. */
                rval = state_file_open(fam, fam->st_file_count, fam->access_mode);
                if ( rval != OK )
                {
                    return rval;
                }
                ...
                if ( fam->write_tfile )
                {
                    if ( !fam->time_state_file )
                    {
                        ...
                        {
                            rval = fam->write_funcs[M_STRING](fam->time_state_file, &fam->state_end_marker, 1);
...
static Return_value write_header(Mili_family *fam)
{
    ...
    if ( rval == OK )
    {
        write_ct = fwrite((void *)fam->char_header, 1, CHAR_HEADER_SIZE, fam->cur_file);
        if ( write_ct != CHAR_HEADER_SIZE )
        {
            rval = SHORT_WRITE;
        }
...
Return_value state_file_open(Mili_family *fam, int index, char mode)
{
    ...
    if ( fam->cur_st_index != index || fam->cur_st_file_mode != mode )
    {
        ...
        rval = open_buffered(fname, access, &fam->cur_st_file, &fam->cur_st_file_size);
...
Return_value state_map_file_open(Mili_family *fam, char mode)
{
    Return_value rval = OK;
    return rval;
}
 ...
 */
void set_file_access(char fam_access, Bool_type file_exists, char *file_access)
{
    switch ( fam_access )
    {
        case 'w':
...
Return_value state_file_close(Mili_family *fam)
{
    ...
    if ( (fam->access_mode == 'w' || fam->access_mode == 'a') && fam->cur_st_index == fam->st_file_count )
    {
        /*
         * Update contents of the write lock file to indicate last
         * complete state file.
        ...
        make_fnam(STATE_DATA, fam, ST_FILE_SUFFIX(fam, fam->st_file_count - 1), p_fname);
 ...
 */
Return_value mc_partition_state_data(Famid fam_id)
{
    Mili_family *p_fam;
...
Return_value ti_file_open(Famid fam_id, int index, char mode)
{
    ...
    if ( fam->ti_cur_index != index )
    {
        ...
        rval = open_buffered(fname, access, &fam->ti_cur_file, &fam->ti_cur_file_size);
        ...
        if ( rval == OK )
        {
            ...
            if ( index == fcnt )
            {
                fam->ti_directory = RENEW_N(File_dir, fam->ti_directory, fcnt, 1, "File dir head");
            }
        }
        else
        {
 ...
 */
Return_value mc_ti_init_metadata(Famid fam_id)
{
    time_t t;
    char char_t[25];
    char nambuf[70];
 ...
 */
Return_value mc_ti_get_metadata(Famid fam_id, char *mili_version, char *host, char *arch, char *timestamp,
                                char *username, char *xmilics_version, char *code_name)
{
    Mili_family *fam;
 ...
 *.
 */
Return_value mc_ti_check_arch(Famid fam_id)
{
    Mili_family *fam;

    char mili_version[32], host[64], arch[32], timestamp[64], xmilics_version[64];

    char ti_mili_version[32], ti_host[64], ti_arch[32], ti_timestamp[64], ti_user[32], ti_xmilics_version[64],
        ti_code_name[64];
    Return_value rval;
    ...
    rval = mc_get_metadata(fam_id, mili_version, host, arch, timestamp, xmilics_version);
    if ( rval != OK )
    {
        return rval;
    }
    rval = mc_ti_get_metadata(fam_id, ti_mili_version, ti_host, ti_arch, ti_timestamp, ti_user, ti_xmilics_version,
                              ti_code_name);
    if ( rval != OK )
    {
        return rval;
 ...
 */
static int is_correct_param_type(int fam_id, char *param_name, int type)
{
    Mili_family *fam;
 ...
 * param table only.
 */
int mc_htable_search_wildcard(int fid, int list_len, Bool_type allow_duplicates, char *key1, char *key2, char *key3,
                              char **return_list)
{
    int i, count, pos;
    Mili_family *fam;
    ...
    count = htable_search_wildcard(table, list_len, allow_duplicates, key1, key2, key3, temp_list);
    if ( count > 0 )
    {
 ...
 * param table only.
 */
int mc_ti_htable_search_wildcard(int fid, int list_len, Bool_type allow_duplicates, char *key1, char *key2, char *key3,
                                 char **return_list)
{
    int i, count, pos = 0;
    Mili_family *fam;
