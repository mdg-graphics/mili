...
static Return_value set_metrics( Mili_family *fam, int data_org,
                                 Sub_srec *psubrec, LONGLONG *size );
 ...
 */
Return_value
taurus_make_srec( Mili_family *fam, int mesh_id, int *p_srec_id )
{
 ...
 */
static int
make_srec( Mili_family *fam, int mesh_id )
{
   Srec *p_sr;
   int srec_id;
   /*
 ...
 */
Return_value
taurus_def_subrec( Mili_family *fam, int srec_id, char *subrec_name, int org,
                   int qty_svars, char **svar_names,
                   char *mclass, int format, int cell_qty, int *mo_ids,
                   Bool_type first )
{
   Return_value status;
   Srec *psr;
   ...
   status = (Return_value) htable_search( fam->subrec_table, subrec_name,
                                          op, &subrec_entry );
   if ( status != OK )
      return status;
   ...
   status = (Return_value)
            htable_search( fam->srec_meshes[srec_id]->mesh_data.umesh_data,
 ...
 */
static Return_value
set_metrics( Mili_family *fam, int data_org,
             Sub_srec *psubrec, LONGLONG *ret_size )
{
   int k, l;
   int count;
   LONGLONG size;
 ...
 */
Return_value
taurus_get_subrec_def( Famid fam_id, int srec_id, int subrec_id,
                       Subrecord *p_subrec )
{
   Sub_srec *p_ssrec;
   int id_cnt;
...
taurus_get_states_per_file( Mili_family *fam, int ctl[], int *file_sz )
{
   ...
   geom_sz = (ndim*numnp + 9*nel8 + 5*nel4 + 6*nel2)
             * EXT_SIZE( fam, M_FLOAT );
   state_sz = (nvqty*numnp + nel8*nv3dact + nel4*nv2dact + nel2*nv1dact
               + nglbv + 1) * EXT_SIZE( fam, M_FLOAT );
 ...
 */
Return_value
taurus_build_state_map( Mili_family *fam, Bool_type initial_build,
                        int ctl[], int srec_id )
{
   ...
   while ( state_file_open( fam, index, 'r' ) == OK )
   {
      ...
      while ( ( offset == 0
                || fam->cur_st_file_size - offset >= (LONGLONG) state_size )
      ...
      if ( st_time < 0
            || (LONGLONG) state_size <= fam->cur_st_file_size
            || index == end_geom_idx )
      {
      ...
      else
      {
         ...
         while ( state_file_open( fam, index, 'r' ) == OK
                 && ( remainder -= fam->cur_st_file_size ) > 0 );
 ...
 */
Return_value
taurus_commit_srecs( Mili_family *fam )
{
   int i_qty, c_qty;
   int i, j, k;
   Srec *psrec;
