...
static char *agg_names[AGG_TYPE_CNT] = {"Scalar", "Vector", "Array", "Vector array"};

static Return_value create_svar(Mili_family *fam, char *name, Svar **ppsv);
static Return_value input_svars(Mili_family *fam, int file_index, LONGLONG file_offset);
static Return_value find_delete_svar(Mili_family *fam, char *name);
static Return_value get_svar_info(Famid fam_id, char *class_name, char *var_name, int *num_blocks, int *size, int *type,
                                  int *srec_id, int *sub_srec_id, char *agg_svar_name);
 ...
 */
Bool_type valid_svar_data(Aggregate_type atype, char *name, int svar_type, int vec_length, char **components, int rank,
                          int *dims)
{
    int i;

    /* Make these checks for all variable types. */
    if ( (atype < SCALAR || atype > VEC_ARRAY) || (name == NULL || strlen(name) == 0) ||
         (svar_type != M_STRING && svar_type != M_FLOAT && svar_type != M_FLOAT4 && svar_type != M_FLOAT8 &&
          svar_type != M_INT && svar_type != M_INT4 && svar_type != M_INT8) )
    {
        return FALSE;
    }
 ...
 */
Return_value mc_def_svars(Famid fam_id, int qty, char *names, int name_stride, char *titles, int title_stride,
                          int types[])
{
    Mili_family *fam;
    char *p_name, *p_title;
    ...
    for ( i = 0; i < qty; i++, p_name += name_stride, p_title += title_stride )
    {
        if ( valid_svar_data(SCALAR, p_name, types[i], 0, NULL, 0, NULL) )
        {
            ...
            if ( status == OK )
            {
                ...
                psv->agg_type = (Aggregate_type *)ios_alloc((LONGLONG)1, fam->svar_i_ios);
        ...
        else
        {
            ...
            for ( p_name -= name_stride; p_name >= names; p_name -= name_stride )
            {
...
Return_value validate_vector_decl(char *name, int size, char **field_names, Svar *existing_data)
{
    int index;
 ...
 */
Return_value mc_def_vec_svar(Famid fam_id, int type, int size, char *name, char *title, char **field_names,
                             char **field_titles)
{
    Mili_family *fam;
    Svar *psv, *psv2;
    ...
    if ( status == OK )
    {
        ...
        *psv->list_size = size;
        ...
        for ( i = 0; i < size; i++ )
        {
            ...
            status = htable_search(fam->svar_table, sv_name, ENTER_MERGE, &var_entry);
            if ( var_entry == NULL )
            {
                break;
            }
            ...
            if ( status != ENTRY_EXISTS )
            {
                ...
                {
                    len = ios_str_dup(&psv2->title, sv_title, fam->svar_c_ios);
                }
                else
                {
                ...
                psv2->agg_type = (Aggregate_type *)ios_alloc((LONGLONG)1, fam->svar_i_ios);
                ...
                *psv2->agg_type = SCALAR;
                ...
                psv2->data_type = (int *)ios_alloc((LONGLONG)1, fam->svar_i_ios);
 ...
 */
Return_value mc_def_arr_svar(Famid fam_id, int rank, int dims[], char *name, char *title, int type)
{
    Mili_family *fam;
    Return_value status;
    Svar *psv;
 ...
 */
Return_value mc_def_vec_arr_svar(Famid fam_id, int rank, int dims[], char *name, char *title, int size,
                                 char **field_names, char **field_titles, int type)
{
    Mili_family *fam;
    Svar *psv, *psv2;
    char *sv_name, *sv_title, *dumchar;
    ...
    if ( !valid_svar_data(VEC_ARRAY, name, type, size, field_names, rank, dims) )
    {
        return INVALID_SVAR_DATA;
    }
    if ( fam->svar_table != NULL && htable_search(fam->svar_table, name, FIND_ENTRY, &var_entry) == OK )
...
static Return_value create_svar(Mili_family *fam, char *name, Svar **ppsv)
{
    ...
    if ( fam->svar_table == NULL )
    {
         ...
         */
        fam->svar_hdr = (int *)ios_alloc((LONGLONG)QTY_SVAR_HEADER_FIELDS, fam->svar_i_ios);
 ...
 */
Return_value mc_get_svar_def(Famid fam_id, char *svar_name, State_variable *p_stvar)
{
    Mili_family *fam;
    Htable_entry *p_hte;
    Return_value rval;
    ...
    if ( p_stvar->agg_type != SCALAR )
    {
        if ( p_stvar->agg_type != VECTOR )
        {
            ...
            p_stvar->dims = NEW_N(int, p_stvar->rank, "State_variable arr dims");
        ...
        if ( p_stvar->agg_type == VEC_ARRAY || p_stvar->agg_type == VECTOR )
        {
            ...
            p_stvar->components = NEW_N(char *, p_stvar->vec_size, "Vector array component names");
            if ( p_stvar->vec_size > 0 && p_stvar->components == NULL )
            {
                return ALLOC_FAILED;
            }

            p_stvar->component_titles = NEW_N(char *, p_stvar->vec_size, "Vector array component names");
            ...
            for ( i = 0; i < p_stvar->vec_size; i++ )
            {
                ...
                if ( p_stvar->components[i] == NULL || p_stvar->component_titles[i] == NULL )
                {
                    return ALLOC_FAILED;
                }
            }
...
static Return_value find_delete_svar(Mili_family *fam, char *name)
{
    ...
    if ( phte != NULL )
    {
        rval = delete_svar_with_ios((Svar *)phte->data, fam->svar_c_ios, fam->svar_i_ios);
...
Return_value commit_svars(Mili_family *fam)
{
    ...
    if ( (int_qty - QTY_SVAR_HEADER_FIELDS) > 0 )
    {
        ...
        rval = add_dir_entry(fam, STATE_VAR_DICT, DONT_CARE, DONT_CARE, 0, NULL, fam->next_free_byte, outbytes);
        if ( rval != OK )
        {
            return rval;
        }
        ...
        fam->svar_hdr = ios_alloc((LONGLONG)QTY_SVAR_HEADER_FIELDS, fam->svar_i_ios);
        if ( fam->svar_hdr == NULL )
        {
            return IOS_ALLOC_FAILED;
        }
...
Return_value load_svars(Mili_family *fam)
{
     ...
     */
    if ( fam->svar_table != NULL && (fam->access_mode == 'w' || fam->access_mode == 'a') )
    {
        fam->svar_hdr = (int *)ios_alloc((LONGLONG)QTY_SVAR_HEADER_FIELDS, fam->svar_i_ios);
        if ( fam->svar_hdr == NULL )
        {
...
static Return_value input_svars(Mili_family *fam, int file_index, LONGLONG file_offset)
{
    ...
    status = ios_input(fam, M_INT, (LONGLONG)QTY_SVAR_HEADER_FIELDS, fam->svar_i_ios, (void **)&header);
    if ( status != OK )
    {
        return status;
    }
    status = ios_input(fam, M_INT, (LONGLONG)header[SVAR_QTY_INT_WORDS_IDX] - QTY_SVAR_HEADER_FIELDS, fam->svar_i_ios,
                       (void **)&svar_i);
    if ( status != OK )
    {
        return status;
    }
    status = ios_input(fam, M_STRING, (LONGLONG)header[SVAR_QTY_BYTES_IDX], fam->svar_c_ios, (void **)&svar_c);
    if ( status != OK )
    {
        return status;
    }
        ...
        switch ( *p_sv->agg_type )
        {
            ...
            case VEC_ARRAY:
                ...
                for ( i = 0; i < *p_sv->list_size; i++ )
                {
                    ...
                    status = htable_search(fam->svar_table, p_c, ENTER_MERGE, &phte);
                    if ( phte == NULL )
                    {
                        return status;
                    }
                ...
                for ( i = 0; i < new_qty; i++ )
                {
                    ...
                    status = htable_search(fam->svar_table, p_c, FIND_ENTRY, &phte);
                    if ( phte == NULL )
                    {
                        return status;
                    }
        ...
        switch ( agg )
        {
            ...
            case VECTOR:
                ...
                for ( i = 0; i < rank; i++ )
                {
                    ...
                    if ( i == 0 )
                    {
                        rval = eprintf(&outlen, "%*tComponents: %s", bi + off, name);
                        if ( rval != OK )
                        {
                            free(idata);
                            free(cdata);
 ...
 * For debugging.
 */
static void dump_svar(void *data)
{
    int i;
    Svar *psv = (Svar *)data;
    char *agg_types[] = {"SCALAR", "VECTOR", "ARRAY", "VEC_ARRAY"};

    printf("name=\"%s\" title=\"%s\" data_type=%d agg_type=%s, order=%d dims=", psv->name, psv->title, *psv->data_type,
           agg_types[*psv->agg_type], *psv->order);
 ...
 * associated with the given class.
 */
Return_value mc_get_svar_size(Famid fam_id, char *class_name, char *var_name, int *num_blocks, int *size, int *type)
{
    int srec_id, sub_srec_id;
    char agg_svar_name[512];

    return get_svar_info(fam_id, class_name, var_name, num_blocks, size, type, &srec_id, &sub_srec_id, agg_svar_name);
}
 ...
 */
Return_value mc_get_svar_mo_ids_on_class(Famid fam_id, char *class_name, char *svar_name, int *blocks)
{
    char agg_svar_name[512];
    ...
    rval =
        get_svar_info(fam_id, class_name, svar_name, &num_blocks, &size, &type, &srec_id, &sub_srec_id, agg_svar_name);
    if ( rval != OK )
    {
        return rval;
    }
 ...
 */
Return_value mc_get_svar_on_class(Famid fam_id, int state, char *class_name, char *svar_name, void *result)
{
    char agg_svar_name[512];
    ...
    rval = mc_read_results(fam_id, state, sub_srec_id, 1, read_results_names, result);
...
/* For vector array state variables that contain vectors
 * we need to recursively go down through the nested state
 * variables and add up the total list size. */
int recurse_vec_arr_list_size(Svar *svar)
{
    ...
    else if ( *svar->agg_type == ARRAY || *svar->agg_type == VECTOR )
    {
        list_size += *svar->list_size;
    }
    else if ( *svar->agg_type == VEC_ARRAY )
    {
        for ( i = 0; i < *svar->list_size; i++ )
        {
            list_size += recurse_vec_arr_list_size(svar->svars[i]);
        }
    }
