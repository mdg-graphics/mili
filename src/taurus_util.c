                     ...
                     void *p_info )
{
   ...
   switch( (Query_request_type) which )
   {
      ...
      case QTY_SVARS:
         ...
         break;
      case QTY_NODE_BLKS:
         *((int *) p_info) = taurus_count_node_entries( fam, *int_args,
                             char_args );
         break;
      case QTY_NODES_IN_BLK:
         rval = taurus_get_node_block_size( fam, int_args, char_args,
                                            &blk_size );
         if ( rval == OK )
            *((int *) p_info) = blk_size;
         break;
      ...
      case QTY_ELEM_CONN_DEFS:
         *((int *) p_info) = taurus_count_elem_conn_defs( fam, *int_args,
                             char_args );
         break;
      case QTY_ELEMS_IN_DEF:
         rval = taurus_get_elem_qty_in_def( fam, int_args, char_args,
                                            &elem_qty );
         if ( rval == OK )
            *((int *) p_info) = elem_qty;
         break;
 ...
 */
Return_value
taurus_non_state_file_close( Mili_family *fam, Famid fam_id )
{
