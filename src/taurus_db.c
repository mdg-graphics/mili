...
Return_value taurus_make_umesh(  Famid fam_id, char *mesh_name,
                                 int dim, int *p_mesh_id );
Return_value taurus_make_srec( Mili_family *fam, int mesh_id, int *p_srec_id );
Return_value taurus_build_state_map( Mili_family *fam, Bool_type initial_build,
                                     int ctl[], int srec_id );
Return_value taurus_def_subrec( Mili_family *fam, int srec_id,
                                char *subrec_name, int org, int qty_svars, char **svar_names,
                                char *class, int format, int cell_qty,
                                int *mo_ids, Bool_type first );
Return_value taurus_def_class( Famid fam_id, int mesh_id, int superclass,
                               char *short_name, char *long_name );
Return_value taurus_def_class_idents( Famid fam_id, int mesh_id,
                                      char *short_name, int start, int stop );
Return_value taurus_def_nodes( Famid fam_id, int mesh_id, char *short_name,
                               int start_node, int stop_node );
Return_value taurus_def_conn_seq( Mili_family *fam, int mesh_id,
                                  char *short_name, int start_el, int stop_el );
Return_value taurus_def_svars( Mili_family *fam, int qty, char **names,
                               char **titles, int types[] );
Return_value taurus_def_vec_svar(Mili_family *fam, int type, int size,
...
static int
taurus_open_family( Mili_family *fam, Famid fam_id, char *mode );
Return_value taurus_load_direc( Mili_family *fam, int ctl[] );
Return_value taurus_load_descriptors( Mili_family *fam, Famid fam_id,
 ...
 */
int
taurus_open( char *root_name, char *path, char *mode, Famid *fam_id )
{
   Mili_family *fam;
...
taurus_open_family( Mili_family *fam, Famid fam_id, char *mode )
{
   ...
   offset = 64* EXT_SIZE( fam, M_INT ) +
            (ndim*numnp + 9*nel8 + 5*nel4 + 6*nel2) *
...
taurus_load_direc( Mili_family *fam, int ctl[] )
{
   ...
   c_qty = 15 * sizeof(int);
   str_dup( &title, "title" );
   rval = add_dir_entry( fam, APPLICATION_PARAM, M_STRING, DONT_CARE, 1,
                         &title, 0, c_qty );
   if ( rval != OK )
      return rval;
 ...
 */
Return_value
taurus_load_svars( Mili_family *fam, int ctl[] )
{
   int activity;
   int ia;
   int it;
   ...
   if( nglbv > 0 )
   {
      ...
      {
         rval = taurus_def_svars( fam, 1, global_primal_shorts,
                                  global_primal_longs, types );

         rval = taurus_def_svars( fam, 1, global_primal_shorts +1,
                                  global_primal_longs +1, types );

         rval = taurus_def_svars( fam, 1, global_primal_shorts +2,
                                  global_primal_longs +2, types );

         rval = taurus_def_svars( fam, 3, global_primal_shorts +3,
                                  global_primal_longs +3,
                                  types );
         ...
         {
            /* Material Variables */
            rval = taurus_def_svars( fam, 1, mat_primal_shorts,
                                     mat_primal_longs, types );

            rval = taurus_def_svars( fam, 1, mat_primal_shorts +1,
                                     mat_primal_longs +1, types );

            rval = taurus_def_svars( fam, 1, mat_primal_shorts +2,
                                     mat_primal_longs +2, types );

            rval = taurus_def_vec_svar( fam, types[0], 3,
                                        mat_primal_shorts[3],
                                        mat_primal_longs[3],
                                        mat_primal_shorts + 4,
                                        mat_primal_longs + 4 );
         ...
         if( numrw > 0 )
         {
            rval = taurus_def_svars( fam, 1, wall_primal_shorts,
                                     wall_primal_longs, types );
         }
      }
      else
      {
         /* Nike3d 'Global' data */
         rval = taurus_def_svars( fam, 1, nike_primal_shorts,
                                  nike_primal_longs, types );
      }
   }
   ...
   if( iu == 1 )
   {
      rval = taurus_def_vec_svar( fam, M_FLOAT, ndim,
                                  node_primal_shorts[0], node_primal_longs[0],
                                  node_posit_shorts, node_posit_longs );
   }
   ...
   {
      rval = taurus_def_vec_svar( fam, M_FLOAT, ndim,
                                  node_primal_shorts[1], node_primal_longs[1],
                                  node_vel_shorts, node_vel_longs );
   }
   ...
   {
      rval = taurus_def_vec_svar( fam, M_FLOAT, ndim,
                                  node_primal_shorts[2], node_primal_longs[2],
                                  node_acc_shorts, node_acc_longs );
   }
   ...
   if ( ixd & K_EPSILON_MASK )
   {
      ...
      /* Nodal A2. */
      if ( ixd & A2_MASK )
         rval = taurus_def_svars( fam, 1, flow_short + 2, flow_long + 2,
                                  types );
   }
   ...
   if ( nel8 > 0 )
   {
      if( nv3d > 0 )
      {
         rval = taurus_def_vec_svar( fam, M_FLOAT, 6,
                                     hex_primal_shorts[0], hex_primal_longs[0],
                                     hex_stress_shorts, hex_stress_longs);

         rval = taurus_def_svars( fam, 1,
                                  hex_primal_shorts +1,
                                  hex_primal_longs +1,
                                  types );
      }
   ...
   {
      rval = taurus_def_vec_svar( fam, M_FLOAT, 6,
                                  shell_primal_shorts[0],
                                  shell_primal_longs[0],
                                  stress_comp_shorts,
                                  stress_comp_longs);

      rval = taurus_def_svars( fam, 1,
                               shell_primal_shorts +1,
                               shell_primal_longs +1,
                               types );

      rval = taurus_def_vec_svar( fam, M_FLOAT, 6,
                                  shell_primal_shorts[2],
                                  shell_primal_longs[2],
                                  stress_comp_shorts,
                                  stress_comp_longs);

      rval = taurus_def_svars( fam, 1,
                               shell_primal_shorts +3,
                               shell_primal_longs +3,
                               types );

      rval = taurus_def_vec_svar( fam, M_FLOAT, 6,
                                  shell_primal_shorts[4],
                                  shell_primal_longs[4],
                                  stress_comp_shorts,
                                  stress_comp_longs);

      rval = taurus_def_svars( fam, 1,
                               shell_primal_shorts +5,
                               shell_primal_longs +5,
                               types );

      rval = taurus_def_vec_svar( fam, M_FLOAT, 3,
                                  shell_primal_shorts[6],
                                  shell_primal_longs[6],
                                  bend_comp_shorts,
                                  bend_comp_longs);

      rval = taurus_def_vec_svar( fam, M_FLOAT, 2,
                                  shell_primal_shorts[7],
                                  shell_primal_longs[7],
                                  shear_comp_shorts,
                                  shear_comp_longs);

      rval = taurus_def_vec_svar( fam, M_FLOAT, 3,
                                  shell_primal_shorts[8],
                                  shell_primal_longs[8],
                                  normal_comp_shorts,
                                  normal_comp_longs);

      rval = taurus_def_svars( fam, 3,
                               shell_primal_shorts +9,
                               shell_primal_longs +9,
                               types );

      if ( nv2d == 33 )
      {
         rval = taurus_def_svars( fam, 1,
                                  shell_primal_shorts +12,
                                  shell_primal_longs +12,
                                  types );
      }
      ...
      {
         rval = taurus_def_vec_svar( fam, M_FLOAT, 6,
                                     shell_primal_shorts[13],
                                     shell_primal_longs[13],
                                     strain_comp_short,
                                     strain_comp_long);

         rval = taurus_def_vec_svar( fam, M_FLOAT, 6,
                                     shell_primal_shorts[14],
                                     shell_primal_longs[14],
                                     strain_comp_short,
                                     strain_comp_long);
         ...
         rval = taurus_def_svars( fam, 1,
                                  shell_primal_shorts +15,
                                  shell_primal_longs +15,
                                  types );
      }
   }
 ...
 */
static Return_value
taurus_init_meshes( Mili_family *fam, Famid fam_id, int ctl[], int *p_mesh_id )
{
   Return_value stat;
   int ndim;
   int numnp;
   ...
   stat = taurus_def_class( fam_id, *p_mesh_id, M_MESH, glob_short_name,
                            glob_long_name );
   if ( stat != OK )
      return stat;

   stat = taurus_def_class_idents( fam_id, *p_mesh_id, glob_short_name, 1,
                                   1 );
   if ( stat != OK )
      return stat;
   ...
   if(  nummat > 0  )
   {
      stat = taurus_def_class( fam_id, *p_mesh_id, M_MAT, mat_short_name,
                               mat_long_name );
      if ( stat != OK )
         return stat;
      stat = taurus_def_class_idents( fam_id, *p_mesh_id, mat_short_name, 1,
                                      nummat );
      if ( stat != OK )
         return stat;
   }
   ...
   if( nglbv > 0 )
   {
      if( activity != 2000 )
      {
         ...
         if( numrw > 0 )
         {
            stat = taurus_def_class( fam_id, *p_mesh_id, M_UNIT,
                                     wall_short_name, wall_long_name );
            if ( stat != OK )
               return stat;
            stat = taurus_def_class_idents( fam_id, *p_mesh_id,
                                            wall_short_name, 1, numrw );
            if ( stat != OK )
               return stat;
         }
      }
      ...
      {
         stat = taurus_def_class( fam_id, *p_mesh_id, M_UNIT,
                                  nike_short_name, nike_long_name );
         if ( stat != OK )
            return stat;
         stat = taurus_def_class_idents( fam_id, *p_mesh_id, nike_short_name,
                                         1, nglbv );
         if ( stat != OK )
            return stat;
      }
   ...
   stat = taurus_def_class( fam_id, *p_mesh_id, M_NODE, node_short_name,
                            node_long_name );
   if ( stat != OK )
      return stat;
   ...
   if( nel8 > 0 )
   {
      stat = taurus_def_class( fam_id, *p_mesh_id, M_HEX, hex_short_name,
                               hex_long_name );
      if ( stat != OK )
         return stat;
      stat = taurus_def_conn_seq( fam_list[fam_id], *p_mesh_id,
                                  hex_short_name, 1, nel8 );
      if ( stat != OK )
         return stat;
   }
   ...
   if( nel2 > 0 )
   {
      stat = taurus_def_class( fam_id, *p_mesh_id, M_BEAM, beam_short_name,
                               beam_long_name );
      if ( stat != OK )
         return stat;
      stat = taurus_def_conn_seq( fam_list[fam_id], *p_mesh_id,
                                  beam_short_name, 1, nel2 );
      if ( stat != OK )
         return stat;
   }
   ...
   if( nel4 > 0 )
   {
      stat = taurus_def_class( fam_id, *p_mesh_id, M_QUAD, shell_short_name,
                               shell_long_name );
      if ( stat != OK )
         return stat;
      stat = taurus_def_conn_seq( fam_list[fam_id], *p_mesh_id,
                                  shell_short_name, 1, nel4 );
      if ( stat != OK )
         return stat;
   }
 ...
 */
Return_value
taurus_load_subrec( Mili_family *fam, int ctl[], int mesh_id, int *p_srec_id )
{
   int mo_ids[2];
   int qty_svars;
   int activity;
   ...
   if ( nglbv > 0 )
   {
         ...
         /* See Nike3d source code nik3di.f */
      {
         ...
         if ( nglbv >= 6 )
         {
            mo_ids[0] = 1;
            mo_ids[1] = 1;
            rval = taurus_def_subrec( fam, *p_srec_id, glob_short_name,
                                      RESULT_ORDERED, 6, global_primal_shorts,
                                      glob_short_name, M_BLOCK_OBJ_FMT, 1,
                                      mo_ids, first );
            first = FALSE;
         ...
         if( nummat > 0 && nglbv > 6 && nglbv_avail > 0 )
         {
            ...
            mo_ids[0] = 1;
            mo_ids[1] = qty_mtls;
            rval = taurus_def_subrec( fam, *p_srec_id, mat_short_name,
                                      RESULT_ORDERED, 4, mat_primal_subrec_shorts,
                                      mat_short_name, M_BLOCK_OBJ_FMT, 1,
                                      mo_ids, first );
            first = FALSE;
         ...
         if( nglbv_avail > 0 )
         {
            ...
            rval = taurus_def_subrec( fam, *p_srec_id, wall_short_name,
                                      OBJECT_ORDERED, 1, wall_primal_shorts,
                                      wall_short_name, M_BLOCK_OBJ_FMT, 1,
                                      mo_ids, first );
            first = FALSE;
      ...
      else
      {
         mo_ids[0] = 1;
         mo_ids[1] = nglbv;
         rval = taurus_def_subrec( fam, *p_srec_id, nike_short_name,
                                   RESULT_ORDERED, 1, nike_primal_shorts,
                                   nike_short_name, M_BLOCK_OBJ_FMT, 1,
                                   mo_ids, first );
         first = FALSE;
      }
   }
   ...
   if ( iu == 1 )
   {
      ...
      mo_ids[0] = 1;
      mo_ids[1] = numnp;
      rval = taurus_def_subrec( fam, *p_srec_id, node_short_name,
                                RESULT_ORDERED, iresults, node_primal_shorts,
                                node_short_name, M_BLOCK_OBJ_FMT, 1,
                                mo_ids, first );
      first = FALSE;
   }
   else if ( iv + ia > 0 )
   {
      ...
      mo_ids[1] = numnp;
      rval = taurus_def_subrec( fam, *p_srec_id, node_short_name,
                                RESULT_ORDERED, iresults, &node_primal_shorts[1],
                                node_short_name, M_BLOCK_OBJ_FMT, 1,
                                mo_ids, first );
      first = FALSE;
   }
   ...
   if ( it || icode == 1 )
   {
      mo_ids[0] = 1;
      mo_ids[1] = numnp;
      rval = taurus_def_subrec( fam, *p_srec_id, "Temperature",
                                OBJECT_ORDERED, 1, temp_short,
                                node_short_name, M_BLOCK_OBJ_FMT, 1,
                                mo_ids, first );
      first = FALSE;
   }
   ...
   if ( ixd )
   {
      ...
      if ( ixd & A2_MASK )
         rval = taurus_def_subrec( fam, *p_srec_id, "Flow vars",
                                   OBJECT_ORDERED, 3, flow_short,
                                   node_short_name, M_BLOCK_OBJ_FMT, 1,
                                   mo_ids, first );
      else if ( ixd & K_EPSILON_MASK )
         rval = taurus_def_subrec( fam, *p_srec_id, "Flow vars",
                                   OBJECT_ORDERED, 2, flow_short,
                                   node_short_name, M_BLOCK_OBJ_FMT, 1,
                                   mo_ids, first );
      first = FALSE;
   }
   ...
   if ( nel8 > 0 )
   {
      if( nv3d > 0 )
      {
         mo_ids[0] = 1;
         mo_ids[1] = nel8;
         rval = taurus_def_subrec( fam, *p_srec_id, hex_short_name,
                                   OBJECT_ORDERED, 2, hex_primal_shorts,
                                   hex_short_name, M_BLOCK_OBJ_FMT, 1,
                                   mo_ids, first );
         first = FALSE;
      }
   }
   ...
   if ( nel2 > 0 )
   {
      if ( nv1d > 0 )
      {
         mo_ids[0] = 1;
         mo_ids[1] = nel2;
         rval = taurus_def_subrec( fam, *p_srec_id, beam_short_name,
                                   OBJECT_ORDERED, 6, beam_primal_shorts,
                                   beam_short_name, M_BLOCK_OBJ_FMT, 1,
                                   mo_ids, first );
         first = FALSE;
      }
   }
   ...
   if ( nel4 > 0 )
   {
      if ( nv2d > 0 )
      {
         ...
         rval = taurus_def_subrec( fam, *p_srec_id, shell_short_name,
                                   OBJECT_ORDERED, qty_svars, p_shell_shorts,
                                   shell_short_name, M_BLOCK_OBJ_FMT, 1,
                                   mo_ids, first );
         first = FALSE;
      }
   }
   ...
   if( activity >= 1000 && activity <= 1005 )
   {
      ...
      if ( nel8 > 0 )
      {
         mo_ids[0] = 1;
         mo_ids[1] = nel8;
         rval = taurus_def_subrec( fam, *p_srec_id, "sand",
                                   OBJECT_ORDERED, 1, sand_db_short,
                                   hex_short_name, M_BLOCK_OBJ_FMT, 1,
                                   mo_ids, first );
         first = FALSE;
      }
 ...
 */
Return_value
taurus_load_descriptors( Mili_family *fam, Famid fam_id,
                         int ctl[], int *p_srec_id )
{
   Return_value stat;
   int p_mesh_id;
 ...
 */
extern Return_value
taurus_non_state_file_open( Mili_family *fam, int index, char mode )
{
   char fname[128];
   int fcnt, cur_qty;
   Return_value rval;
 ...
 */
static void
taurus_cleanse( Mili_family *fam )
{
   free( fam->root );
   free( fam->path );
   free( fam->file_root );
