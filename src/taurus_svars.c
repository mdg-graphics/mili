...
extern Return_value delete_svar_with_ios(Svar *psv, IO_mem_store *pcioms, IO_mem_store *piioms);
        ...
        if ( valid_svar_data(SCALAR, names[i], types[i], 0, NULL, 0, NULL) )
        {
            ...
            if ( status == OK )
            {
                ...
                if ( strlen(titles[i]) > 0 )
                    len = ios_str_dup(&psv->title, titles[i], fam->svar_c_ios);
                else
                    len = ios_str_dup(&psv->title, names[i], fam->svar_c_ios);
                if ( len == 0 )
                    return IOS_STR_DUP_FAILED;
    ...
    if ( status == OK )
    {
        ...
        *psv->list_size = size;
        ...
        for ( i = 0; i < size; i++ )
        {
            ...
            status = (Return_value)htable_search(fam->svar_table, sv_name, ENTER_MERGE, &var_entry);
            if ( var_entry == NULL )
            {
                break;
            }
...
static Return_value taurus_create_svar(Mili_family *fam, char *name, Svar **ppsv)
{
    ...
    status = (Return_value)htable_search(fam->svar_table, name, op, &phte);
...
static Return_value find_delete_svar(Mili_family *fam, char *name)
{
    ...
    rval = (Return_value)htable_search(fam->svar_table, name, FIND_ENTRY, &phte);
    if ( rval == OK )
    {
...
Return_value taurus_commit_svars(Mili_family *fam)
{
    ...
    if ( int_qty - QTY_SVAR_HEADER_FIELDS > 0 )
    {
        ...
        /* Add directory entry. */
        rval =
            add_dir_entry(fam, STATE_VAR_DICT, DONT_CARE, DONT_CARE, 0, NULL, (LONGLONG)DONT_CARE, (LONGLONG)DONT_CARE);
        if ( rval != OK )
            return rval;
