 ...
 */
Return_value mc_query_family(Famid fam_id, int which, void *num_args, char *char_args, void *p_info)
{
    Mili_family *fam;
    Return_value rval;
    int blk_size;
    ...
    switch ( (Query_request_type)which )
    {
        ...
        case QTY_ELEM_CONN_DEFS:
            *((int *)p_info) = count_elem_conn_defs(fam, *int_args, char_args);
            break;
        ...
        case NEXT_STATE_LOC:
            p_i = (int *)p_info;
            rval = gen_next_state_loc(fam, int_args[0], int_args[1], p_i, p_i + 1);
            break;
        case CLASS_EXISTS:
            mesh_id = int_args[0];
        ...
        case QTY_FACETS_IN_SURFACE:
            ...
            {
                id = int_args[1];
                *((int *)p_info) = ((Mesh_object_class_data *)p_hte->data)->surface_sizes[id - 1];
            }
            break;
        default:
 ...
 */
Return_value mc_cleanse_subrec(Subrecord *p_subrec)
{
    int i;
 ...
 */
Return_value mc_cleanse_st_variable(State_variable *p_svar)
{
    int i;
    ...
    {
        if ( (p_svar->agg_type != SCALAR) && (p_svar->agg_type != VECTOR) && (p_svar->dims != NULL) )
        {
            free(p_svar->dims);
            p_svar->dims = NULL;
        }
 ...
 */
Return_value mc_get_mesh_id(Famid fam_id, char *mesh_name, int *p_mesh_id)
{
    Mili_family *fam;
    int i;
 ...
 * classes in a mesh.
 */
Return_value mc_set_buffer_qty(Famid fam_id, int mesh_id, char *class_name, int buf_qty)
{
    Mili_family *fam;
        ...
        if ( fam->srec_meshes[i] == p_mesh )
        {
            ...
            for ( j = 0; j < p_srec->qty_subrecs; j++ )
            {
                ...
                if ( p_subrec->organization == OBJECT_ORDERED &&
                     (reset_all_classes || strcmp(class_name, p_subrec->mclass) == 0) )
                {
                    /* Have class match; re-init buffer queue. */
                    rval = init_buffer_queue(p_subrec->ibuffer, buf_qty, p_subrec->ibuffer->buffer_size);
                    if ( rval != OK )
                    {
                        rval_accum = rval;
                    }
...
Return_value init_buffer_queue(Buffer_queue *p_bq, int buf_qty, long length)
{
    ...
    if ( p_bq->buffer_count != 0 && (p_bq->buffer_size != length || buf_qty == 0) )
    {
    ...
    if ( p_bq->buffer_count == 0 && buf_qty != 0 )
    {
        ...
        p_bq->data_buffers = (void **)NEW_N(float *, buf_qty, "Input buffer queue");
        p_bq->state_numbers = NEW_N(int, buf_qty, "Input queue states");
        if ( (buf_qty > 0) && (p_bq->data_buffers == NULL || p_bq->state_numbers == NULL) )
        {
            if ( alloc_sz > 0 )
            {
    ...
    else if ( p_bq->buffer_count > buf_qty )
    {
        ...
        for ( i = 0, gone = p_bq->recent; i < p_bq->buffer_count - buf_qty; i++ )
        {
        ...
        p_bq->data_buffers = RENEW_N(void *, p_bq->data_buffers, p_bq->buffer_count, buf_qty - p_bq->buffer_count,
                                     "Reduced input buffer queue");
        p_bq->state_numbers = RENEW_N(int, p_bq->state_numbers, p_bq->buffer_count, buf_qty - p_bq->buffer_count,
    ...
    else if ( p_bq->buffer_count < buf_qty )
    {
        ...
        p_bq->data_buffers =
            RENEW_N(void *, p_bq->data_buffers, p_bq->buffer_count, cnt, "Extended input buffer queue");
        p_bq->state_numbers = RENEW_N(int, p_bq->state_numbers, p_bq->buffer_count, cnt, "Extended input queue states");
        if ( (cnt > 0) && (p_bq->data_buffers == NULL || p_bq->state_numbers == NULL) )
        {
            if ( alloc_sz > 0 )
            {
                for ( i = 0; i < alloc_sz; i++ )
...
{
    LONGLONG rlen, flen;
    int count = 0, found_A = 0, rval, multifiles_found = 0, single_file = 0, start_check, numbers = 0;
    ...
    do
    {
        if ( (entry = readdir(dirp)) != NULL )
        {
            ...
            if ( (strncmp(family->file_root, entry->d_name, rlen) == 0) )
            {
                ...
                for ( start_check = strlen(family->file_root); start_check < strlen(entry->d_name) && !found_A;
                      start_check++ )
                {
    ...
    do
    {
        if ( strncmp(fd.cFileName, family->file_root, rlen) == 0 )
        {
            ...
            if ( (strncmp(family->file_root, fd.cFileName, rlen) == 0) )
            {
                ...
                // Check for how many processor files.
                for ( start_check = rlen; start_check < strlen(fd.cFileName) && !found_A; start_check++ )
                {
        ...
        FindNextFileA(dirHandle, &fd) );  // Continues a file search from a previous call to the FindFirstFile function
 ...
 * for a root name.
 */
Return_value mc_countDomains(char *path, char *root, int *domCount)
{
    StringArray sarr;
    int i, len = 0, qty = 0;
    char fname[256];
    ...
    for ( i = 0; i < qty; i++ )
    {
        ...
        if ( !isdigit(fname[len - 1]) )
        {
            if ( fname[len - 1] == '_' && /* Ignore TI_A files */
                 fname[len - 2] == 'I' && fname[len - 3] == 'T' )
            {
                continue;
            }

            if ( isdigit(fname[len - 2]) && /* Parallel case */
                 isdigit(fname[len - 3]) && isdigit(fname[len - 4]) )
            {
                (*domCount)++;
            }
        }
...
void swap_bytes(int qty, long field_size, void *p_source, void *p_destination)
{
    ...
    for ( bound = ((unsigned char *)p_source) + qty * field_size, p_src = p_source, p_dest = p_destination;
          p_src < bound; p_src += field_size, p_dest += field_size )
    {
 ...
 */
void mc_print_error(char *preamble, int rval)
{
    ...
    char *dir_wrt_conflict = "Append access precluded by directory version mismatch.";
    char *hdr_wrt_conflict = "Append access precluded by header version mismatch.";
    ...
    switch ( rval )
    {
        ...
        case (int)BAD_CONTROL_STRING:
            fprintf(stderr,
                    "%sMili - Family open control string format "
                    "error.\n",
                    pre);
            break;
        ...
        case (int)MAPPED_OLD_HEADER:
            fprintf(stderr,
                    "%sMili - Mapped old header format into new "
                    "format.\n",
                    pre);
            break;
        ...
        case (int)IOS_ALLOC_FAILED:
            fprintf(stderr,
                    "%sMili - Error allocating memory from IO "
                    "Store.\n",
                    pre);
            break;
        ...
        case (int)INVALID_SVAR_DATA:
            fprintf(stderr,
                    "%sMili - Invalid state variable definition "
                    "data.\n",
                    pre);
            break;
        ...
        case (int)IOS_OUT_OF_SYNC:
            fprintf(stderr,
                    "%sMili - Actual/expected data in IO store out "
                    "of sync.\n",
                    pre);
            break;
        ...
        case (int)OVERSIZED_SARRAY:
            fprintf(stderr,
                    "%sMili - StingArray buffer exceeds maximum "
                    "addressable length.\n",
                    pre);
            break;
        case (int)FILE_FAMILY_NAME_MISMATCH:
            fprintf(stderr,
                    "%sMili - Family root does not exist in file "
                    "name.\n",
                    pre);
            break;
        ...
        case (int)DATA_PRECISION_MISMATCH:
            fprintf(stderr,
                    "%sMili - Writing to lower or reading from higher "
