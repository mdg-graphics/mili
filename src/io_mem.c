...
ios_create( int datatype )
{
   ...
   {
      case M_STRING:
         piomb->data = (void *) NEW_N( char, ALLOC_UNITS,
                                       "char io mem data" );
         pioms->type = M_STRING;
         break;
      case M_INT:
         piomb->data = (void *) NEW_N( int, ALLOC_UNITS,
                                       "int io mem data" );
         pioms->type = M_INT;
         break;
      case M_FLOAT:
         piomb->data = (void *) NEW_N( float, ALLOC_UNITS,
                                       "float io mem data" );
         pioms->type = M_FLOAT;
         break;
   }
           ...
           IO_mem_store *pioms, void **new_data )
{
   ...
   pioms->data_buffers = RENEW_N( IO_mem_buffer *, pioms->data_buffers,
                                  index, 1, "Load IO buffer list" );
   if (pioms->data_buffers == NULL)
   {
      return ALLOC_FAILED;
   }
   ...
   switch( datatype )
   {
      case M_STRING:
         ...
         rd_cnt = fam->read_funcs[M_STRING]( fam->cur_file, piomb->data,
                                             qty_units );
         break;
      case M_INT:
         ...
         rd_cnt = fam->read_funcs[M_INT]( fam->cur_file, piomb->data,
                                          qty_units );
         break;
      case M_FLOAT:
         piomb->data = (void *) NEW_N( float, qty_units,
                                       "float io mem data" );
         if (qty_units > 0 && pioms->data_buffers == NULL)
         {
            free(pioms->data_buffers);
         ...
         rd_cnt = fam->read_funcs[M_FLOAT]( fam->cur_file, piomb->data,
                                            qty_units );
         break;
      default:
         if ( mili_verbose )
         {
            fprintf(stderr,
                    "Mili Warning in ios_input--unhandled data type.\n" );
         }
              ...
              IO_mem_store *pioms, void **new_data )
{
   ...
   switch( datatype )
   {
      case M_STRING:
         ...
         rd_cnt = fam->read_funcs[M_STRING]( fam->ti_cur_file, piomb->data,
                                             qty_units );
         break;
      case M_INT:
         piomb->data = (void *) NEW_N( int, qty_units, "int io mem data" );
         ...
         rd_cnt = fam->read_funcs[M_INT]( fam->ti_cur_file, piomb->data,
                                          qty_units );
         break;
      case M_FLOAT:
         piomb->data = (void *) NEW_N( float, qty_units,
         ...
         rd_cnt = fam->read_funcs[M_FLOAT]( fam->ti_cur_file, piomb->data,
                                            qty_units );
         break;
      default:
         if ( mili_verbose )
         {
            fprintf(stderr,
                    "Mili Warning in ti_ios_input--unhandled data type.\n" );
         }
         return UNKNOWN_DATA_TYPE; /* Fail status. */
   }
...
ios_alloc( LONGLONG qty, IO_mem_store *pioms )
{
   ...
   if ( piomb->used + qty > piomb->size )
   {
      ...
      pioms->data_buffers = RENEW_N( IO_mem_buffer *,
                                     pioms->data_buffers,
                                     pioms->current_index + 1,
                                     1,
                                     "Grow IO data buffer list" );
      if ( pioms->data_buffers == NULL )
      {
         /* Uh oh; punt. */
         return NULL;
...
ios_unalloc( void *pmem, int qty_units, IO_mem_store *pioms )
{
   ...
   switch( pioms->type )
   {
      ...
      default:
         ...
         {
            fprintf(stderr,
                    "Mili Warning in ios_unalloc--unhandled data type.\n" );
         }
         return UNKNOWN_DATA_TYPE; /* Fail status. */
   }
   ...
   {
      if ( pmem >= (*ppiomb)->data &&
            (char *) pmem < (char *) (*ppiomb)->data
            + (*ppiomb)->used * unit_size )
      {
         piomb = *ppiomb;
         break;
      }
   ...
   if ( ppiomb == bound || piomb == NULL)
   {
      ...
      {
         fprintf( stderr,
                  "Mili - Cannot locate IO buffer memory to unallocate.\n" );
      }
   ...
   if ( (char *) pmem < (char *) piomb->data + piomb->output * unit_size )
   {
      ...
      {
         fprintf( stderr,
                  "Mili Warning - unallocated memory already written.\n" );
      }
   }

   /* Calculate unit indices for range of invalid units. */
   start = (int) (((unsigned long) ((char *) pmem - (char *) piomb->data))
                  / unit_size);
   ...
   if ( rval == OBJECT_RANGE_OVERLAP )
   {
      if ( mili_verbose )
      {
         fprintf( stderr, "Mili - Unable to unallocate IO buffer "
                  "memory.\n" );
      }
      return IOS_NO_VIABLE_BUFFER; /* Fail status. */
   }
 ...
 */
Return_value
ios_output( Mili_family *fam, FILE *ofile, IO_mem_store *pioms,
   ...
   *num_items_written = 0;
   ...
   switch( dtype )
   {
      ...
      default:
         ...
         {
            fprintf(stderr,
                    "Mili Warning in ios_output--unhandled data type.\n" );
         }
         return UNKNOWN_DATA_TYPE; /* Fail status. */
   }
 ...
 */
void *
ios_get_data( IO_mem_store *pioms, int unit_offset )
{
   IO_mem_buffer **ppiomb, **bound;
   IO_mem_buffer *piomb;
   int rem, sum;
 ...
 */
Return_value
ios_traverse_init( IO_mem_store *pioms, int last )
{
   if ( pioms == NULL )
   {
      return NULL_POINTER;
   ...
   else if ( pioms->traverse_index < pioms->current_index )
   {
      ...
      if ( pioms->traverse_index < pioms->current_index )
      {
         if ( piomb->invalid != NULL )
         {
            if ( mili_verbose )
            {
               fprintf( stderr, "Mili - Invalid data encountered in "
                        "IO Store.\n" );
            }
   ...
   if ( *pp_data != NULL )
   {
      ...
      if ( slen == rem )
      {
         ...
         else
         {
            ...
            {
               pioms->traverse_next = (char *)
                                      pioms->data_buffers[idx]->data;
               ...
               if ( pioms->data_buffers[idx]->invalid != NULL )
               {
                  if ( mili_verbose )
                  {
                     fprintf( stderr, "Mili - Invalid data encountered "
                              "in IO store.\n" );
                  }
                  return IOS_INVALID_DATA;
               }
            ...
            else
            {
               ...
               *pp_data = NULL;
               if ( mili_verbose )
               {
                  fprintf( stderr, "Mili - NULL buffers encountered"
                           " in IO store.\n" );
               }
               return IOS_INVALID_DATA;
            }
