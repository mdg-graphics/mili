 ...
 * Create a hash table.
 */
Hash_table *htable_create(int table_size)
{
    Hash_table *pht = NEW(Hash_table, "Hash table struct");
    if ( pht != NULL )
    {
        pht->table = NEW_N(Htable_entry *, table_size, "Hash table bucket array");
        if ( pht->table == NULL )
        {
            free(pht);
            pht = NULL;
 ...
 */
Return_value htable_search(Hash_table *table, char *key, Hash_action op, Htable_entry **entry)
{
    int bucket;
    Htable_entry *phte = NULL, *phte2 = NULL;
 ...
 *
 */
Return_value ti_htable_search(Hash_table *table, char *key, Hash_action op, Htable_entry **entry)
{
    int bucket;
    ...
    for ( i = 0; i < len; i++ )
    {
        if ( i < len - 4 )
        {
            if ( temp_key[i] == '/' && temp_key[i + 1] == '+' && temp_key[i + 2] == '+' )
            {
                temp_key[i] = '\0';
                break;
            }
 ...
 */
void htable_del_entry(Hash_table *table, Htable_entry *entry)
{
    int bucket;
 ...
 */
void htable_delete(Hash_table *table, void (*user_func)(void *), int free_data)
{
    int i;
    Htable_entry *phte, *phte2;
    int tsize;
 ...
 */
Return_value htable_add_entry_data(Hash_table *table, char *key, Hash_action op, void *entry_data)
{
    Htable_entry *phte;
    Return_value rval = OK;
 ...
 * Add the supplied data to the entry associated with key.
 */
Return_value ti_htable_add_entry_data(Hash_table *table, char *key, Hash_action op, void *entry_data)
{
    Htable_entry *phte;
    Return_value rval;
 ...
 */
Return_value htable_get_data(Hash_table *htable, void ***data_ptrs, int *num_entries)
{
    int i, j;
    void **pv;
    Htable_entry *phte;
 ...
 */
void htable_dump(Hash_table *table, int compress, void (*user_data_dump)())
{
    Htable_entry **hta;
    int empties;
    int tsize;
    ...
    printf("Table size = %d; quantity entries = %d.\n", table->size, table->qty_entries);

    empties = 0;

    for ( i = 0, hta = table->table; i < tsize; i++ )
    {
        if ( (empties == 0 || !compress) || (compress && empties > 0 && hta[i] != NULL) )
        {
            printf("\n%4d", i + 1);
        }
 ...
 */
void htable_prt_error(int return_code)
{
 ...
 * a wildcard search.
 */
void htable_delete_wildcard_list(int list_count, char **wildcard_list)
{
    int i;
 ...
 */
int htable_search_wildcard(Hash_table *table, int list_len, Bool_type allow_duplicates, char *key1, char *key2,
                           char *key3, char **return_list)
{
    Htable_entry *phte;
    ...
    Bool_type dup_match = FALSE, key1_match = TRUE, key2_match = TRUE, key3_match = TRUE;
            ...
            do
            {
                ...
                else
                {
                    if ( key1_match && key2_match && key3_match )
                    {
                        ...
                        {
                            str_dup(&return_list[return_list_count], phte->key);
 ...
 */
static int getMatchType(char *input)
{
    int end_target;
    int i;          // counter
    ...
    if ( !input || strlen(input) == 0 || strcmp(input, "NULL") == 0 )
    {
        return -1;
    }
 ...
 */
Bool_type str_begins_with(char *str, char *substr)
{
 ...
 * This function will check the beginning of str for substr
 */
Bool_type str_ends_with(char *str, char *substr)
{
    Bool_type result = FALSE;
...
static int match(char *target, char *key, int match_type)
{
    // types are absolute (no star)= 1
    // begining match ( something*)= 2
    // end match (*something)      = 3
