...
static void read_lines( int nline, int offset,  int base_size, int last_line,
                        int *data, char *line_buffer, FILE *file);
                         ...
                         Partition_mili **mapin )
{
   ...
   if ( map->nproc > 1 )
   {
      map->nnp  = NEW_N( int, map->nproc,
                         "Number of Nodal Points (local)" );
      map->ncso = NEW_N( int, map->nproc,
                         "Number of Contact-surface-only nodes (local)" );
      map->nelh = NEW_N( int, map->nproc,
                         "Number of Hexahedral Elements (local)" );
      map->nelb = NEW_N( int, map->nproc,
                         "Number of Beam Elements (local)" );
      map->nels = NEW_N( int, map->nproc,
                         "Number of Shell Elements (local)" );
      map->nelt = NEW_N( int, map->nproc,
                         "Number of Thick-Shell Elements (local)" );
      map->neld = NEW_N( int, map->nproc,
                         "Number of Discrete Elements (local)" );
      ...
      {
         fgets(line_buffer, 100, map->partition_file);
         read_lines( nline, 0,  10, last_line,
                     map->nnp, line_buffer, map->partition_file);
      }
      ...
      if (map->nelhg != 0)
      {
         fgets(line_buffer, 100, map->partition_file);
         read_lines( nline, 0,  10, last_line,
                     map->nelh, line_buffer, map->partition_file);
      }
      ...
      if (map->nelbg != 0)
      {
         fgets(line_buffer, 100, map->partition_file);
         read_lines( nline, 0,  10, last_line,
                     map->nelb, line_buffer, map->partition_file);
      }
      ...
      if (map->nelsg != 0)
      {
         fgets(line_buffer, 100, map->partition_file);
         read_lines( nline, 0,  10, last_line,
                     map->nels, line_buffer, map->partition_file);
      }
      ...
      if (map->neltg != 0)
      {
         fgets(line_buffer, 100, map->partition_file);
         read_lines( nline, 0,  10, last_line,
                     map->nelt, line_buffer, map->partition_file);
      }
      ...
      if (map->nnpg != 0)
      {
         ...
         for (proc = 0; proc < map->nproc; proc++)
         {
            if (map->nnp[proc] != 0)
            {
               ...
               map->ncso[proc] = 0;
               read_lines( nline, offset,  10, last_line,
                           map->npltg, line_buffer, map->partition_file);
            }
            offset += map->nnp[proc];
         }
      }
      ...
      if (map->nelhg != 0)
      {
         ...
         map->elhltg = NEW_N( int, nelhtot,
                              "Hexahedral Element Local-to-Global Map" );
         offset = 0;
         for (proc = 0; proc < map->nproc; proc++)
         {
            if (map->nelh[proc] != 0)
            {
               ...
               fgets(line_buffer, 100, map->partition_file);
               read_lines( nline, offset,  10, last_line,
                           map->elhltg, line_buffer, map->partition_file);
            }
      ...
      if (map->nelbg != 0)
      {
         ...
         map->elbltg = NEW_N( int, nelbtot,
                              "Beam Element Local-to-Global Map" );
         offset = 0;
         for (proc = 0; proc < map->nproc; proc++)
         {
            if (map->nelb[proc] != 0)
            {
               ...
               fgets(line_buffer, 100, map->partition_file);
               read_lines( nline, offset,  10, last_line,
                           map->elbltg, line_buffer, map->partition_file);
            }
      ...
      if (map->nelsg != 0)
      {
         ...
         map->elsltg = NEW_N( int, nelstot,
                              "Shell Element Local-to-Global Map" );
         offset = 0;
         for (proc = 0; proc < map->nproc; proc++)
         {
            if (map->nels[proc] != 0)
            {
               ...
               fgets(line_buffer, 100, map->partition_file);
               read_lines( nline, offset,  10, last_line,
                           map->elsltg, line_buffer, map->partition_file);
            }
      ...
      if (map->neldg != 0)
      {
         map->eldltg = NEW_N( int, map->neldg,
                              "Discrete Element Local-to-Global Map" );
      }
 ...
 */
void
free_partition_mili( Partition_mili **mapin )
{
...

{
   ...
   num_entries = htable_search_wildcard(fam->ti_param_table, 0, FALSE,
                                        "node.map", "State-0", "NULL",
                                        wildcard_list );
   if ( num_entries >0 )
   {
      status = mc_ti_read_array( fam_id, wildcard_list[0],
                                 (void **) &temp_ptr, &num_items_read ) ;
   ...
   if (alloc_mem)
   {
      map->elbltg = NEW_N( int, nelstot,
                           "Beam Element Local-to-Global Map" );
   }

   num_entries = htable_search_wildcard(fam->ti_param_table, 0, FALSE,
                                        "beam.map", "State-0", "NULL",
                                        wildcard_list );
   if ( num_entries >0 )
   {
      status = mc_ti_read_array( fam_id, wildcard_list[0],
...

{
   ...
   num_entries = htable_search_wildcard(fam->ti_param_table, 0, FALSE,
                                        "numnp_local", "State-0", "NULL",
                                        wildcard_list );
   if ( num_entries >0 )
   {
      status = mc_ti_read_scalar( fam_id, wildcard_list[0],
                                  &map->nnp[proc] ) ;
   }

   num_entries = htable_search_wildcard(fam->ti_param_table, 0, FALSE,
                                        "numelh_local", "State-0", "NULL",
                                        wildcard_list );
   if ( num_entries >0 )
   {
      status = mc_ti_read_scalar( fam_id, wildcard_list[0],
...
int dump_partition( char *partition_file_name,
                    Partition_mili *map )
{
 ...
 *
 */
int calc_global_element_number( Partition_mili *map, int obj_type,
                                int local_element, int proc )
{
   int i, offset=0;
