...
static Return_value dump_param_array(LONGLONG (*read_funcs[])(), FILE *p_f, Dir_entry dir_ent, char *dir_string,
                                     Dump_control *p_dc, int head_indent, int body_indent);

static Return_value read_param_array_len(Mili_family *fam, Param_ref *p_pr, int *param_array_type,
                                         int *param_array_len);

extern char *dtype_names[QTY_PD_ENTRY_TYPES + 1] = {NULL,      "String",  "Float",     "Float32",
                                                    "Float64", "Integer", "Integer32", "Integer64"};
 ...
 */
Return_value mc_read_scalar(Famid fam_id, char *name, void *p_value)
{
    Mili_family *fam;
    Htable_entry *phte;
    Return_value rval;
 ...
 */
Return_value mc_wrt_scalar(Famid fam_id, int type, char *name, void *p_value)
{
    Mili_family *fam;
    Return_value status;
...
Return_value write_scalar(Mili_family *fam, int type, char *name, void *data, Dir_entry_type etype)
{
    ...
    if ( rval == OK )
    {
        ...
        if ( rval == OK )
        {
            ...
            rval = add_dir_entry(fam, etype, type, SCALAR, 1, &name, fam->next_free_byte, outbytes);
 ...
 */
Return_value mc_read_string(Famid fam_id, char *name, char *p_value)
{
    Mili_family *fam;
    Htable_entry *phte;
    Return_value status;
 ...
 */
Return_value mc_wrt_string(Famid fam_id, char *name, char *value)
{
    Mili_family *fam;
    Return_value status;
...
Return_value write_string(Mili_family *fam, char *name, char *value, Dir_entry_type etype)
{
    ...
    if ( rval == OK )
    {
        ...
        if ( rval == OK )
        {
            ...
            rval = add_dir_entry(fam, etype, M_STRING, DONT_CARE, 1, &name, fam->next_free_byte, c_qty);
            ...
            if ( rval == OK )
            {
                ...
                /* Write the string. */
                write_ct = fam->write_funcs[M_STRING](fam->cur_file, c_buf, c_qty);
                if ( write_ct != c_qty )
                {
                    if ( c_buf != value )
                    {
    ...
    else if ( rval == ENTRY_EXISTS )
    {
        ...
        if ( (LONGLONG)entry[LENGTH_IDX] >= s_len )
        {
            ...
            /* Re-write the string. */
            write_ct = fam->write_funcs[M_STRING](fam->cur_file, c_buf, s_len);
            if ( write_ct != s_len )
            {
                return SHORT_WRITE;
            }
 ...
 */
Return_value mc_read_param_array(Famid fam_id, char *name, void **p_data)
{
    Mili_family *fam;
    Htable_entry *phte;
    Return_value status;
 ...
 */
Return_value mc_read_param_array_len(Famid fam_id, char *name, int *param_array_type, int *param_array_len)
{
    Mili_family *fam;
    Htable_entry *phte;
    Return_value rval;
...
Return_value read_param_array(Mili_family *fam, Param_ref *p_pr, void **p_data)
{
    ...
    switch ( num_type )
    {
        ...
        case M_INT4:
            ...
            p_data[0] = (void *)idata;
            nitems = (fam->read_funcs[M_INT])(fam->cur_file, idata, (LONGLONG)cell_qty);
            length /= 4;
            break;
        ...
        case M_INT8:
            ...
            nitems = (fam->read_funcs[M_INT8])(fam->cur_file, lidata, (LONGLONG)cell_qty);
            length /= 8;
            break;
        ...
        case M_FLOAT4:
            ...
            p_data[0] = (void *)fdata;
            nitems = (fam->read_funcs[M_FLOAT])(fam->cur_file, fdata, (LONGLONG)cell_qty);
            length = (length / 4) - 2;
            break;
        ...
        case M_FLOAT8:
            ...
            p_data[0] = (void *)ddata;
            nitems = (fam->read_funcs[M_FLOAT8])(fam->cur_file, ddata, (LONGLONG)cell_qty);
            length /= 8;
            break;
    }
 ...
 * Read the length of a  parameter array from the referenced family.
 */
static Return_value read_param_array_len(Mili_family *fam, Param_ref *p_pr, int *param_array_type, int *param_array_len)
{
    int file, entry_idx;
    int i;
    LONGLONG offset;
 ...
 */
Return_value mc_wrt_array(Famid fam_id, int type, char *name, int order, int *dimensions, void *data)
{
    Mili_family *fam;
    Return_value status;
    ...
    status = write_array(fam, type, name, order, dimensions, data, APPLICATION_PARAM);
    return status;
}
                         ...
                         Dir_entry_type etype)
{
    ...
    /* Add entry into directory. */
    outbytes = ((order + 1) * EXT_SIZE(fam, M_INT)) + atoms * EXT_SIZE(fam, type);
    rval = add_dir_entry(fam, etype, type, ARRAY, 1, &name, fam->next_free_byte, outbytes);
    if ( rval != OK )
    {
        free(i_buf);
        return rval;
...
int mc_get_app_parameter_count(Famid fam_id)
{
    ...
    count = htable_search_wildcard(fam->param_table, 0, FALSE, "*", "NULL", "NULL", list);
    list = (char **)malloc(count * sizeof(char *));
    count = htable_search_wildcard(fam->param_table, 0, FALSE, "*", "NULL", "NULL", list);
    for ( i = 0; i < count; i++ )
    {
        if ( strstr(list[i], "Labels[") != NULL || strstr(list[i], "Labels-ElemIds[") != NULL )
        {
            continue;
        }
        status = htable_search(fam->param_table, list[i], FIND_ENTRY, &entry);
        if ( status != OK )
        {
            continue;
        }
                        ...
                        int head_indent, int body_indent)
{
    ...
    {
        return dump_param_array(fam->read_funcs, p_f, dir_ent, dir_strings[0], p_dc, head_indent, body_indent);
    }

    num_type = dir_ent[MODIFIER1_IDX];
    rfunc = fam->read_funcs[num_type];
    param_name = (dir_ent[TYPE_IDX] == MILI_PARAM) ? "MILI PARAM" : "APP PARAM";
    cbuf = NULL;
    hi = head_indent;
    bi = body_indent;
    ...
    switch ( num_type )
    {
        case M_STRING:
            ...
            if ( !p_dc->include_external &&
                 (strcmp(dir_strings[0], "host name") == 0 || strcmp(dir_strings[0], "host type") == 0 ||
            ...
            if ( p_dc->formatted )
            {
                rval = eprintf(&outlen, "%*t%s:%*t%s \"%s\";%*t@%ld;", hi + 1, param_name, hi + 14,
                               dtype_names[num_type], dir_strings[0], hi + 45, offset);
                if ( rval != OK )
                {
                    free(cbuf);
                    return rval;
        ...
        case M_INT4:
            ...
            if ( p_dc->formatted )
            {
                rval = eprintf(&outlen, "%*t%s:%*t%s \"%s\";%*t@%ld;%*t=%d\n", hi + 1, param_name, hi + 14,
                               dtype_names[num_type], dir_strings[0], hi + 45, offset, hi + 57, int_param);
                if ( rval != OK )
                {
                    return rval;
                }
        ...
        case M_INT8:
            ...
            if ( p_dc->formatted )
            {
                rval = eprintf(&outlen, "%*t%s:%*t%s \"%s\";%*t@%ld;%*t=%ld\n", hi + 1, param_name, hi + 14,
                               dtype_names[num_type], dir_strings[0], hi + 45, offset, hi + 57, long_int_param);
                if ( rval != OK )
                {
                    return rval;
                }
        ...
        case M_FLOAT4:
            ...
            if ( p_dc->formatted )
            {
                rval = eprintf(&outlen, "%*t%s:%*t%s \"%s\";%*t@%ld;%*t=%+13.6e\n", hi + 1, param_name, hi + 14,
                               dtype_names[num_type], dir_strings[0], hi + 45, offset, hi + 57, double_param);
                if ( rval != OK )
                {
                    return rval;
                }
    ...
    switch ( num_type )
    {
        ...
        case M_INT4:
            ...
            if ( p_dc->formatted )
            {
                rval = eprintf(&outlen, "%*t%s:%*t%s \"%s\";%*t@%ld;\n", hi + 1, param_name, hi + 14,
                               dtype_names[num_type], handle, hi + 45, offset);
                if ( rval != OK )
                {
                    free(idata);
                    return rval;
                ...
                {
                    rval = eprintf(&outlen, "%*t%d%s%d%s%d ... %d%s%d%s%d", body_indent + 1, idata[0], cell_sp,
                                   idata[1], cell_sp, idata[2], idata[cell_qty - 3], cell_sp, idata[cell_qty - 2],
        ...
        case M_INT8:
            ...
            if ( p_dc->formatted )
            {
                ...
                {
                    rval = eprintf(&outlen, "%*t%ld%s%ld%s%ld ... %ld%s%ld%s%ld", body_indent + 1, lidata[0], cell_sp,
                                   lidata[1], cell_sp, lidata[2], lidata[cell_qty - 3], cell_sp, lidata[cell_qty - 2],
        ...
        case M_FLOAT4:
            ...
            if ( p_dc->formatted )
            {
                ...
                {
                    rval = eprintf(&outlen, f_fmt, body_indent + 1, (double)fdata[0], cell_sp, (double)fdata[1],
                                   cell_sp, (double)fdata[2], (double)fdata[cell_qty - 3], cell_sp,
                                   (double)fdata[cell_qty - 2], cell_sp, (double)fdata[cell_qty - 1]);
                    if ( rval != OK )
                    {
                        free(fdata);
                        return rval;
        ...
        case M_FLOAT8:
            ...
            if ( p_dc->formatted )
            {
                ...
                {
                    rval = eprintf(&outlen, d_fmt, body_indent + 1, ddata[0], cell_sp, ddata[1], cell_sp, ddata[2],
                                   ddata[cell_qty - 3], cell_sp, ddata[cell_qty - 2], cell_sp, ddata[cell_qty - 1]);
                    if ( rval != OK )
                    {
                        free(ddata);
                        return rval;
