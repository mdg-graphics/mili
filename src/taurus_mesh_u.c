 ...
 */
Return_value
taurus_make_umesh(  Famid fam_id, char *mesh_name, int dim, int *p_mesh_id )
{
   Mili_family *fam;
   int i;
   LONGLONG outbytes;
   ...
   if ( fam->mesh_type == UNDEFINED )
   {
      ...
      rval = add_dir_entry( fam, MILI_PARAM, M_INT, SCALAR, 1, &mesh_type,
                            fam->next_free_byte, outbytes );
      if ( rval != OK )
         return rval;

      /* Now create entry for the param table. */
      rval = htable_search( fam->param_table, "mesh type",
                            ENTER_ALWAYS, &phte );
      if (phte == NULL)
      {
         return rval;
   ...
   if ( fam->dimensions == 0 )
   {
      ...
      outbytes = EXT_SIZE( fam, M_INT );
      rval = add_dir_entry( fam, MILI_PARAM, M_INT, SCALAR, 1, &mesh_dims,
                            offset, outbytes );
      if ( rval != OK )
         return rval;
   ...
   rval = add_dir_entry( fam, MILI_PARAM, M_STRING, DONT_CARE, 1,
                         &mname, DONT_CARE, strlen(mesh_name) + 1 );
   if ( rval != OK )
      return rval;
 ...
 */
Return_value
taurus_def_class( Famid fam_id, int mesh_id, int superclass, char *short_name,
                  char *long_name )
{
   Mili_family *fam;
   Hash_table *p_ht;
   ...
   rval = (Return_value) htable_search( p_ht, short_name, ENTER_UNIQUE,
                                        &p_hte );
   ...
   rval = create_class_data( superclass, short_name, long_name,
                             (Mesh_object_class_data**)&p_hte->data );
 ...
 */
static void
taurus_delete_mo_data( void *ptr_mo_data )
{
   Mesh_object_class_data *p_mocd;
 ...
 */
extern void
taurus_delete_mesh( Mili_family *fam )
{
   htable_delete( fam->meshes[0]->mesh_data.umesh_data,
                  taurus_delete_mo_data, TRUE );
 ...
 */
Return_value
taurus_load_nodes( Famid fam_id, int mesh_id, char *short_name, void *data )
{
   Mili_family *fam;
   ...
   rval = (Return_value)
          htable_search( fam->meshes[mesh_id]->mesh_data.umesh_data,
 ...
 */
Return_value
taurus_load_conns( Famid fam_id, int mesh_id, char *short_name, int *conns,
                   int *mats, int *parts )
{
   Mili_family *fam;
   int fil_qty, ent_qty, name_cnt;
   ...
   for ( i = 0; i < fil_qty; i++ )
   {
      ...
      for ( j = 0; j < ent_qty; j++ )
      {
         ...
         {
            /* Found an appropriate entry - load it into memory. */
            rval = taurus_load_conn_def( fam, &i, superclass,
                                         p_de[j], (void *) dest_ptrs,
                                         &max_mat );
            if ( rval != OK )
               return rval;
    ...
    */
   current_max = taurus_get_max_material( fam_id, mesh_id,
                                          &max_mat_short_name );
   if ( cum_max_mat > current_max )
      taurus_def_class_idents( fam_id, mesh_id, max_mat_short_name,
                               current_max + 1, cum_max_mat );
 ...
 */
static int
taurus_get_max_material( Famid fam_id, int mesh_id, char **found_short_name )
{
   int int_args[2];
   ...
   {
      mc_get_simple_class_info( fam_id, mesh_id, M_MAT, j,
 ...
 */
static Return_value
taurus_load_conn_def( Mili_family *fam, int *file_idx, int superclass,
                      Dir_entry dir_ent, void *dest_ptrs, int *p_max_mat )
{
   int *ebuf;
   int *p_src;
   ...
   while( int_qty > 0 )
   {
      ...
      else
      {
          ...
          * needs to be ignored.
          */
         not_valid_qty = (offset / EXT_SIZE( fam, M_INT ) + read_cnt)
                         % 262144;
         if( not_valid_qty > 0 )
 ...
 */
static Return_value
taurus_load_nodes_def( Mili_family *fam, int file_idx, Dir_entry dir_ent,
                       void *coords )
{
   LONGLONG offset;
   float *dest;
   ...
   while( float_qty > 0 )
   {
      ...
      {
         /* Read coordinates from file. */
         read_cnt = fam->read_funcs[M_FLOAT]( fam->cur_file, dest,
                                              float_qty );
         break;
      }
      else
 ...
 */
Return_value
taurus_get_class_qty( Mili_family *fam, int *modifiers, int *class_qty )
{
 ...
 */
int
taurus_count_elem_conn_defs( Mili_family *fam, int mesh_id, char *class_name )
{
   int fil_qty, ent_qty, name_cnt;
 ...
 */
Return_value
taurus_get_elem_qty_in_def( Mili_family *fam, int *int_args, char *short_name,
                            int *elem_qty )
{
 ...
 */
int
taurus_count_node_entries( Mili_family *fam, int mesh_id, char *class_name )
{
   int fil_qty, ent_qty, name_cnt;
 ...
 */
Return_value
taurus_reset_class_data( Mili_family *fam, int mesh_id, Bool_type force_init )
{
   ...
   else if ( fam != taurus_class_data_fam ||
             mesh_id != taurus_class_data_mesh_id )
   {
      if ( taurus_class_data != NULL )
         free( taurus_class_data );
      ...
      rval = htable_get_data( p_md->mesh_data.umesh_data,
                              (void ***) &taurus_class_data,
                              &taurus_all_class_count );
   }
   return rval;
}
