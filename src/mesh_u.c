...
static const char *superclass_names[M_QTY_SUPERCLASS] =
{
   "M_UNIT", "M_NODE", "M_TRUSS", "M_BEAM", "M_TRI", "M_QUAD", "M_TET",
   "M_PYRAMID", "M_WEDGE", "M_HEX", "M_MAT", "M_MESH", "M_SURFACE",
   "M_PARTICLE"
};
...
static Return_value update_nodes_def( Mili_family *fam, int mesh_id,
                                      char *class_name, int start, int stop,
                                      float *coords );
static void delete_umesh( Mesh_descriptor *mesh );
static Return_value add_mesh( Mili_family *fam, Dir_entry dir_ent,
                              char *name_handle );
static Return_value load_nodes_def( Mili_family *fam, int file_idx,
                                    Dir_entry dir_ent, void *coords );
static Return_value load_conn_def( Mili_family *fam, int file_idx,
                                   Dir_entry dir_ent, void *conns );
static Return_value load_conn_def_surface( Mili_family *fam, int file_idx,
      Dir_entry dir_ent, void *conns );
 ...
 */
Return_value
classEnum_to_className( int classEnum, char *className )
{
 ...
 */
Return_value
className_to_classEnum( char *className, int *classEnum )
{
   Return_value rval = OK;
 ...
 */
Bool_type
mc_is_particle_class( Famid fam_id, char *class_name )
{
  int i=0;
  int num_entries=0;
  ...
  num_entries = mc_ti_htable_search_wildcard(fam_id, 0, FALSE,
 ...
 */
Return_value
mc_make_umesh( Famid fam_id, char *mesh_name, int dim, int *p_mesh_id )
{
   Mili_family *fam;
   int i;
   char *mname;
   ...
   if ( fam->mesh_type == UNDEFINED )
   {
      ...
      write_scalar( fam, M_INT, "mesh type", &fam->mesh_type,
                    MILI_PARAM );
   }
   ...
   {
      fam->dimensions = dim;
      write_scalar( fam, M_INT, "mesh dimensions", &fam->dimensions,
                    MILI_PARAM );
   }
 ...
 */
static Return_value
make_umesh( Mili_family *fam, Bool_type initial_create, char *mesh_name,
            int *p_mesh_id )
{
   Mesh_descriptor *pmd;
   char pname[32];
   ...
   fam->meshes = RENEW_N( Mesh_descriptor *, fam->meshes, fam->qty_meshes, 1,
                          "Mesh pointer" );
   if (fam->meshes == NULL)
   {
      return ALLOC_FAILED;
 ...
 */
Return_value
create_class_data( int superclass, char *short_name, char *long_name,
 ...
 */
Return_value
mc_def_class( Famid fam_id, int mesh_id, int superclass, char *short_name,
              char *long_name )
{
   Mili_family *fam;
   Hash_table *p_ht;
   ...
   if (milisilo)
   {
      rval = mc_silo_def_class( fam_id, mesh_id, superclass,
                     ...
                     int stop )
{
   ...
   if (milisilo)
   {
      rval = mc_silo_def_class_idents( fam_id, mesh_id, short_name,
                                       start, stop );
      return ( rval );
   }
#endif
   ...
   rval = mc_query_family( fam_id, CLASS_SUPERCLASS, &mesh_id, short_name,
                           (void *) &superclass );
   if ( rval != OK )
   {
      return rval;
   ...
   rval = add_dir_entry( fam, CLASS_IDENTS, mesh_id, stop - start + 1, 1,
                         &short_name, fam->next_free_byte, rec_size );
   if ( rval != OK )
   {
      return rval;
   }
              ...
              int stop_node, float *coords )
{
   ...
   if (milisilo)
   {
      rval = mc_silo_def_nodes( fam_id, mesh_id, short_name,
   ...
   rec_size = data_words * EXT_SIZE( fam, M_FLOAT ) + QTY_NODE_TAGS
              * EXT_SIZE( fam, M_INT );
   ...
   if ( rval != OK )
   {
      if ( rval == OBJECT_RANGE_OVERLAP )
      {
         ...
         rval = update_nodes_def( fam, mesh_id, short_name, start_node,
                                  stop_node, coords );
      }
   ...
   /* Register an entry in the directory. */
   rval = add_dir_entry( fam, NODES, mesh_id, stop_node - start_node + 1, 1,
                         &short_name, fam->next_free_byte, rec_size );
   if ( rval != OK )
   {
      return rval;
   ...
   tags[0] = start_node;
   tags[1] = stop_node;
   num_items_written = fam->write_funcs[M_INT]( fam->cur_file, tags,
                       QTY_NODE_TAGS );
   ...
   num_items_written = fam->write_funcs[M_FLOAT]( fam->cur_file, coords,
                       data_words );
 ...
 */
Return_value
mc_def_node_labels( Famid fam_id, int mesh_id, char *short_name,
                    int qty, int *labels )
{
   int dims[3];
   Mili_family *fam;
   ...
   status = mc_query_family( fam_id, CLASS_SUPERCLASS, &temp_mesh_id,
                             short_name, (void *) &superclass );
   if (status != OK)
   {
      return status;
   }
   ...
   */
   ...
   status = mc_ti_wrt_array( fam_id,  M_INT, (char *) label_name, 1, dims,
                             (void *)  labels );
   if (status != OK)
   {
      return status;
 ...
 */
static Return_value
update_nodes_def( Mili_family *fam, int mesh_id, char *class_name,
                  int start, int stop, float *coords )
{
   ...
   for ( i = 0; i < fam->file_count && searching; i++ )
   {
      ...
      for ( j = 0; j < ent_qty; j++ )
      {
         if ( (Dir_entry_type) p_de[j][TYPE_IDX] == NODES &&
               p_de[j][MODIFIER1_IDX] == mesh_id &&
               p_de[j][MODIFIER2_IDX] >= node_qty &&
               strcmp( names[name_cnt], class_name ) == 0 )
         {
            ...
            read_cnt = fam->read_funcs[M_INT]( fam->cur_file, range,
                                               QTY_NODE_TAGS );
            ...
            else if ( start >= range[0] && stop <= range[1] )
            {
               ...
               if ( range[0] != start )
               {
                  offset = (start - range[0])
                           * EXT_SIZE( fam, M_FLOAT ) * fam->dimensions;
               }
               else
               {
                  offset = 0;
               ...
               write_cnt = fam->write_funcs[M_FLOAT]( fam->cur_file,
                                                      coords, data_words );
                 ...
                 int start_el, int stop_el, int *data )
{
   ...
   if (milisilo)
   {
      rval = mc_silo_def_conn_seq( fam_id, mesh_id, short_name,
                                   start_el, stop_el, data );
      return ( rval );
   }
#endif
   ...
   /* Register an entry in the directory. */
   rval = add_dir_entry( fam, ELEM_CONNS, mesh_id, stop_el - start_el + 1, 1,
                         &short_name, fam->next_free_byte, rec_size );
   if ( rval != OK )
   {
      return rval;
   ...
   num_items_written = fam->write_funcs[M_INT]( fam->cur_file, data,
                       data_words );
   if (num_items_written != data_words)
   {
      return SHORT_WRITE;
                 ...
                 int el_qty, int *elem_ids, int *data )
{
   ...
   rec_size = (data_words + QTY_CONN_HEADER_FIELDS + blk_qty * 2)
              * EXT_SIZE( fam, M_INT );
   ...
   /* Register an entry in the directory. */
   rval = add_dir_entry( fam, ELEM_CONNS, mesh_id, el_qty, 1, &short_name,
                         fam->next_free_byte, rec_size );
   if ( rval != OK )
   {
      free(p_int_blk);
 ...
 */
Return_value
mc_def_conn_arb_labels( Famid fam_id, int mesh_id, char *short_name,
                        int el_qty, int *elem_ids, int *labels, int *data )
{
   Mili_family *fam;
   int i;
   int qty;
   ...
   rval = mc_def_conn_arb( fam_id, mesh_id, short_name,
                           el_qty, elem_ids, data );
   if ( rval == OK )
   {
      ...
      fam = fam_list[fam_id];
      rval = htable_search( fam->meshes[mesh_id]->mesh_data.umesh_data,
                            short_name, FIND_ENTRY, &p_hte );
      ...
      if ( p_hte != NULL )
      {
         ...
         rval = mc_def_conn_labels( fam_id, mesh_id, short_name,
                                    el_qty, labels, labels_mats );
 ...
 */
Return_value
mc_def_conn_seq_labels( Famid fam_id, int mesh_id, char *short_name,
                        int start_el, int stop_el, int *labels, int *data )
{
   ...
   rval = mc_def_conn_seq( fam_id, mesh_id, short_name,
                           start_el, stop_el, data );
   if ( rval != OK )
   {
      return rval;
...
Return_value
mc_get_globals_and_locals(Famid fam_id, int mesh_id,  char* class_name , 
                          int *global_ids, int *local_ids, int total_count)
{
   Mili_family *fam;
   ...
   for(i=0; i<count;i++)
   {
      sprintf(final_name,"GLOBAL_IDS[/Mesh-%d/Sname-%s/DEF-%d]",
           mesh_id, class_name,i);
      sprintf(final_local_name,"GLOBAL_IDS_LOCAL_MAP[/Mesh-%d/Sname-%s/DEF-%d]",
           mesh_id, class_name,i);
      
      status = mc_read_param_array_len( fam_id, final_name, 
                                        &param_array_type, &param_array_len );
 ...
 */
Return_value
mc_def_global_ids( Famid fam_id, int mesh_id, char *short_name,
                    int qty, int *idents, int *global_ids )
{
   ...
   label_name[0] ='\0';
   label_prefix[0] = '\0';
   sprintf(new_name,"[/Mesh-%d/Sname-%s/DEF-%d]",
           mesh_id, short_name,
 ...
 */
Return_value
mc_def_conn_arb_labels_global_ids( Famid fam_id, int mesh_id, char *short_name,
                        int el_qty, int *elem_ids, int *labels, int *data , 
                        int *global_ids)
{
   Mili_family *fam;
   ...
   if ( rval == OK )
   {
      ...
      mc_def_global_ids(fam_id, mesh_id, short_name, 
                        el_qty, elem_ids, global_ids);
   }
 ...
 */
Return_value
mc_def_conn_seq_labels_global_ids( Famid fam_id, int mesh_id, char *short_name,
                        int start_el, int stop_el, int *labels, int *data , 
                        int* global_ids)
{
   Return_value status;
   
   status = mc_def_conn_seq_labels( fam_id, mesh_id, short_name, 
                                    start_el, stop_el, labels, data );
                                                 
   if(status == OK)
   {
      status =  mc_def_seq_global_ids( fam_id, mesh_id, short_name,
                                start_el, stop_el, global_ids);
   }
                                   ...
                                   int start_el, int stop_el,int* labels)
{
   ...
   return mc_def_conn_labels( fam_id, mesh_id, short_name,
                             qty, labels, element_ids );
 ...
 */
Return_value
mc_def_conn_labels( Famid fam_id, int mesh_id, char *short_name,
                    int qty, int *labels, int *idents )
{
   int dims[3];
   Mili_family *fam;
   int temp_mesh_id=mesh_id, superclass;
   ...
   status = ti_make_label_description( mesh_id, 
                                       fam->label_class_list[class_index].last_matid ,
                                       superclass_name, short_name,  label_descriptor);
   status = mc_ti_def_class( fam_id, mesh_id, state,
                             fam->label_class_list[class_index].last_matid ,
                             superclass_name, TRUE, FALSE,
                             short_name, short_name);
   if (status != OK)
   {
      return status;
   ...
   status = mc_ti_wrt_array( fam_id,  M_INT, (char *) label_id_name, 1, dims,
                             (void *)  idents );
 ...
 */
Return_value
mc_def_conn_surface( Famid fam_id, int mesh_id, char *short_name,
                     int qty_of_facets, int *data, int *surface_id )
{
   ...
   p_mocd->surface_sizes = RENEW_N( int, p_mocd->surface_sizes, *surface_id, 1,
                                    "No. of facets in surface" );
   ...
   rec_size = (data_words + QTY_CONN_HEADER_FIELDS)
              * EXT_SIZE( fam, M_INT );
   ...
   /* Register an entry in the directory. */
   rval = add_dir_entry( fam, SURFACE_CONNS, mesh_id, qty_of_facets, 1,
                         &short_name, fam->next_free_byte, rec_size );
   if (rval != OK)
   {
      return rval;
 ...
 */
static Return_value
add_geom( Mesh_descriptor *mesh, char *short_name, int start_mo, int stop_mo )
{
#if TIMER
 ...
 */
Return_value
insert_range( Block_list *p_bl, int start, int stop )
{
   Int_range *pir, *pir2;
   Return_value rval;
 ...
 */
Return_value
check_object_ids( Block_list *mo_group, int qty_id_blks,
                  int *mo_id_blks )
{
   Return_value rval;
   Int_range *pir;
 ...
 */
void
delete_mo_data( void *ptr_mo_data )
{
   Mesh_object_class_data *p_mocd;
 ...
 */
static void
delete_umesh( Mesh_descriptor *mesh )
{
   free( mesh->name );
 ...
 */
Return_value
mc_load_nodes( Famid fam_id, int mesh_id, char *short_name, void *data )
{
   Mili_family *fam;
   ...
   for ( i = 0; i < fil_qty; i++ )
   {
      ...
      for ( j = 0; j < ent_qty; j++ )
      {
         if ( (Dir_entry_type) p_de[j][TYPE_IDX] == etype &&
               p_de[j][MODIFIER1_IDX] == mesh_id &&
 ...
 */
Return_value
mc_load_conns( Famid fam_id, int mesh_id, char *short_name, int *conns,
               int *mats, int *parts )
{
   Mili_family *fam;
   int fil_qty, ent_qty, name_cnt;
   ...
   for ( i = 0; i < fil_qty; i++ )
   {
      ...
      for ( j = 0; j < ent_qty; j++ )
      {
         if ( (Dir_entry_type) p_de[j][TYPE_IDX] == ELEM_CONNS &&
               p_de[j][MODIFIER1_IDX] == mesh_id &&
               strcmp( names[name_cnt], short_name ) == 0 )
         {
            /* Found an appropriate entry - load it into memory. */
...
Return_value
mc_load_conn_labels( Famid fam_id, int mesh_id, char *short_name,
                     int qty,
                     ...
                     int *element_list, int *labels )
{
   ...
   char mili_version[M_MAX_NAME_LEN], host[M_MAX_NAME_LEN],
        arch[M_MAX_NAME_LEN], timestamp[M_MAX_NAME_LEN],
   ...
   *block_range = NULL;
   ...
   num_entries_labels = htable_search_wildcard(table, 0, FALSE,
                                               "Element Labels[", sname,
                                               "NULL", NULL );
   if ( num_entries_labels==0 ) {
      particle_class = mc_is_particle_class( fam_id, short_name );
	   if ( particle_class ) {
         ...
         num_entries_labels = htable_search_wildcard(table, 
                                                     0, FALSE,
                                                     "Element Labels[",sname,
                                                     "NULL", NULL );
         ...
         if ( num_entries_labels == 0 )
            num_entries_labels = htable_search_wildcard(table, 0, FALSE,
						                                      "Element Labels[", "Sname-particle",
						                                      "NULL", NULL );
         if ( num_entries_labels == 0 )
            num_entries_labels = htable_search_wildcard(table, 0, FALSE,
						                                      "Element Labels[", "Sname-particle-elem",
						                                      "NULL", NULL );
         if ( num_entries_labels==0 ) {
            *num_blocks=0;
            *block_range=NULL;
   ...
   wildcard_list_labels = NEW_N(char *, num_entries_labels,
                                "Element Labels" );
   ...
   num_entries_labels = htable_search_wildcard(table, 0, FALSE,
                                               "Element Labels[", sname,
                                               "NULL", wildcard_list_labels);

   /* Determine if this is a pre 8.1 database to figure out the naming of
    * the ids. */
   status = mc_get_metadata(fam_id, mili_version, host, arch,
   ...
   for ( i=0; 
         i<num_entries_labels; 
         i++ )
   {
      status = mc_ti_read_array(fam_id, wildcard_list_labels[i],
      ...
      status = mc_ti_read_array(fam_id, wildcard_list_id,
                                (void **) &labels_elem_ptr, &dummy );
      if ( status!=OK ) {
   ...
   /* Convert 1D list to 2D block list. */
   status = list_to_blocks( total_qty_read, labels,
                            &block_range_ptr, num_blocks );
   if (status != OK)
   {
      return status;
   }
 ...
 */
Return_value
mc_get_node_label_info(Famid fam_id, int mesh_id, char *short_name,
                       int *num_blocks, int *num_labels)
{
   ...
   *num_labels = 0;
   ...
   /* Determine the superclass */
   status = mc_query_family(fam_id, CLASS_SUPERCLASS, &mesh_id,
                            short_name, (void *) &superclass);
   if (status != OK) {
      return status;
   }
   ...
   if(v1_file)
	...
	if(v1_file)
	{
   	status = mc_ti_read_array(fam_id, "Node Labels",
      	                       (void **) &labels_ptr, &num_items_read);
   }else
	{
		strcpy( label_name, "Node Labels" );
 ...
 */
Return_value
mc_load_node_labels( Famid fam_id, int mesh_id, char *short_name,
                     int *num_blocks, int **block_range, int *labels )
{
   Mili_family *fam;
   int i;
   int num_items_read;
   ...
   */
   ...
   if (status != OK)
   {
      ...
      if ( labels_found )
           status = mc_ti_read_array( fam_id, labels_var,
				                          (void **) &labels_ptr, &num_items_read );
      if(labels_var != NULL) 
      {
         free(labels_var);
 ...
 */
Return_value
mc_load_surface( Famid fam_id, int mesh_id, int surf,
                 char *short_name, void *conns )
{
   Mili_family *fam;
   int fil_qty, ent_qty, name_cnt;
   int i, j;
   ...
   for ( i = 0; i < fil_qty; i++ )
   {
      ...
      for ( j = 0; j < ent_qty; j++ )
      {
         if ( (Dir_entry_type) p_de[j][TYPE_IDX] == SURFACE_CONNS &&
               p_de[j][MODIFIER1_IDX] == mesh_id &&
               strcmp( names[name_cnt], short_name ) == 0 )
         {
            if( surf == this_surf )
 ...
 */
static Return_value
load_conn_def( Mili_family *fam, int file_idx, Dir_entry dir_ent,
               void *dest_ptrs )
{
   ...
   read_cnt = fam->read_funcs[M_INT]( fam->cur_file, econn_hdr,
                                      QTY_CONN_HEADER_FIELDS );
   ...
   read_cnt = fam->read_funcs[M_INT]( fam->cur_file, ebuf,
                                      elem_qty * word_qty );
 ...
 */
static Return_value
load_conn_def_surface( Mili_family *fam, int file_idx, Dir_entry dir_ent,
                       void *dest_ptr )
{
   int econn_hdr[QTY_CONN_HEADER_FIELDS];
   int read_cnt;
 ...
 */
static Return_value
load_nodes_def( Mili_family *fam, int file_idx, Dir_entry dir_ent,
                void *coords )
{
 ...
 */
Return_value
get_class_qty( Mili_family *fam, int *modifiers, int *class_qty )
{
 ...
 */
void
get_elem_conn_classes_names( Mili_family *fam, int mesh_id ,int *count, char *** out_names)
{
 ...
 */
int
count_elem_conn_defs( Mili_family *fam, int mesh_id, char *short_name )
{
   int fil_qty, ent_qty, name_cnt;
   int qty_conn_entries;
   int i, j;
 ...
 */
Return_value
get_elem_qty_in_def( Mili_family *fam, int *int_args, char *short_name,
                     int *elem_qty )
{
   ...
   if ( fam != last_fam  ||
         mi != mesh_id  ||
         int_args[1] < econn_entry_cnt ||
         strcmp( last_class, short_name ) != 0 )
   {
 ...
 */
int
count_node_entries( Mili_family *fam, int mesh_id, char *short_name )
{
   int fil_qty, ent_qty, name_cnt;
 ...
 */
Return_value
mc_get_class_idents( Famid fam_id, int mesh_id,
                     char *class_name,
                     int *num_blocks, int **idents )
   ...
   for ( i = 0; i < all_class_count; i++ )
   {
      if ( !strcmp(class_data[i]->short_name, class_name ) )
      {
         *num_blocks = class_data[i]->blocks->block_qty;
         ...
         {
            temp_idents = NEW_N( int, class_data[i]->blocks->block_qty*2,
                                 "Class idents" );
            if (class_data[i]->blocks->block_qty > 0 &&
                  temp_idents == NULL)
            {
 ...
 */
Return_value
get_node_block_size( Mili_family *fam, int *modifiers, char *short_name,
                     int *blk_size )
{
   Mili_family *last_fam = NULL;
   static int mesh_id = -1;
 ...
 */
Return_value
mc_get_class_info( Famid fam_id, int mesh_id, int superclass,
 ...
 */
Return_value
mc_get_class_info_by_name( Famid fam_id, int *mesh_id, const char *in_short_name,
                           int *superclass, int *class_index, char *in_long_name,
                          int *num_elems )
{
   ...
   char long_name[1024];
	...
	for(cur_mesh = 0; cur_mesh<nmeshes; cur_mesh++)
	{
		for (i = 0; i < M_QTY_SUPERCLASS ; i++)
		{
			...
			status = mc_query_family(fam_id, QTY_CLASS_IN_SCLASS, 
			                        (void*) args, NULL,(void*) &ngroups);
			if(status != OK)
			{
				return status;
			}
			...
			{
				nelems = 0;
				status = mc_get_class_info(fam_id, cur_mesh,i, j,
                           			   short_name, long_name, 
													&nelems);
				if(status != OK)
				{
 ...
 */
Return_value
mc_get_simple_class_info( Famid fam_id, int mesh_id, int superclass,
                          int class_index, char *short_name, char *long_name,
 ...
 */
Return_value
reset_class_data( Mili_family *fam, int mesh_id, Bool_type force_init )
{
   ...
   else if ( fam != class_data_fam || mesh_id != class_data_mesh_id )
   {
      ...
      rval = htable_get_data( p_md->mesh_data.umesh_data,
                              (void ***) &class_data, &all_class_count );
   }
   return rval;
}
...
init_meshes( Mili_family *fam )
{
   ...
   rval = htable_search( fam->param_table, "mesh dimensions",
                         FIND_ENTRY, &p_hte );
   if (p_hte == NULL)
   {
      return rval;
      ...
      for ( j = 0; j < ent_qty; j++ )
      {
         ...
         switch( etype )
         {
            case NODES:
               ...
               /* Read the nodal data header (start and stop id's). */
               read_ct = fam->read_funcs[M_INT]( fam->cur_file, node_hdr,
                                                 QTY_NODE_TAGS );
                ...
                */
               rval = add_geom( fam->meshes[mod1], names[name_cnt],
                                node_hdr[0], node_hdr[1] );
               if (rval != OK)
               {
                  return rval;
               }
            ...
            case ELEM_CONNS:
               ...
               /* Read the connectivity data header. */
               read_ct = fam->read_funcs[M_INT]( fam->cur_file, conn_hdr,
                                                 QTY_CONN_HEADER_FIELDS );
               ...
               read_ct = fam->read_funcs[M_INT]( fam->cur_file, elem_blks,
                                                 blk_qty * 2 );
            ...
            case CLASS_IDENTS:
               ...
               read_ct = fam->read_funcs[M_INT]( fam->cur_file,
                                                 ident_entry,
            ...
            case CLASS_DEF:
                ...
                */
               rval = create_class_data( p_de[j][MODIFIER2_IDX],
                                         names[name_cnt],
                                         names[name_cnt + 1],
                                         &p_mocd );
               if (rval != OK)
               ...
               rval = htable_add_entry_data(p_ht,
                                            names[name_cnt],
                                            ENTER_UNIQUE,
                                            p_mocd);
               if (rval != OK && rval != ENTRY_EXISTS)
            ...
            case SURFACE_CONNS:
...
#endif
               p_ht = fam->meshes[mod1]->mesh_data.umesh_data;
               rval = htable_search( p_ht, names[name_cnt],
                                     FIND_ENTRY, &p_hte );
               if (p_hte == NULL)
               {
                  return rval;
               ...
               p_mocd->surface_sizes = RENEW_N( int, p_mocd->surface_sizes,
                                                surface_id, 1,
                                                "No. of facets in surface" );
               if (p_mocd->surface_sizes == NULL)
               {
 ...
 */
static Return_value
add_mesh( Mili_family *fam, Dir_entry dir_ent, char *name_handle )
{
   char *mname;
   int mesh_id;
   Htable_entry *p_hte;
 ...
 */
Return_value
dump_nodes( Mili_family *fam, FILE *p_f, Dir_entry dir_ent,
            ...
            int head_indent, int body_indent )
{
   ...
   rval = eprintf( &num_written,
                   "%*tNODE DEFS:%*tClass: %s;%*t@%ld;%*tIdents: %d-%d\n",
                   hi, hi + 13, dir_strings[0], hi + 44, offset, hi + 56,
                   node_header[0], node_header[1] );
   if (rval != OK)
   {
      return rval;
   }
   ...
   if ( p_dc->brevity == DV_SHORT )
   {
      ...
      if ( qty > 1 )
      {
         status = fseek( p_f, (qty - 2) * dims * EXT_SIZE( fam, M_FLOAT ),
                         SEEK_CUR );
         if (status != 0)
         {
            free(fbuf);
 ...
 */
Return_value
dump_elem_conns( Mili_family *fam, FILE *p_f, Dir_entry dir_ent,
                 char **dir_strings, Dump_control *p_dc,
                 int head_indent, int body_indent )
{
   LONGLONG offset;
   ...
   read_ct = fam->read_funcs[M_INT]( p_f, elem_header,
                                     QTY_CONN_HEADER_FIELDS );
   if (read_ct < QTY_CONN_HEADER_FIELDS)
   {
      return SHORT_READ;
   ...
   rval = eprintf( &num_written,
                   "%*tELEM DEFS:%*tClass: %s (%s);%*t@%ld;%*tQty blocks: %d\n",
                   hi, hi + 13, dir_strings[0], superclass_names[superclass],
                   hi + 44, offset, hi + 56, qty_blks );
   if (rval != OK)
   {
      return rval;
   }
   ...
   for ( i = 0, p_i = blks; i < qty_blks; p_i += 2, i++ )
   {
      ...
      if ( p_dc->brevity == DV_SHORT )
      {
         ...
         rval = eprintf( &num_written, "%*tm%d p%d%*t%-5d",
                         bi, conn[qty_wds - 2],
                         conn[qty_wds - 1], bi + 10, conn[0] );
         if (rval != OK)
         {
            free(conn);
            free(blks);
         ...
         {
            status = fseek(p_f,
                           (elem_qty - 2) * qty_wds * EXT_SIZE( fam, M_INT ),
                           SEEK_CUR);
            if (status != 0)
            {
               free(conn);
      ...
      else
      {
         ...
         for ( k = 0, p_elem = conn; k < elem_qty; p_elem += qty_wds, k++ )
         {
            rval = eprintf( &num_written, "%*tm%d p%d%*t%-5d",
                            bi, p_elem[qty_wds - 2],
                            p_elem[qty_wds - 1], bi + 10, p_elem[0] );
            if (rval != OK)
            {
               free(conn);
               free(blks);
 ...
 */
Return_value
dump_surface_conns( Mili_family *fam, FILE *p_f, Dir_entry dir_ent,
                    char **dir_strings, Dump_control *p_dc,
                    int head_indent, int body_indent )
{
   LONGLONG offset;
   ...
   read_ct = fam->read_funcs[M_INT](p_f, econn_header,
                                    QTY_CONN_HEADER_FIELDS);
   if (read_ct < QTY_CONN_HEADER_FIELDS)
   {
      return SHORT_READ;
   ...
   /* Dump listing header for connectivity data. */
   rval = eprintf( &num_written,
                   "%*tSURF DEFS:%*tClass: %s (%s);%*t@%ld;%*tQty facets: %d\n",
                   hi, hi + 13, dir_strings[0], superclass_names[superclass],
                   hi + 44, offset, hi + 56, qty_facets );
   if (rval != OK)
   {
      return rval;
   }
   ...
   if ( p_dc->brevity == DV_SHORT )
   {
      ...
      {
         status = fseek(p_f,
                        (qty_facets - 2) * qty_wds * EXT_SIZE( fam, M_INT ),
                        SEEK_CUR );
         if (status != 0)
         {
            return SEEK_FAILED;
   ...
   else
   {
      ...
      {
         rval = eprintf( &num_written, "%*t%*t%-5d",
                         bi, bi + 10, p_elem[0] );
         if (rval != OK)
         {
            free(conn);
 ...
 */
Return_value
dump_class_idents( Mili_family *fam, FILE *p_f, Dir_entry dir_ent,
                   char **dir_strings, Dump_control *p_dc,
                   int head_indent, int body_indent )
{
   LONGLONG offset;
   ...
   /* Dump entry. */
   rval = eprintf( &num_written,
                   "%*tIDENTS:%*tClass: %s (%s);%*t@%ld;%*tIdents: %d-%d\n",
                   hi, hi + 13, dir_strings[0], superclass_names[superclass],
                   hi + 44, offset, hi + 56, start, stop );
 ...
 */
static int SDTLIBCC
mc_compare_labels( const void *label1, const void *label2 )
{
 ...
 */
int
mc_get_conn_count( int superclass )
{
 ...
 */
Return_value
mc_get_unique_node_count( int *nodes, int qty, int *count )
{
   int i;
   int max_node=-1;
   int *temp_buff;
 ...
 */
void
mili_delete_mo_class_data( void *p_data )
{
 ...
 */
void
mc_blocks_to_list( int qty_blocks, int *mo_blocks, 
		   int *list_count, int *list, 
                   int decrement_indices )
{
    int i, j;
    int increment, ident;
