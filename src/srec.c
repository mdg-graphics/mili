             ...
             Translated_ref *refs, void **p_out)
{
  ...
  for (i = 0; i < qty; i++)
  {
    ...
    offset = p_sd->offset + EXT_SIZE(fam, M_INT) + EXT_SIZE(fam, M_FLOAT) + p_subrec->offset + p_subrec->lump_offsets[idx];
...
truncate_family(Mili_family *p_fam, int st_index)
{
  ...
  if (rval == OK)
  {
    ...
    else
    {
      ...
      {
        p_fam->file_map = RENEW_N(State_file_descriptor, p_fam->file_map, p_fam->st_file_count, file_qty, "Shrunken file map on restart");
      }
