...
static Return_value get_ro_svars(Mili_family *fam, int state, Sub_srec *p_subrec, int qty, Translated_ref *refs,
                                 void **p_out);
static Return_value get_oo_svars(Mili_family *fam, int state, Sub_srec *p_subrec, int qty, Translated_ref *refs,
                                 void **p_out);
static Return_value translate_reference(Sub_srec *p_subrec, char *result, Svar **pp_svar, Translated_ref *p_tref);
static Return_value map_subset_spec(Svar *p_svar, int indices[], int index_qty, char *component, int comp_index,
                                    char *sub_component, int sub_comp_index, Translated_ref *p_tref);
static void (*get_data_dist_func(int data_type, int length, int cell_size))();
static void distribute_all_4(void *p_input, int cell_qty, int cell_size, int cell_offset, int length, void *p_output);
static void distribute_all_8(void *p_input, int cell_qty, int cell_size, int cell_offset, int length, void *p_output);
static void distribute_scalar_4(void *p_input, int cell_qty, int cell_size, int cell_offset, int length,
                                void *p_output);
static void distribute_scalar_8(void *p_input, int cell_qty, int cell_size, int cell_offset, int length,
                                void *p_output);
static void distribute_vector_4(void *p_input, int cell_qty, int cell_size, int cell_offset, int length,
                                void *p_output);
static void distribute_vector_8(void *p_input, int cell_qty, int cell_size, int cell_offset, int length,
                                void *p_output);
static Return_value add_srec(Mili_family *fam, int index, Dir_entry dir_ent);
static Return_value def_subrec_from_db(Mili_family *fam, int srec_id, int *i_data, char *c_data, int *c_data_len);
static Return_value set_metrics(Mili_family *fam, int data_org, Sub_srec *psubrec, Mesh_object_class_data *p_mocd,
                                LONGLONG *size);
 ...
 */
Return_value mc_open_srec(Famid fam_id, int mesh_id, int *p_srec_id)
{
    Mili_family *fam;
 ...
 */
Return_value mc_close_srec(Famid fam_id, int srec_id)
{
    Mili_family *fam;
    Srec *p_srec;
 ...
 */
Return_value mc_def_subrec(Famid fam_id, int srec_id, char *subrec_name, int org, int qty_svars, char *svar_names,
                           int name_stride, char *mclass, int format, int cell_qty, int *mo_ids, int *flag)
{
    ...
    rval = mc_query_family(fam_id, SREC_MESH, &local_srec_id, NULL, (void *)&mesh_id);
    ...
    rval = mc_query_family(fam_id, CLASS_SUPERCLASS, &mesh_id, mclass, (void *)&superclass);
    ...
    status = htable_search(fam->subrec_table, subrec_name, op, &subrec_entry);
    ...
    for ( i = 0, sv_name = svar_names; i < qty_svars; sv_name += name_stride, i++ )
    {
        status = htable_search(fam->svar_table, sv_name, FIND_ENTRY, &svar_entry);
        if ( svar_entry == NULL )
        {
    ...
    status = htable_search(fam->srec_meshes[srec_id]->mesh_data.umesh_data, mclass, FIND_ENTRY, &class_entry);
    if ( class_entry == NULL )
    {
        delete_subrec((void *)psubrec);
        htable_del_entry(fam->subrec_table, subrec_entry);
    ...
    if ( M_SURFACE == superclass )
    {
        ...
        if ( flag == NULL )
        {
            ...
            surface_flag = NEW_N(int, svar_atoms, "Surface variable display buffer");
        ...
        else
        {
            ...
            memcpy((void *)surface_flag, (const void *)flag, svar_atoms * sizeof(int));
        }
...
Return_value list_to_blocks(int size, int *list, int **dest, int *blk_cnt)
{
    ...
    *blk_cnt = 0;
    ...
    while ( i < size )
    {
        ...
        while ( i < size && list[i] == list[i - 1] + 1 )
        {
            i++;
        }
    ...
    *size = 0;
    ...
    if ( data_org == OBJECT_ORDERED )
    {
        ...
        for ( ppsvar = psubrec->svars, i = 0; ppsvar < bound; ppsvar++, i++ )
        {
            ...
            if ( (superclass == M_SURFACE) && (psubrec->surface_variable_flag != NULL) )
            {
    ...
    else if ( data_org == RESULT_ORDERED )
    {
        ...
        if ( (psubrec->qty_svars > 0) &&
             (psubrec->lump_atoms == NULL || psubrec->lump_sizes == NULL || psubrec->lump_offsets == NULL) )
        {
            return ALLOC_FAILED;
        }
 ...
 */
Return_value mc_get_subrec_def(Famid fam_id, int srec_id, int subrec_id, Subrecord *p_subrec)
{
    Sub_srec *p_ssrec;
    int id_cnt;
    ...
    /* Get the superclass. */
    rval = htable_search(fam->srec_meshes[srec_id]->mesh_data.umesh_data, p_ssrec->mclass, FIND_ENTRY, &p_hte);
    if ( p_hte == NULL )
    {
        return rval;
    }
    ...
    switch ( superclass )
    {
        ...
        case M_PARTICLE:
            ...
            mo_qty = 0;
#endif
            for ( p_isrc = p_ssrec->mo_id_blks, p_idest = p_subrec->mo_blocks; p_isrc < bound;
                  p_idest += 2, p_isrc += 2 )
            {
                /* Copy block. */
                *p_idest = *p_isrc;
        ...
        case M_SURFACE:
            ...
            p_subrec->surface_variable_flag = NEW_N(int, p_subrec->qty_svars, "Surface variable flag");
            if ( p_subrec->qty_svars > 0 && p_subrec->surface_variable_flag == NULL )
            {
                return ALLOC_FAILED;
            }
...
Return_value commit_srecs(Mili_family *fam)
{
    ...
    for ( i = fam->commit_max + 1; i < fam->qty_srecs; i++ )
    {
        ...
        *pi++ = psrec->qty_subrecs;
        ...
        rval = add_dir_entry(fam, STATE_REC_DATA, (int)i_qty, (int)c_qty, 0, NULL, fam->next_free_byte, outbytes);
        if ( rval != OK )
        {
            free(i_data);
            free(c_data);
...
static Return_value add_srec(Mili_family *fam, int index, Dir_entry dir_ent)
{
    ...
    read_ct = fam->read_funcs[M_INT](fam->cur_file, srec_hdr, QTY_SREC_HEADER_FIELDS);
    ...
    {
        rval = def_subrec_from_db(fam, srec_hdr[SREC_ID_IDX], subrec_i_data, subrec_c_data, &c_data_len);
        if ( rval != OK )
        {
            break;
        }
        ...
        rval = htable_search(fam->srec_meshes[p_srec_id]->mesh_data.umesh_data, p_subrec->mclass, FIND_ENTRY,
                             &class_entry);
        if ( class_entry == NULL )
        {
            break;
...
static Return_value def_subrec_from_db(Mili_family *fam, int srec_id, int *i_data, char *c_data, int *c_data_len)
{
    ...
    *c_data_len = (int)(p_c - c_data);
    ...
    /* Get the object class. */
    rval = htable_search(fam->srec_meshes[srec_id]->mesh_data.umesh_data, p_subrec->mclass, FIND_ENTRY, &class_entry);
    if ( class_entry == NULL )
    {
        free(svars);
        free(p_subrec->mclass);
    ...
    if ( superclass != M_MESH )
    {
        ...
        for ( p_src = p_i, p_dest = p_subrec->mo_id_blks; p_src < bound; p_dest += 2, p_src += 2 )
        {
            p_dest[0] = p_src[0];
            p_dest[1] = p_src[1];
        ...
        if ( superclass == M_SURFACE )
        {
            p_i += 2 * qty_id_blks;
            svar_atoms = *p_i++;
            p_subrec->surface_variable_flag = NEW_N(int, svar_atoms, "Surface flags");
    ...
    /* Save a reference in order of its creation. */
    p_sr->subrecs = RENEW_N(Sub_srec *, p_sr->subrecs, p_sr->qty_subrecs, 1, "Srec data order incr");
 ...
 */
Return_value mc_read_results(Famid fam_id, int state, int subrec_id, int qty, char **results, void *data)
{
    Mili_family *fam;
    int st;
    Srec *p_sr;
    ...
    p_trans_refs = NEW_N(Translated_ref, qty, "Translated svar ref array for read");
    ...
    for ( i = 0; i < qty; i++ )
    {
        ...
        rval = translate_reference(p_subrec, results[i], &p_sv, p_trans_refs + i);
...
static Return_value translate_reference(Sub_srec *p_subrec, char *result, Svar **pp_svar, Translated_ref *p_tref)
{
    ...
    int index_qty = 0, comp_index = 0, secondary_comp_index = 0;
    ...
    Translated_ref tr;
    int i;
    Bool_type have_component, have_second_component;
    ...
    if ( have_component )
    {
        ...
        for ( comp_index = 0; comp_index < *p_svar->list_size; comp_index++ )
        {
            ...
            if ( strcmp(p_svar->svars[comp_index]->name, component) == 0 )
            {
                ...
                {
                    for ( secondary_comp_index = 0; secondary_comp_index < *p_svar->svars[comp_index]->list_size;
    ...
    else if ( *p_svar->agg_type != SCALAR )
    {
        ...
        rval = map_subset_spec(p_svar, indices, index_qty, component, comp_index, secondary_component,
                                 ...
                                 void **p_out)
{
    ...
    LONGLONG offset;
    LONGLONG read_cnt, new_read_atoms, read_atoms, length;
    int file_num;
    LONGLONG buf_pos;
    int i, j;
    int idx;
    ...
    for ( i = 0; i < qty; i++ )
    {
        ...
        offset = p_sd->offset + EXT_SIZE(fam, M_INT) + EXT_SIZE(fam, M_FLOAT) + p_subrec->offset +
        ...
        if ( fam->db_type == TAURUS_DB_TYPE )
        {
            ...
            while ( length > 0 )
            {
                if ( offset + length * EXT_SIZE(fam, data_type) <= fam->cur_st_file_size )
                {
                    read_cnt = fam->state_read_funcs[data_type](fam->cur_st_file, p_obuf + buf_pos, length);
                    if ( read_cnt < length )
                    {
                        rval = SHORT_READ;
                        break;
                ...
                {
                    new_read_atoms = (LONGLONG)(fam->cur_st_file_size - offset) / EXT_SIZE(fam, data_type);
                    read_cnt = fam->state_read_funcs[data_type](fam->cur_st_file, p_obuf + buf_pos, new_read_atoms);
        ...
        {
            read_cnt = fam->state_read_funcs[data_type](fam->cur_st_file, p_obuf, read_atoms);
        ...
        for ( j = subset ? i : i + 1; j < qty; j++ )
        {
            if ( strcmp(refs[j].name, refs[i].name) == 0 && !refs[j].done )
            {
                ...
                dist_func = get_data_dist_func(data_type, refs[i].reqd_qty, refs[i].atom_qty);
                if ( dist_func != NULL )
                {
                    /* Move data. */
                    dist_func((void *)p_obuf, p_subrec->mo_qty, refs[j].atom_qty, refs[j].offset, refs[j].reqd_qty,
                              p_out[j]);
                    refs[j].done = TRUE;
                }
                else
                                 ...
                                 void **p_out)
{
    ...
    LONGLONG offset;
    void (*dist_func)();
    LONGLONG read_cnt, read_atoms;
    int file_num;
    LONGLONG length, new_read_atoms;
    LONGLONG buf_pos;
    ...
    else
    {
         ...
         */
        read_from_file = TRUE;
        ibuf_len = p_subrec->lump_atoms[0] * internal_sizes[data_type] * p_subrec->mo_qty;
    ...
    if ( read_from_file )
    {
        ...
        if ( fam->db_type == TAURUS_DB_TYPE )
        {
            ...
            while ( length > 0 )
            {
                if ( offset + length * EXT_SIZE(fam, data_type) <= fam->cur_st_file_size )
                {
                    read_cnt = fam->state_read_funcs[data_type](fam->cur_st_file, (char *)ibuf + buf_pos, length);
                ...
                else
                {
                    new_read_atoms = (LONGLONG)(fam->cur_st_file_size - offset) / EXT_SIZE(fam, data_type);
                    read_cnt =
                        fam->state_read_funcs[data_type](fam->cur_st_file, (char *)ibuf + buf_pos, new_read_atoms);
                    if ( read_cnt < new_read_atoms )
                    {
                        if ( p_bq->buffer_count == 0 )
                        {
        ...
        {
            read_cnt = fam->state_read_funcs[data_type](fam->cur_st_file, (char *)ibuf, read_atoms);
    ...
    for ( i = 0; i < qty; i++ )
    {
        ...
        /* Get appropriate function to move data to output buffer. */
        dist_func = get_data_dist_func(data_type, refs[i].reqd_qty, obj_vec_size);
        if ( dist_func != NULL )
        {
            /* Move data. */
            dist_func((void *)ibuf, p_subrec->mo_qty * qty_facets, obj_vec_size / qty_facets,
                      refs[i].offset + obj_vec_offset, refs[i].reqd_qty, p_out[i]);
        }
        else
        {
            rval = INVALID_DATA_TYPE;
 ...
 * output buffer.
 */
static void distribute_scalar_4(void *p_input, int cell_qty, int cell_size, int cell_offset, int length, void *p_output)
{
    int *p_in, *p_out;
    int i;
 ...
 * output buffer.
 */
static void distribute_scalar_8(void *p_input, int cell_qty, int cell_size, int cell_offset, int length, void *p_output)
{
    double *p_in, *p_out;
    int i;
 ...
 * output buffer.
 */
static void distribute_vector_4(void *p_input, int cell_qty, int cell_size, int cell_offset, int length, void *p_output)
{
    int *p_in, *p_out;
    int i, j;
 ...
 * output buffer.
 */
static void distribute_vector_8(void *p_input, int cell_qty, int cell_size, int cell_offset, int length, void *p_output)
{
    double *p_in, *p_out;
    int i, j;
 ...
 */
Return_value mc_wrt_stream(Famid fam_id, int type, LONGLONG qty, void *data)
{
    LONGLONG write_ct;
    LONGLONG byte_ct;

    write_ct = (fam_list[fam_id]->state_write_funcs[type])(fam_list[fam_id]->cur_st_file, data, qty);
    if ( write_ct != qty )
    {
        return SHORT_WRITE;
    }
 ...
 */
Return_value mc_wrt_subrec(Famid fam_id, char *subrec_name, int start, int stop, void *data)
{
    Mili_family *fam;
    Sub_srec *psubrec;
    ...
    fam = fam_list[fam_id];
    rval = htable_search(fam->subrec_table, subrec_name, FIND_ENTRY, &subrec_entry);
    if ( subrec_entry == NULL )
    {
        return rval;
    }
    ...
    rval = mc_query_family(fam_id, SREC_MESH, &srec_id, NULL, (void *)&mesh_id);
    if ( OK != rval )
    {
        return (rval);
    }
    ...
    rval = mc_query_family(fam_id, CLASS_SUPERCLASS, &mesh_id, psubrec->mclass, (void *)&superclass);
    if ( rval != OK )
    {
        return rval;
    }
    ...
    {
        /* Calc file offset. */
        loc = fam->cur_st_offset + psubrec->offset + psubrec->lump_sizes[0] * start_i;
        /* Calc atom count. */
        if ( superclass != M_SURFACE )
        {
    ...
    else
    {
        ...
        /* Calc file offset. */
        loc = fam->cur_st_offset + psubrec->offset + lump_offsets[start_i];
        /* Calc atom count. */
        qty = (lump_offsets[stop_i] - lump_offsets[start_i] + psubrec->lump_sizes[stop_i]) / EXT_SIZE(fam, type);
    }
    ...
    write_ct = (fam->state_write_funcs[type])(fam->cur_st_file, data, qty);
    if ( write_ct != qty )
    {
        return SHORT_WRITE;
    }
...
static Return_value rebuild_state_tfile(Mili_family *fam)
{
    if ( !fam->write_tfile )
    {
        return OK;
 ...
 */
Return_value mc_end_state(Famid fam_id, int srec_id)
{
    Mili_family *fam;
    int state_qty;
    State_descriptor *p_sd;
 ...
 */
Return_value mc_new_state(Famid fam_id, int srec_id, float time, int *p_file_suffix, int *p_file_state_index)
{
    Mili_family *fam;
    Return_value rval;
    int state_qty;
    ...
    {
        fam->state_map = RENEW_N(State_descriptor, fam->state_map, state_qty, 1, "Addl state descr");
    }
 ...
 */
Return_value mc_rewrite_subrec(Famid fid, char *subrec_name, int start, int stop, void *data, int st_index)
{
    State_descriptor *sd;
    ...
    rval = htable_search(fam_list[fid]->subrec_table, subrec_name, FIND_ENTRY, &subrec_entry);
    if ( subrec_entry == NULL )
    {
    ...
    else
    {
        ...
        while ( state_file_open(fam, index, 'r') == OK )
        {
            if ( initial )
            {
                ...
                (*p_fmap) = RENEWC_N(State_file_descriptor, *p_fmap, index + 1, 1, "New file map entry");
                if ( *p_fmap == NULL )
                {
                    return ALLOC_FAILED;
                }
                                 ...
                                 int head_indent, int body_indent)
{
    ...
    Mesh_object_class_data *p_mocd;
    Htable_entry *class_entry;
    char *short_subr_hdr = "%*t%d) \"%s\"%*tOrg:%s;%*tClass:%s;%*tVars:%d;%*tBlocks:%d\n";
    int outlen;
    Return_value rval = OK;
    int read_ct;
    ...
    for ( i = 0; i < subrec_qty; i++ )
    {
        ...
        {
            rval = eprintf(&outlen, short_subr_hdr, bi, i + 1, p_snam, bi + 20, org_names[subrec_i_data[0]], bi + 32,
                           p_cnam, bi + 54, svar_qty, bi + 63, blk_qty);
            if ( rval != OK )
            {
                free(i_data);
                free(c_data);
            ...
            /* Get the object class. */
            rval = htable_search(fam->srec_meshes[p_srec_id]->mesh_data.umesh_data, p_cnam, FIND_ENTRY, &class_entry);
            if ( class_entry == NULL )
            {
                break;
            }
        ...
        else
        {
            rval = eprintf(&outlen, "%*t%d) \"%s\"%*tOrg: %s;%*tClass: %s;\n", bi, i + 1, p_snam, bi + 30,
                           org_names[subrec_i_data[0]], bi + 45, p_cnam);
            if ( rval != OK )
            {
                free(i_data);
                free(c_data);
        ...
        rval = esprintf(cbuf, "%*tState vars (%d): %s ", indent3, svar_qty, subrec_c_data);
        if ( rval != OK )
        {
            free(i_data);
            free(c_data);
        ...
        {
            rval = eprintf(&outlen, "%*tIdent blocks in column-major order:\n", indent3);
            if ( rval != OK )
            {
                free(i_data);
                free(c_data);
        ...
        for ( j = 0; j < rows; j++ )
        {
            ...
            rval = eprintf(&outlen, "%*t%8d - %-8d", indent3, p_i[idx], p_i[idx + 1]);
            if ( rval != OK )
            {
                free(i_data);
                free(c_data);
            ...
            if ( idx < blk_qty * 2 )
            {
                rval = eprintf(&outlen, "%*t%8d - %-8d", 3, p_i[idx], p_i[idx + 1]);
                if ( rval != OK )
                {
                    free(i_data);
                    free(c_data);
 ...
 *
 */
Return_value mc_get_subrec_cnt(Famid fam_id, int staterec_id, char *svar_name, int *cnt)
{
    Subrecord p_subrec;
    int subrec_qty = 0;
    int i = 0, j = 0, k = 0, status = 0, temp_count = 0;

    i = staterec_id;

    /* Get subrecord count for this state record. */
    status = mc_query_family(fam_id, QTY_SUBRECS, (void *)&i, NULL, &subrec_qty);
 ...
 *
 */
Return_value mc_get_subrec_ids(Famid fam_id, int staterec_id, char *svar_name, int *cnt, int **sids)
{
    Subrecord subrec;
    int subrec_qty = 0;
 ...
 *
 */
Return_value mc_get_subrec_fields(Famid fam_id, int staterec_id, int sid, char *name, char *mclass, int *qty_svars,
                                  int *organization, int *qty_objects, int *qty_blocks)
{
    Subrecord subrec;
 ...
 *
 */
Return_value mc_get_svars(Famid fam_id, int staterec_id, int *num_svars, char ***svar_names)
{
    Subrecord subrec;
    char **p_svar_names;
    int subrec_qty = 0;
    ...
    /* Get subrecord count for this state record. */
    status = mc_query_family(fam_id, QTY_SUBRECS, (void *)&staterec_id, NULL, &subrec_qty);
    ...
    for ( i = 0; i < subrec_qty; i++ )
    {
        /* Get sub-record binding */
        status = mc_get_subrec_def(fam_id, staterec_id, i, &subrec);
        for ( j = 0; j < subrec.qty_svars; j++ )
        {
            svar_found = FALSE;
            for ( k = 0; k < svar_index; k++ )
            {
 ...
 * Assumes unstructured mesh type (c/o mesh object id checking).
 */
Return_value mc_def_subrec_from_subrec(Famid fam_id, int srec_id, Subrecord *in_subrec)
{
    Mili_family *fam;
    Return_value status;
    Srec *psr;
    ...
    if ( in_subrec->organization != OBJECT_ORDERED && in_subrec->organization != RESULT_ORDERED )
    {
        return UNKNOWN_ORGANIZATION;
    }
    ...
    rval = mc_query_family(fam_id, CLASS_SUPERCLASS, &mesh_id, in_subrec->class_name, (void *)&superclass);
    ...
    status = htable_search(fam->subrec_table, in_subrec->name, op, &subrec_entry);
    if ( subrec_entry == NULL )
    {
        return status;
    }
    ...
    {
        status = htable_search(fam->svar_table, in_subrec->svar_names[i], FIND_ENTRY, &svar_entry);
        if ( svar_entry == NULL )
        {
            /* Variable not found.  Free all resources and return. */
            delete_subrec((void *)psubrec);
    ...
    /* Get the referenced object class. */
    status =
        htable_search(fam->srec_meshes[srec_id]->mesh_data.umesh_data, in_subrec->class_name, FIND_ENTRY, &class_entry);
    if ( class_entry == NULL )
    {
        delete_subrec((void *)psubrec);
        htable_del_entry(fam->subrec_table, subrec_entry);
    ...
    if ( M_SURFACE == superclass )
    {
        ...
        else
        {
            ...
            memcpy((void *)surface_flag, (const void *)in_subrec->surface_variable_flag, svar_atoms * sizeof(int));
        }
