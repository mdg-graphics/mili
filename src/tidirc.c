...
/* Prototypes */
static Bool_type valid_ti_dir_entry_data( Dir_entry_type etype,
      int string_qty,
      char **strings );
 ...
 */
static Bool_type
valid_ti_dir_entry_data( Dir_entry_type etype, int string_qty, char **strings )
{
   int i;
                  ...
                  LONGLONG length )
{
   ...
   p_fd->dir_entries = RENEW_N( Dir_entry, p_fd->dir_entries, qty_ent, 1,
                                "Addl file dir entry" );
   ...
   if ( string_qty > 0 )
   {
      p_fd->names = RENEW_N( char *, p_fd->names, qty_nam, string_qty,
                             "Addl name ptr" );
   ...
   if ( string_qty > 0 )
   {
      ...
      for ( i = 0; i < string_qty; i++ )
      {
         len = ios_str_dup( p_fd->names + qty_nam + i, strings[i],
                            p_fd->name_data );
         if ( len == 0 )
         {
...
commit_ti_dir( Mili_family *fam )
{
   ...
   num_written = fam->write_funcs[M_INT]( fam->ti_cur_file, p_fd->dir_entries,
                                          words );
   ...
   if(fam->char_header[DIR_VERSION_IDX]==1){
      ...
      fam->ti_next_free_byte += (words + QTY_DIR_HEADER_FIELDS-1) *
                             EXT_SIZE( fam, M_INT );
   }else {
      num_written = fam->write_funcs[M_INT]( fam->ti_cur_file, dir_header,
      ...
      fam->ti_next_free_byte += (words + QTY_DIR_HEADER_FIELDS) *
                             EXT_SIZE( fam, M_INT );
   }
 ...
 */
Return_value
load_ti_directories( Mili_family *fam )
{
   int fcnt;
   int fnum;
   int qty_ent, qty_nam;
      ...
      if ( nitems > 0 )
      {
         ...
         for ( i = 0; i < qty_ent; i++ )
         {
            ...
            if ( etype == TI_PARAM )
            {
               ...
               rval = htable_search( fam->ti_param_table, p_name,
                                     ENTER_ALWAYS, &phte );
               if (phte == NULL)
               {
                  free(p_fd->names);
