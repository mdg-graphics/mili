 ...
 */
Return_value
mc_mr_get_geom( int dbid, int mesh_id, int superclass,
                int *qty_elems,
                int **conn, int **mat, int *num_mats, int **mat_list,
                int **part,
                int **elem_ids, int **labels)
{
   int  i, j;
   int  int_args[2];
   int *conn_ptr=NULL, *mat_ptr=NULL, *part_ptr=NULL,
        *elems_ptr=NULL, *labels_ptr=NULL;
   ...
   {
      status = mc_get_class_info( dbid, mesh_id, superclass, i,
   ...
   *labels   = (int *) malloc(qty_object*sizeof(int));
   ...
   for (i=0; i<qty_classes; i++)
   {
      ...
      status = mc_load_conns( dbid, mesh_id, short_name,
                              conn_ptr+conn_ct*obj_index,
                              mat_ptr+obj_index, part_ptr+obj_index );
      if ( status != OK )
      {
         return status;
      }

      status = mc_load_conn_labels( dbid, mesh_id, short_name,
                                    obj_qty,
                                    &block_qty, &block_range,
                                    elems_ptr+obj_index,
                                    labels_ptr+obj_index );
      if ( status != OK )
      {
         return status;
      }
...
Return_value
mc_mr_get_subrecs(int fid, int *subrec_total, Subrecord **p_subr)
{
	Subrecord *p_subrec;
	int srec_qty=0;
	int *subrec_qty = NULL;
	...
	stat = mc_query_family( fid, QTY_SREC_FMTS, NULL, NULL, 
                            (void *) &srec_qty );
	 if ( stat != 0 )
    {
    ...
    {
	 	stat = mc_query_family( fid, QTY_SUBRECS, (void *) &i, NULL, 
                                (void *) &subrec_qty[i] );
                     ...
                     int  *svar_result_len)
{
   ...
   {
      subrec_ptr = &p_subr[i];
      if ((!strcmp(subrec_ptr->name, subrec_name) ||
           !strcmp("*", subrec_name)) &&
          !strcmp(subrec_ptr->class_name, class_name))
      {
         for (j=0; j<subrec_ptr->qty_svars; j++)
         {
            if (!strcmp(subrec_ptr->svar_names[j], svar_name))
            {
               ...
               if ( (p_sv.agg_type == ARRAY || p_sv.agg_type == VEC_ARRAY)  &&
                     p_sv.rank>0 && p_sv.dims )
               {
 ...
 */
Return_value
mc_mr_get_result(int dbid, int state,
                 int subrec_qty, Subrecord *p_subrec,
                 char *class_name, char *svar_name,
                 int *len, int *veclen, int *type,
                 ...
                 void **result)
{
         ...
         for (j=0; j<p_subrec[i].qty_svars; j++)
         {
            if (!strcmp(p_subrec[i].svar_names[j], svar_name))
            {
               ...
               for (k=0; k<p_subrec[i].qty_blocks; k++)
               {
                  ...
                  temp_block_list[temp_block_cnt]   =
                     (int) p_subrec[i].mo_blocks[block_cnt];
                  temp_block_list[temp_block_cnt+1] =
                     (int) p_subrec[i].mo_blocks[block_cnt+1];
                  temp_len += (temp_block_list[temp_block_cnt+1] -
                               temp_block_list[temp_block_cnt]) + 1;
   ...
   *len         = temp_len;
   ...
   status = mc_mr_get_result_len(dbid,
                                 subrec_qty, 
											p_subrec,
                                 class_name,
                                 "*", svar_name,
   ...
   {
      result_buff_flt      = NEW_N( float, result_len,
                                    "Result buffer - flt[mc_mr_get_result[" );
      result_buff          = (void *) result_buff_flt;
      temp_result_buff_flt = NEW_N( float, result_len,
                                    "Temp result buffer - flt[mc_mr_get_result[" );
   ...
   {
      result_buff_dbl      = NEW_N( double, result_len,
                                    "Result buffer - dbl[mc_mr_get_result[" );
      result_buff          =  (void *) result_buff_dbl;
      temp_result_buff_dbl = NEW_N( double, result_len,
                                    "Temp result buffer - flt[mc_mr_get_result[" );
   ...
   {
      result_buff_int      = NEW_N( int, result_len,
                                    "Result buffer - int[mc_mr_get_result[" );
      result_buff          =  (void *) result_buff_int;
      temp_result_buff_int = NEW_N( int, result_len,
                                    "Temp result buffer - int[mc_mr_get_result[" );
   ...
   {
      result_buff_long      = NEW_N( LONGLONG, result_len,
                                     "Result buffer - long[mc_mr_get_result[" );
      result_buff           =  (void *) result_buff_long;
      temp_result_buff_long = NEW_N( LONGLONG, result_len,
                                     "Temp result buffer - long[mc_mr_get_result[" );
   ...
   for (i=0; i<subrec_qty; i++)
   {
     ...
     {
         status = mc_read_results( dbid, state, i, 1, results,
                                   (void *) temp_result_buff );
         ...
         for (j=0; j<subrec_ptr->qty_blocks*2; j++)
         {
            ...
            for (k=elem_id_start*result_vec_len;
                  k<elem_id_stop*result_vec_len;
                  k++)
            {
 ...
 */
Return_value
mc_mr_get_subrec_list(int dbid, int subrec_qty,
                      Subrecord *p_subrec, char *class_name,
 ...
 */
Return_value
mc_mr_get_subrec_vars(int dbid, int subrec_qty,
                      Subrecord *p_subrec,
                      char *class_name, char *subrec_name,
 ...
 */
Return_value
mc_mr_is_valid_svar(Subrecord *p_subrec, char *svar, Bool_type *valid )
{
  int j=0;
  *valid = FALSE;
  for (j=0; 
       j<p_subrec->qty_svars; 
       j++)
  {
 ...
 */
Return_value
mc_mr_dump_subrecs(int dbid, char *fname,
                   int subrec_qty, Subrecord *p_subrec)
{
   int  i, j;
   ...
   for ( i=0; i< subrec_qty; i++ )
   {
      ...
      fprintf(fp, "\n\nSubrecord %d  Name=\"%s\" - [Class=\"%s\"] ",
              i,  p_subrec[i].name, p_subrec[i].class_name);
      fprintf(fp, "\n\tState Variables:");
      for (j=0; j<p_subrec[i].qty_svars; j++)
      {
 ...
 */
Return_value
mc_mr_get_param_list( int dbid,      Bool_type ti_param,
                      char *search_pattern,
                      int *list_len, char **param_list )
{
   int  num_entries;
   char **temp_list;
   ...
   {
      num_entries = mc_ti_htable_search_wildcard(dbid, 0, FALSE,
                    "*", "NULL", "NULL",
                    NULL );
 ...
 */
Return_value
mc_mr_get_param_attributes( int dbid, char *param_name, Bool_type ti_param,
                            int *type, int *len )
{
   int  byte_count;
   int  i;
 ...
 */
Return_value
mc_mr_get_param_data( int dbid, char *param_name, Bool_type ti_param,
                      void **param_data )
{
   int  byte_count;
   int  i;
 ...
 */
Return_value
mc_mr_open_input_dbase( char *fname, Mili_analysis *analy )
{
   Famid fid;
   Return_value status = OK;
