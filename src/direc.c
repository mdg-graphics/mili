 ...
 */
static Bool_type
valid_dir_entry_data( Dir_entry_type etype, int string_qty, char **strings )
{
   int i;
...
commit_dir( Mili_family *fam )
{
   ...
   if (rval == OK)
   {
      ...
      num_written = fam->write_funcs[M_INT8]( fam->cur_file,
                                             p_fd->dir_entries, words );
      ...
      if(fam->char_header[DIR_VERSION_IDX]==1){
         ...
         fam->next_free_byte += (words + QTY_DIR_HEADER_FIELDS)
                             * EXT_SIZE( fam, M_INT );
      }
   }
 ...
 */
Return_value
load_directories( Mili_family *fam )
{
   int fcnt;
   int fnum;
