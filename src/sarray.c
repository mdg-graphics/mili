...
static unsigned long long max = (CLONGLONGCONST(1) << (sizeof(SAIntType) * 8)) - 1;
 ...
 * insertion.
 */
Return_value sa_add(StringArray *p_sarr, char *str, int *qty)
{
    StringArray sarr;
    ...
    *qty = SAQTY(sarr);
    ...
    if ( avail < add )
    {
        ...
        /* Move the strings. */
        for ( p_src = sarr + blen - 1, p_dest = sarr + newlen - 1; p_src >= p_stop; *p_dest-- = *p_src-- )
            ;
 ...
 */
static int sort_sa_offsets(const void *elem1, const void *elem2)
{
    return strcmp(sort_sarr + *((SAIntType *)elem1), sort_sarr + *((SAIntType *)elem2)) * sort_coefficient;
}
 ...
 * respectively.
 */
void sa_sort(StringArray sarr, int ascending)
{
    int qty;
    ...
    /* Sort 'em */
    qsort(SAI(sarr) + RESERVED_INT_QTY, qty, sizeof(SAIntType), sort_sa_offsets);
}
    ...
    while ( do_more && rd == p_input )
    {
        ...
        else
        {
            ...
            if ( status != OK )
            {
                ...
                {
                    fprintf(stderr, "StringArray - unable to add string \"%s\".\n", p_input);
                }
                else if ( status == ALLOC_FAILED )
                {
                    fprintf(stderr, "StringArray - failed to grow buffer.\n");
                }
