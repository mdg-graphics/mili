 ...
 */
Return_value ti_file_seek(Famid fam_id, LONGLONG offset)
{
    Mili_family *fam;
    int status;
 ...
 */
static Return_value ti_read_scalar(Famid fam_id, Param_ref *p_pr, void *p_value)
{
    int file, entry_idx;
    LONGLONG offset;
    Return_value rval;
 ...
 */
Return_value mc_ti_read_scalar(Famid fam_id, char *name, void *p_value)
{
    ...
    status = ti_htable_search(fam->ti_param_table, new_name, FIND_ENTRY, &phte);
    if ( phte == NULL )
    {
        return status;
    }
 ...
 */
Return_value mc_ti_wrt_scalar(Famid fam_id, int type, char *name, void *p_value)
{
    Mili_family *fam;
 ...
 */
static Return_value ti_read_string(Famid fam_id, Param_ref *p_pr, char *p_value)
{
    int file, entry_idx;
    LONGLONG offset;
    Return_value rval;
 ...
 */
Return_value mc_ti_read_string(Famid fam_id, char *name, char *p_value)
{
    Mili_family *fam;
    Htable_entry *phte;
    char new_name[128];
 ...
 */
Return_value mc_ti_wrt_string(Famid fam_id, char *name, char *value)
{
    Mili_family *fam;
...
Return_value read_htable_array(Famid fam_id, char *name, void **p_data, int *num_items_read)
{
    ...
    *num_items_read = 0;
    ...
    while ( next )
    {
        ...
        nitems = (fam->read_funcs[M_INT])(fam->cur_file, p_pr->dims, (LONGLONG)p_pr->rank);
        ...
        switch ( num_type )
        {
            ...
            case M_INT4:
                ...
                {
                    idata = RENEW_N(int, idata, current_count, cell_qty, "Param array bufr");
                }
                ...
                nitems = (fam->read_funcs[M_INT])(fam->cur_file, idata + current_count, (LONGLONG)cell_qty);
                length /= 4;
                break;
            ...
            case M_INT8:
                ...
                {
                    lidata = RENEW_N(LONGLONG, lidata, current_count, cell_qty, "Param array bufr");
                }
                ...
                nitems = (fam->read_funcs[M_INT8])(fam->cur_file, lidata + current_count, (LONGLONG)cell_qty);
                length /= 8;
                break;
            ...
            case M_FLOAT4:
                ...
                {
                    fdata = RENEW_N(float, fdata, current_count, cell_qty, "Param array bufr");
                }
                ...
                nitems = (fam->read_funcs[M_FLOAT])(fam->cur_file, fdata + current_count, (LONGLONG)cell_qty);
                length = (length / 4) - 2;
                break;
            ...
            case M_FLOAT8:
                ...
                {
                    ddata = RENEW_N(double, ddata, current_count, cell_qty, "Param array bufr");
                }
                ...
                nitems = (fam->read_funcs[M_FLOAT8])(fam->cur_file, ddata + current_count, (LONGLONG)cell_qty);
                length /= 8;
                break;
        }
 ...
 */
static Return_value ti_read_array(Famid fam_id, Param_ref *p_pr, void **p_data, int *num_items_read)
{
    int file, entry_idx;
    int cell_qty;
    int i;
    ...
    nitems = (fam->read_funcs[M_INT])(fam->ti_cur_file, p_pr->dims, (LONGLONG)p_pr->rank);
    if ( nitems != (LONGLONG)p_pr->rank )
    {
        free(p_pr->dims);
        return SHORT_READ;
    ...
    switch ( num_type )
    {
        ...
        case M_INT4:
            ...
            p_data[0] = (void *)idata;
            nitems = (fam->read_funcs[M_INT])(fam->ti_cur_file, idata, (LONGLONG)cell_qty);
            length /= 4;
            break;
        ...
        case M_INT8:
            ...
            p_data[0] = (void *)lidata;
            nitems = (fam->read_funcs[M_INT8])(fam->ti_cur_file, lidata, (LONGLONG)cell_qty);
            length /= 8;
            break;
        ...
        case M_FLOAT4:
            ...
            p_data[0] = (void *)fdata;
            nitems = (fam->read_funcs[M_FLOAT])(fam->ti_cur_file, fdata, (LONGLONG)cell_qty);
            length = (length / 4) - 2;
            break;
        ...
        case M_FLOAT8:
            ...
            p_data[0] = (void *)ddata;
            nitems = (fam->read_funcs[M_FLOAT8])(fam->ti_cur_file, ddata, (LONGLONG)cell_qty);
            length /= 8;
            break;
    }
    free(p_pr->dims);
 ...
 */
Return_value mc_ti_read_array(Famid fam_id, char *name, void **p_data, int *num_items_read)
{
 ...
 */
Return_value mc_ti_wrt_array(Famid fam_id, int type, char *name, int order, int *dimensions, void *data)
{
    Mili_family *fam;
    Return_value status;
    ...
    if ( milisilo )
    {
        status = mc_silo_wrt_array(fam_id, type, name, order, dimensions, data);
        return (status);
    }
#endif
    ...
    status = write_array(fam, type, name, order, dimensions, data, TI_PARAM);
    return status;
}
 ...
 */
Return_value mc_ti_def_class(Famid fam_id, int meshid, int state, int matid, char *superclass, Bool_type meshvar,
                             Bool_type nodal, char *short_name, char *long_name)
{
    Mili_family *fam;
 ...
 * Define class information for a new TI Class.
 */
Return_value mc_ti_set_class(Famid fam_id, int meshid, int state, int matid, char *superclass, Bool_type meshvar,
                             Bool_type nodal, char *short_name, char *long_name)
{
    Return_value stat;
    stat = mc_ti_def_class(fam_id, meshid, state, matid, superclass, meshvar, nodal, short_name, long_name);
    return stat;
}
...
{
    sprintf(new_name, "[/Mesh-%d/Sname-%s/Scls-%s/Mat-%d/]", meshid, short_name, superclass, mat_id);
 ...
 */
Return_value mc_ti_undef_class(Famid fam_id)
{
    Mili_family *fam;
 ...
 */
Return_value mc_ti_savedef_class(Famid fam_id)
{
    Mili_family *fam;
 ...
 */
Return_value mc_ti_restoredef_class(Famid fam_id)
{
    Mili_family *fam;
 ...
 */
void mc_ti_enable(Famid fam_id)
 ...
 */
void mc_ti_disable(Famid fam_id)
 ...
 */
void mc_ti_data_found(Famid fam_id)
 ...
 */
void mc_ti_data_not_found(Famid fam_id)
 ...
 */
Bool_type mc_ti_check_if_data_found(Famid fam_id)
 ...
 * Make a TI name with embedded class info
 */
Return_value mc_ti_make_var_name(Famid fam_id, char *name, char *class, char *new_name)
{
    Mili_family *fam;
    ...
    {
        sprintf(new_name, "%s[/Mesh-%d/Sname-%s/++/IsMvar-%s/IsNod-%s/Scls-%s/Mat-%d/St-%d]", name, fam->ti_meshid,
                temp_class, meshvar, node_or_elem, fam->ti_superclass_name, fam->ti_matid, fam->ti_state);
    }
 ...
 * Make a TI name with embedded class info
 */
Bool_type mc_ti_find_var_name(Famid fam_id, char *name, char *class, char **var_name)
{
    Mili_family *fam;
    ...
    num_entries = htable_search_wildcard(table, 0, FALSE, vname, sname, "NULL", NULL);

    if ( num_entries == 0 )
        return FALSE;

    wildcard_list = NEW_N(char *, num_entries, "WC List");

    num_entries = htable_search_wildcard(table, 0, FALSE, vname, sname, "NULL", wildcard_list);
 ...
 */
Bool_type mc_ti_verify_var_name(Famid fam_id, char *name)
{
    Htable_entry *phte;
    Mili_family *fam;
    int status = OK;
 ...
 * length.
 */
Return_value mc_ti_get_class(Famid fam_id, int meshid, char *name, char *class, int *state, int *matid, int *superclass,
                             Bool_type *isMeshvar, Bool_type *isNodal, int *datatype, int *datalength)
{
    return mc_ti_get_data_attributes(fam_id, meshid, name, class, state, matid, superclass, isMeshvar, isNodal,
                                     datatype, datalength);
}
 ...
 * length.
 */
Return_value mc_ti_get_data_attributes(Famid fam_id, int meshid, char *name, char *this_class, int *state, int *matid,
 ...
 * class type
 */
int mc_ti_get_superclass_from_name(char *name)
{
 ...
 */
int mc_ti_get_material_from_name(char *name)
{
 ...
 * length.
 */
Return_value mc_ti_get_data_len(Famid fam_id, char *name, int *datatype, int *datalength)
{
    Htable_entry *phte;
    Hash_table *table;
    Param_ref *p_pr;
 ...
 */
Return_value mc_ti_get_metadata_from_name(char *name, char *class, int *meshid, int *state, int *matid, int *superclass,
