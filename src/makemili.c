 ...
 */
static void writeVariable_json(char *var_elem, Variable *variable, JSON_Object *root_object)
{
    ...
    if ( variable->realNameCount > 0 )
    {
        ...
        strcat(components, "]");
        json_object_dotset_value(root_object, variable_string, json_parse_string(components));
    }
    ...
    switch ( sv->agg_type )
    {
        ...
        case ARRAY:
            ...
            strcat(numbers, "]");
            json_object_dotset_value(root_object, variable_string, json_parse_string(numbers));
            free(numbers);
            break;
        default:
            break;
    ...
    json_object_dotset_value(root_object, variable_string, json_parse_string(subrec_string));
}
/**
...
static void add_variables(Famid database_id, Hash_table *variable_ht, Variable *variable, char *class_name)
{
    State_variable *sv;
    int i;
            ...
            switch ( sv->agg_type )
            {
                ...
                case VEC_ARRAY:
                    ...
                    for ( i = 0; i < sv->vec_size; i++ )
                    {
                        ...
                        add_variables(database_id, variable_ht, component_sv, class_name);
                    }
 ...
 */
static Hash_table *process_state_variables(Famid database_id, Hash_table *classTable)
{
    int i, j, k, l;
    int srec_qty = 0;
    Return_value rval = OK;
    ...
    char components[512];
    rval = mc_query_family(database_id, QTY_SREC_FMTS, NULL, NULL, (void *)&srec_qty);
    ...
    if ( rval == OK )
    {
        ...
        for ( i = 0; i < srec_qty; i++ )
        {
            ...
            rval = mc_query_family(database_id, QTY_SUBRECS, (void *)&i, NULL, (void *)&subrecord_count);
            ...
            if ( rval == OK )
            {
                if ( !subrec_string )
                {
                    ...
                    subrec_string = NEW_N(char, sub_size, "subrec_string in process_state_variables");
                }
                Subrecord sr;
                for ( j = 0; j < subrecord_count; j++ )
                {
                    ...
                    if ( rval == OK )
                    {
                        ...
                        {
                            State_variable temp;
                            rval = mc_get_svar_def(database_id, sr.svar_names[k], &temp);
                            if ( htable_search(variable_ht, temp.short_name, FIND_ENTRY, &entry) == OK )
                            {
                                ...
                                if ( htable_search(classTable, sr.class_name, FIND_ENTRY, &entry) == OK )
                                {
                                    ...
                                    entry = NULL;
                                    if ( htable_search(mili_class->variables, var->state_var->short_name, FIND_ENTRY,
                                                       &entry) != OK )
                                    {
                            ...
                            if ( strcmp(sr.class_name, "glob") == 0 || strcmp(sr.class_name, "mat") == 0 )
                            {
                                var->center = AVT_ZONECENT;
                            }
                            var->state_var = NEW(State_variable, "State Variable");

                            rval = mc_get_svar_def(database_id, sr.svar_names[k], var->state_var);
                            if ( rval != OK )
                            {
                                continue;
                            }
                                    ...
                                    for ( l = 0; l < var->state_var->vec_size; l++ )
                                    {
                                        ...
                                        if ( htable_search(variable_ht, temp.short_name, FIND_ENTRY, &entry) == OK )
                                        {
                                            ...
                                            if ( component_var->subrec_ids[component_var->subrec_count - 1] != j )
                                            {
                                                ...
                                                component_var->subrec_name[component_var->subrec_count] =
                                                    (char *)calloc(sizeof(char), 64);
                                                strcpy(component_var->subrec_name[component_var->subrec_count],
                                                       sr.name);
                                                    ...
                                                    if ( strcmp(var->realNames[0], "stress") == 0 )
                                                    {
                                                               ...
                                                               strcmp(temp.short_name, "szx") == 0) )
                                                        {
                                                            var->realNames[var->realNameCount] =
                                                                calloc(1, strlen(temp.short_name) + 1);
                                        ...
                                        rval = mc_get_svar_def(database_id, components, component_var->state_var);
...
static void cleanVariable(Variable *variable)
{
    /*
    typedef struct
    {
...
static void cleanVariableTable(Hash_table *table)
{
    int current_index = 0;
 ...
 *    Famid database_id Identifier for the database
 */
static int gather_parameters(Famid database_id)
{
    ...
    for ( i = 0; i < parameter_count; i++ )
    {
        ...
        else if ( strncmp(parameters_next[i], "MAT_NAME_", 9) )
        {
            ...
            status = htable_search(fam->param_table, parameters_next[i], FIND_ENTRY, &entry);
    ...
    if ( elementSetCount > 0 )
    {
        ...
        for ( i = 0; i < parameter_count; i++ )
        {
            if ( parameters_next[i] != NULL && strstr(parameters_next[i], "_es_") )
            {
                ...
                status = mc_ti_read_array(database_id, parameters_next[i], (void **)&es_ids, &items_read);
                elementSets[currentElementSet].integrationPoints = NEW_N(int, items_read, "integrationPoints");
 ...
 */
static void write_state_variables_json(Hash_table *variable_ht, JSON_Object *root_object)
{
    int cur_table_entry;
    Htable_entry *entry;
    Variable *sv;
    ...
    for ( cur_table_entry = 0; cur_table_entry < variable_ht->size; cur_table_entry++ )
    {
 ...
 */
static int countClasses(Famid database_id)
{
    int qty_classes = 0;
    ...
    for ( mesh_id = 0; mesh_id < fam->qty_meshes; mesh_id++ )
    {
        for ( SUPERCLASS = 0; SUPERCLASS < M_QTY_SUPERCLASS; SUPERCLASS++ )
        {
            ...
            status =
                mc_query_family(database_id, QTY_CLASS_IN_SCLASS, (void *)int_args, NULL, (void *)&current_class_count);
            if ( status != OK )
            {
                return -1;
            }
 ...
 */
static int readClasses(Famid database_id, Hash_table *classes, Mili_Class *classes_array)
{
    int classes_count = 0;
    int mesh_id;
    int SUPERCLASS;
    ...
    for ( mesh_id = 0; mesh_id < fam->qty_meshes; mesh_id++ )
    {
        for ( SUPERCLASS = 0; SUPERCLASS < M_QTY_SUPERCLASS; SUPERCLASS++ )
        {
            ...
            status = mc_query_family(database_id, QTY_CLASS_IN_SCLASS, (void *)int_args, NULL, (void *)&qty_classes);
            if ( status != OK )
            {
                return status;
            }
            ...
            {
                status = mc_get_simple_class_info(database_id, mesh_id, SUPERCLASS, i, short_name, long_name,
 ...
 */
static void writeClasses_json(Hash_table *classTable, Mili_Class *miliClasses, JSON_Object *root_object)
{
    int qty_classes = 0;
    int qty_written = 0;
    char class_elem[64];
    ...
    for ( i = 0; i < qty_classes; i++ )
    {
        ...
        if ( variables->qty_entries > 0 )
        {
            ...
            json_object_dotset_value(root_object, class_elem_variable, json_parse_string(class_elem_variables));
        }
    }
}
...
static void writeElementSets_json(JSON_Object *root_object)
{
 ...
 */
static int writeMaterials_json(Famid database_id, JSON_Object *root_object)
{
    int qty_classes = 0;
    int i;
    int start_ident;
    ...
    for ( mesh_id = 0; mesh_id < fam->qty_meshes; mesh_id++ )
    {
        ...
        for ( i = 0; i < qty_classes; i++ )
        {
            status = mc_get_simple_class_info(database_id, mesh_id, M_MAT, i, short_name, long_name, &start_ident,
                                              &stop_ident);
 ...
 */
static void write_steps_json(Famid database_id)
{
    Mili_family *fam;  // This is the Mili database plot file
...
static void getGlobalName(char fileName[256], Mili_family *fam)
{
 ...
 */
static FILE *create_outfile(Famid database_id, int global)
{
 ...
 *   @return void
 */
static int write_database_metadata_json(Famid database_id, JSON_Object *root_object)
{
    int rval = OK;
    numProcessors = 1;
 ...
 */
Return_value write_mili_metadata(Famid database_id, int global)
{
...
int mc_create_metadata(char *db_name)
{
    char *path_index;
    Famid fam_id;
    int rval = OK;
 ...
 *
 */
int update_visit_file(char *database_name, Famid database_id)
{
    Mili_family *fam;  // This is the Mili database plot file
...
static void merge_element_sets(JSON_Object *base, const JSON_Object *incoming)
{
    ...
    for ( i = 0; i < count; i++ )
    {
        ...
        if ( has_element_sets )
        {
            test = json_object_get_object(base_object, element_set_name);
            if ( test )
            {
                continue;
            }
            adding_value = json_value_deep_copy(json_object_get_value_at(in_object, i));
            status = json_object_set_value(base_object, element_set_name, adding_value);
        }
        else
        {
...
static void remove_subrecords(JSON_Object *base)
{
    int i;
    const char *var_name;
    JSON_Status rval;
...
static void merge_variables(JSON_Object *base, const JSON_Object *incoming)
{
    int i;
    const char *var_name;
    JSON_Status status;
...
static int get_processor_id(char *file_name, int *num_length)
{
    int pos = 0;
    int process_number;
    *num_length = 0;
...
static void remove_element_counts(JSON_Object *base)
{
    int i;
    const char *var_name;
    JSON_Status rval;
...
static void merge_classes(JSON_Object *base, const JSON_Object *incoming)
{
    int i, j, k;
    const char *var_name;
    JSON_Status status;
 ...
 */
static void fix_count(JSON_Object *base_object, char *target)
{
    LONGLONG count;
