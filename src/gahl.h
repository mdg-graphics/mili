...
/* Public routines. */
Hash_table *
htable_create(                 /* Create a hash table */
   int table_size );          /* Requested number of buckets in table */

Return_value
htable_search(                 /* Search to insert/find an entry in the table */
...
Return_value
ti_htable_search(              /* Search to insert/find an entry in the TI hash table */
   Hash_table *table,         /* Hash table to operate on */
   char *key,                 /* Hash key */
   Hash_action op,            /* Operation to perform */
   Htable_entry **entry );    /* (output) Destination for entry address */
...
Return_value
ti_htable_add_entry_data(      /* Add data to an entry in the table */
   Hash_table *table,         /* Table to operate on */
   char *key,                 /* Hash key */
   Hash_action op,            /* Search operation to perform */
   void *entry_data );        /* Destination for array address */
...
void
htable_delete_wildcard_list(   /* Free up storage for a wildcard result list */
   int list_count,            /* Number of entries in the list */
   char **wildcard_list );    /* The list of matching entries */

int
htable_search_wildcard(        /* Search for a list of entries based on two input keys */
   Hash_table *table,         /* Table to operate on */
   ...
   char *key3,
   char **return_list );      /* List of matching entries */
