...
translate_segment( char *fmt )
{
   ...
   for ( p_end = p_cur, p_dest = sav; p_end < destbuf + cur_len;
         *p_dest++ = *p_end++ );
...
advance_va_list( char *fmt )
{
   ...
   while ( regexec( &all_re, p_src, (LONGLONG) 1, all_match, 0 ) == 0 )
   {
      ...
      switch ( *p_argtype )
      {
         ...
         default:
            fprintf( stderr,
                     "libeprtf: error matching conversion expression!\n" );
            return;
      }
   }
...
translate_string( char *fmt )
{
   ...
   while ( regexec( &t_re, p_src, (LONGLONG) 1, t_match, 0 ) == 0 )
   {
      ...
      {
         for ( p_test = p_tabfmt - 2, no_tab = 1; *p_test == '%';
               no_tab ^= 0x1, p_test-- );
         if ( no_tab )
         {
      ...
      for ( p_dest = tmp, p_copy = p_last; p_copy < p_tabfmt;
            *p_dest++ = *p_copy++ );
      *p_dest = '\0';
      ...
      else
      {
         ...
         for ( p_dest = colbuf, p_copy++,
               bound = p_src + t_match[0].rm_eo - 1;
               p_copy < bound;
               *p_dest++ = *p_copy++ );
         *p_dest = '\0';
         col = atoi( colbuf );
 ...
 */
Return_value
eprintf( int *num_written, char *format_spec, ... )
{
#ifdef HAVE_EPRINT
   int status;
 ...
 */
Return_value
efprintf( FILE *p_f, char *format_spec, ... )
{
#ifdef HAVE_EPRINT
   int status;
 ...
 */
Return_value
esprintf( char *p_output, char *format_spec, ... )
{
#ifdef HAVE_EPRINT
   int status;
