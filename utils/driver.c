...
Return_value parse_procmat_select_list(char *list_string, Bool_type procmat, int select_len, short *select_list)
{
    ...
    for ( i = 0; i < procmat_num_index; i++ )
    {
        ...
        for ( j = procmat_nums[i] - 1; j <= procmat_nums[i + 1] - 1; j++ )
        {
            select_list[j] = 1;
            if ( procmat == PROC )
            {
...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr,
                "Unable to read open file limit.\n"
    ...
    else
    {
        ...
        else
        {
            ...
            {
                fprintf(stderr,
                        "Unable to read new open file limit.\n"
                        "(getrlimit(RLIMIT_NOFILE, &rl) failed)\n"
                        "(%d, %s)",
                        errno, strerror(errno));
                fprintf(stderr, "Running with default 1020 filedescriptors.\n");
    ...
    else if ( db_type == MILI_DB_TYPE )
    {
        ...
        *out_db = NEW(Mili_analysis, "Mili_analysis struct");
        ...
        if ( p_f != NULL && !env.batch_overwrite )
        {
            ...
            fprintf(stderr, "\n\t*            WARNING!  WARNING!                    *");
            fprintf(stderr, "\n\t*                                                  *");
            fprintf(stderr, "\n\t*      Output file %s exists                        ", env.output_file_name);
            fprintf(stderr, "\n\t*                                                  *");
            fprintf(stderr, "\n\t*                                                  *");
            fprintf(stderr, "\n\t****************************************************\n\n");
        ...
        else  // User has turned on the output of tfiles
        {
            ...
            {
                mc_print_error(
                    "Trying to alter output time stamp version of existing database. \nAppending in existing output's "
                    "format.",
                    status);
            }
        }
        ...
        if ( env.ti_enabled )
        {
            ...
            {
                fprintf(stderr,
                        "There was an error loading labels that XmiliCS could not recover from. Exiting .....\n");
                // exit(201);
            }
#if TIMER
            stop_time = clock();
...
void find_pad_count(char *input_file_name)
{
    ...
    while ( (dirp = readdir(dir)) != NULL )
    {
        ...
        if ( strstr(dirp->d_name, env.plotfile_name) != NULL && dirp->d_name[last_pos] == 'A' &&
             dirp->d_name[last_pos - 1] != '_' )
        {
...
int wait_for_start(char *input_file_name)
{
    int diff = 0;
    int pad = 0;
    char directory[MAXPATHLEN];
...
int file_select(struct dirent *entry)
{
    ...
    if ( (strncmp(env.plotfile_name, entry->d_name, strlen(env.plotfile_name)) == 0) &&
         (entry->d_name[ptr - 1] == 'A') && (entry->d_name[ptr - 2] >= '0' && entry->d_name[ptr - 2] <= '9') &&
         !(entry->d_name[ptr - 2] == '_') && !((entry->d_name[cptr] == '_') && (entry->d_name[cptr + 1] == 'c')) )
    {
...
int ti_file_select(struct dirent *entry)
{
    ...
    int ptr = strlen(entry->d_name);
    if ( (strncmp(env.plotfile_name, entry->d_name, strlen(env.plotfile_name)) == 0) &&
         !((entry->d_name[cptr] == '_') && (entry->d_name[cptr + 1] == 'c')) && (entry->d_name[ptr - 1] == 'A') &&
         (entry->d_name[ptr - 2] == '_') && (entry->d_name[ptr - 3] == 'I') && (entry->d_name[ptr - 4] == 'T') &&
