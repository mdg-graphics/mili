 ...
 */
void combine_umesh(Mili_analysis *in_db, Mili_analysis *out_db)
{
 ...
 *
 */
Return_value mc_combine_classes(Mili_family *in_fam, Mili_family *out_fam)
{
    int class_qty_in, id_blocks, *ids, i, j, superclass;
    char short_name[128];
    char long_name[128];
    ...
    for ( i = 0; i < in_fam->qty_meshes; i++ )
    {
        ...
        if ( rval == OK )
        {
            ...
            {
                rval = mc_get_class_info_by_index(in_fam, &i, &j, &superclass, short_name, long_name);

                if ( rval == OK )
                {
                    rval = mc_query_family(out_fam->my_id, CLASS_SUPERCLASS, modifiers, short_name, query);

                    if ( rval == ENTRY_NOT_FOUND )
                    {
                        rval = mc_def_class(out_fam->my_id, i, superclass, short_name, long_name);
                        if ( superclass == M_UNIT || superclass == M_MAT || superclass == M_MESH ||
...
Return_value define_variable(Mili_family *in, Mili_family *out, char *variable_name)
{
    ...
    switch ( state_var.agg_type )
    {
        case (SCALAR):
            ...
            rval = mc_def_svars(out_fam_id, 1, state_var.short_name, 0, state_var.long_name, 0, types);
            break;
        case (ARRAY):
            rval = mc_def_arr_svar(out_fam_id, state_var.rank, state_var.dims, state_var.short_name,
                                   state_var.long_name, state_var.num_type);
            break;
        case (VECTOR):
            ...
            rval = mc_def_vec_svar(out_fam_id, state_var.num_type, state_var.vec_size, state_var.short_name,
                                   state_var.long_name, state_var.components, state_var.component_titles);
            break;
        case (VEC_ARRAY):
            for ( idx = 0; idx < state_var.vec_size; idx++ )
            {
            ...
            rval = mc_def_vec_arr_svar(out_fam_id, state_var.rank, state_var.dims, state_var.short_name,
                                       state_var.long_name, state_var.vec_size, state_var.components,
                                       state_var.component_titles, state_var.num_type);
            break;
        default:
            break;
    }  // End switch/case
 ...
 * length.
 */
Return_value mc_get_data_len(Famid fam_id, char *name, int *datatype, long *datalength)
{
    Htable_entry *phte;
    Param_ref *p_pr;
    LONGLONG *entry;
...
Return_value add_parameter(int fam_id, int out_fam_id, char *name)
{
...
Return_value add_ti_parameter(int fam_id, int fam_id_out, char *name)
{
    ...
    int datatype = 0, num_items_read = 0;
    long datalength = 0;
        ...
        switch ( datatype )
        {
            case (M_INT):
                ...
                if ( status == OK )
                {
                    ...
                    status = mc_ti_wrt_array(fam_id_out, datatype, name, 1, dims, (void *)data_int);
                }
            ...
            case (M_INT8):
                ...
                {
                    dims[0] = num_items_read;
                    status = mc_ti_wrt_array(fam_id_out, datatype, name, 1, dims, (void *)data_long);
                }
                break;
            ...
            case (M_FLOAT):
                ...
                {
                    dims[0] = num_items_read;
                    status = mc_ti_wrt_array(fam_id_out, datatype, name, 1, dims, (void *)data_float);
                }
                break;
            case (M_FLOAT8):
                ...
                {
                    dims[0] = num_items_read;
                    status = mc_ti_wrt_array(fam_id_out, datatype, name, 1, dims, (void *)data_double);
                }
                break;
...
Return_value mc_copy_geometry(Mili_analysis **in_db, Mili_analysis *out_db, TILabels *labels)
{
    ...
    int *temp_conns = NULL, *temp_mats = NULL, *temp_parts = NULL, *out_conns = NULL;
    Return_value status = (Return_value)OK;
    ...
    for ( iter = labels->labels; iter != NULL; iter = iter->next )
    {
        ...
        if ( iter->sclass == M_NODE )
        {
            ...
            status = mc_def_nodes(out_db->db_ident, iter->mesh_id, iter->sname, 1, iter->size, coords);
            mc_print_error("mc_def_nodes: ", status);
            status = mc_def_node_labels(out_db->db_ident, iter->mesh_id, iter->sname, iter->size, iter->labels);
            mc_print_error("mc_def_nodes: ", status);
            if ( coords != NULL )
            {
                free(coords);
    ...
    for ( iter = labels->labels; iter != NULL; iter = iter->next )
    {
        ...
        for ( cur_proc = 0; cur_proc < num_procs; cur_proc++ )
        {
            ...
            status =
                mc_load_conns(in_db[cur_proc]->db_ident, iter->mesh_id, iter->sname, temp_conns, temp_mats, temp_parts);
            if ( status != OK )
            {
        ...
        conn_size = -1;
        status = mc_def_conn_seq_labels(out_db->db_ident, iter->mesh_id, iter->sname, 1, iter->size, iter->labels,
                                        out_conns);
        if ( out_conns != NULL )
        {
            free(out_conns);
 ...
 *
 */
Return_value mc_subrec_exists(Famid fam_id, int srec_id, char *subrec_name, char *class_name)
{
    Return_value rval = (Return_value)OK;
    Mili_family *fam;
 ...
 *
 */
Return_value convert_local_moids(int **global_ids, int *in_count, Subrecord *subrec, int *map)
{
 ...
 *
 */
Return_value get_subrec_contributions(Mili_analysis **in_db, Mili_analysis *out_db, TILabels *in_labels)
{
    ...
    for ( cur_proc = 0; cur_proc < num_procs; cur_proc++ )
    {
        ...
        rval = mc_query_family(in_dbid, QTY_SREC_FMTS, NULL, NULL, (void *)&srec_qty);
        ...
        {
            rval = mc_query_family(in_dbid, SREC_MESH, &srec_id, NULL, (void *)&mesh_id);
            ...
            for ( i = 0; i < subrec_qty; i++ )
            {
                ...
                rval = mc_query_family(in_dbid, CLASS_SUPERCLASS, &mesh_id, subrec.class_name, (void *)&stype);
                if ( rval != OK )
                {
    ...
    for ( srec_id = 0; srec_id < out_srec_qty; srec_id++ )
    {
        ...
        for ( working_container = subrec_container; working_container != NULL;
              working_container = working_container->next )
        {
 ...
 *
 */
Return_value mc_combine_srecs(Mili_analysis **in_db, Mili_analysis *out_db, TILabels *in_labels)
{
    int num_procs = in_labels->num_procs;
    ...
    for ( cur_proc = 0; cur_proc < num_procs; cur_proc++ )
    {
        ...
        for ( srec_id = 0; srec_id < srec_qty; srec_id++ )
        {
            ...
            for ( i = 0; i < subrec_qty; i++ )
            {
                ...
                if ( found )
                {
                    if ( (stype != M_MESH) && (stype != M_MAT) && (stype != M_UNIT) )
                    {
                        ...
                        rval = convert_local_moids(&global_ids, &count, &subrec,
                                                   iter->map + iter->offset_per_processor[cur_proc]);
    ...
    for ( srec_id = 0; srec_id < out_srec_qty; srec_id++ )
    {
              ...
              working_container = working_container->next )
        {
            ...
            if(   ( working_container->stype != M_MESH ) &&
               ...
               working_container->subrecord_map = (int*)
            calloc(working_container->total_external_contributions,sizeof(int));
            }*/
            for ( i = 0; i < num_procs; i++ )
            {
                /*  This work will be in the next working release */ /*
                 if(   ( working_container->stype != M_MESH ) &&
                       ...
                       ( working_container->stype != M_UNIT ) ) {
                    if(i > 0) {
                       working_container->subrecord_offsets_per_proc[i] =
                 working_container->subrecord_offsets_per_proc[i-1] +
...
int get_param_type(int fam_id, char *param_name)
{
    Mili_family *fam;
...
Return_value combine_non_state_definitions(Mili_analysis **in_db, Mili_analysis *out_db, TILabels *labels)
{
    ...
    for ( cur_proc = 0; cur_proc < num_procs; cur_proc++ )
    {
        ...
        count = htable_search_wildcard(fam_list[in_db[cur_proc]->db_ident]->param_table, 0, FALSE, "*", "NULL", "NULL",
                                       list);
        ...
        for ( i = 0; i < count; i++ )
        {
            ...
            status = htable_search(fam_list[out_db->db_ident]->param_table, list[i], FIND_ENTRY, &entry);
...
Return_value merge_state_data(int proc, TILabels *in_labels, Mili_analysis *in_db, Mili_analysis *out_db)
{
    Return_value rval = (Return_value)OK;
    ...
    size_t state_size;
    LONGLONG out_offset, in_offset;
    int state_qty, i, j, k, ii, kk, jj, ll, srec_id, subrec_qty, contribute_subrec, precision, mo_qty, qty_svars,
        qty_out_svars, iorder, stype, num_type, atom_size, in_round, out_round, agg_type, step, iprec;
    char *subrec_name, *class_name;
    ...
    if ( out_db->state_times == NULL ||
         (out_fam->state_qty < env.current_state_max && env.current_state_array_size < env.current_state_max) )
    {
    ...
    for ( i = 0; i < subrec_qty; i++ )
    {
        ...
        {
            out_psubrec = out_psr->subrecs[contribute_subrec];

            if ( rval != ENTRY_NOT_FOUND )
            {
                rval = (Return_value)htable_search(in_fam->srec_meshes[srec_id]->mesh_data.umesh_data,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if ( out_psubrec->mo_qty != iter->size || sub_contibutions[contribute_subrec] > 1 )
                            {
                                ...
                                for ( cur_in_moid_blck = 0; cur_in_moid_blck < in_psubrec->qty_id_blks;
                                      cur_in_moid_blck++ )
                                {
                                    for ( in_moid = in_psubrec->mo_id_blks[cur_in_moid_blck * 2];
                                          in_moid <= in_psubrec->mo_id_blks[cur_in_moid_blck * 2 + 1]; in_moid++ )
                                    {
                                        match_moid = map[in_moid - 1] + 1;
                                        index_out = 0;
                                        for ( cur_out_moid_block = 0; cur_out_moid_block < out_psubrec->qty_id_blks;
                                              cur_out_moid_block++ )
                                        {
                                            if ( out_psubrec->mo_id_blks[cur_out_moid_block * 2] <= match_moid &&
                                                 match_moid <= out_psubrec->mo_id_blks[cur_out_moid_block * 2 + 1] )
                                            {
                                                temp_array[in_index++] =
                                                    index_out + match_moid -
                                                    out_psubrec->mo_id_blks[cur_out_moid_block * 2];
                                                break;
                                            }
                                            index_out += out_psubrec->mo_id_blks[cur_out_moid_block * 2 + 1] -
                                                         out_psubrec->mo_id_blks[cur_out_moid_block * 2] + 1;
                            ...
                            case SCALAR:
                                for ( k = 0; k < mo_qty; k++ )
                                {
                                    ...
                                    for ( kk = 0; kk < atom_size; kk++ )
                                        *(out_buf + index_out + kk) = *(in_buf + k * atom_size + kk);
                                }
                    ...
                    for ( j = 0; j < qty_svars; j++ )
                    {
                        ...
                        for ( k = 0; k < qty_out_svars; k++ )
                        {
                            ...
                            switch ( agg_type )
                            {
                                ...
                                case VEC_ARRAY:
                                    ...
                                    {
                                        object_offset += (out_svar->dims[iorder]) * *(out_svar->list_size) *
                                                         out_psubrec->mo_qty * iprec;
                                    }
                                    break;
                            }
                        }
                        ...
                        switch ( agg_type )
                        {
                            ...
                            case VEC_ARRAY:
                                ...
                                {
                                    int expanded_list_size = recurse_vec_arr_list_size(in_svar);
                                    for ( k = 0; k < mo_qty; k++ )
                                    {
                                        if ( (stype == M_UNIT) || (stype == M_MAT) || (stype == M_MESH) ||
                                             (map == NULL) )
                                        {
                                            index_out = k * out_svar->dims[iorder] * expanded_list_size;
                                        }
                                        else
                                            index_out = (out_svar->dims[iorder]) * expanded_list_size * (map[k]);

                                        for ( ii = 0; ii < (in_svar->dims[iorder]) * expanded_list_size; ii++ )
                                        {
                                            jj = (index_out + ii) * atom_size;
                                            for ( kk = 0; kk < atom_size; kk++ )
                                            {
                                    ...
                                    out_offset +=
                                        (out_svar->dims[iorder]) * expanded_list_size * out_psubrec->mo_qty * iprec;
                                    in_offset +=
                                        (in_svar->dims[iorder]) * expanded_list_size * in_psubrec->mo_qty * iprec;
                                }
                                break;
                            default:
                                break;
...
void l2gnums(int proc, int *offsets, int *inmap, int count, int *loc_list, int *gbl_list)
{
    int offset;
    int i, j;
