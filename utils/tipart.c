...
/* Prototypes */
static void usage();
static void create_mili_filename( int nproc, int procnum,
                                  char *mili_file_by_processor,
                                  char *rootname ) ;
...
main( int argc, char *argv[] )
{
    ...
    */
    ...
    for (i=0; i<map->nproc; i++)
    {
        ...
        create_mili_filename( map->nproc, i, mili_file_by_processor,
                              mili_file ) ;
        ...
        status = mc_ti_def_class( fid, meshid, i, matid, "M_NODE",
                                  TRUE, TRUE, "Node", "Node");
        if ( status != 0 )
        {
            mc_close( fid );
        ...
        status = mc_ti_wrt_array( fid,  M_INT, (char *) var_name, 1, dims, 
                                (void *)  &map->npltg[offset_plt]);
    ...
    status = mc_get_metadata( fid, mili_version, host, arch,
                              timestamp, xmilics_version ) ;
    if ( status != OK ) {
         exit(1);
    }
    status = mc_ti_get_metadata( fid, ti_mili_version, ti_host, ti_arch,
                                 ti_timestamp, ti_user, 
                                 xmilics_version, code_name ) ;
    if ( status != OK ) {
         exit(1);
    }
    ...
    status = mc_ti_def_class( fid, meshid, state, matid, "M_NODE",
                              TRUE, TRUE, "Node", "Node");
 ...
 */
static void
create_mili_filename( int nproc, int procnum,
                      char *mili_file_by_processor, char *rootname )
{
    char proc_number[16];
