 ...
 */
extern int mili_db_open(char *path_root, int *p_dbid)
{
    char *p_c;
    ...
    else
    {
        ...
        for ( p_src = path_root, p_dest = path_text; p_src < p_root_start - 1; *p_dest++ = *p_src++ )
        {
            ;
        }
    ...
    for ( p_src = p_root_start, p_dest = root; p_src <= p_root_end; *p_dest++ = *p_src++ )
    {
        ;
    }
    *p_dest = '\0';
 ...
 */
extern int taurus_db_open(char *path_root, int *p_dbid)
{
    char *p_c;
    char *p_root_start, *p_root_end;
    char *p_src, *p_dest;
 ...
 * Open a plotfile and initialize an database.
 */
Bool_type open_output_dbase(char *path_root, Mili_analysis *dbase)
{
    Famid dbid;
    char *p_c;
    char *p_root_start, *p_root_end;
