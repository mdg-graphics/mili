...
Label *create_label(char *shortname, int nproc, int sclass, int mesh_id, Bool_type isMeshVar, Bool_type isNodal)
{
    ...
    label->num_per_processors = (long *)calloc(nproc, sizeof(long));
...
int load_ti_labels(Mili_analysis **in_db, int nproc, TILabels *labels)
{
    ...
    int *temp_int = NULL, *label_temp_int = NULL;
    long *temp_long = NULL, *label_temp_long = NULL;
    ...
    for ( proc = env.start_proc; proc < env.stop_proc; proc++ )
    {
        ...
        for ( i = 0; i < wildcard_count; i++ )
        {
                 ...
                 strncmp("Node Labels", wildcard_list[i], strlen("Node Labels")) == 0 )
            {
                ...
                status =
                    mc_ti_get_metadata_from_name(wildcard_list[i], labels_class, &labels_meshid, &labels_state,
                                                 &labels_matid, &labels_superclass, &labels_isMeshvar, &labels_isNodal);

                if ( fam->char_header[DIR_VERSION_IDX] > 1 )
                {
                    status = mc_read_param_array_len(fam_id, wildcard_list[i], &datatype, &datalength);
                }
                else
                {
                    status = mc_ti_get_data_len(fam_id, wildcard_list[i], &datatype, &datalength);
                }
                if ( status != OK )
                {
                    continue;
                ...
                {
                    labels->labels = create_label(labels_class, nproc, labels_superclass, labels_meshid,
                ...
                {
                    last_label->next = create_label(labels_class, nproc, labels_superclass, labels_meshid,
                                                    labels_isMeshvar, labels_isNodal);
    ...
    for ( proc = env.start_proc; proc < env.stop_proc; proc++ )
    {
                 ...
                 strncmp("Node Labels", wildcard_list[i], strlen("Node Labels")) == 0 )
            {
                ...
                if ( strncmp("Node Labels", wildcard_list[i], strlen("Node Labels")) != 0 )
                {
                    ...
                    for ( j = 0; j < wildcard_count; j++ )
                    {
                        ...
                        {
                            if ( strncmp("Element Labels[", wildcard_list[j], strlen("Element Labels[")) == 0 )
                            {
                                break;
                            }
                        }
                    ...
                    switch ( datatype )
                    {
                        case (M_INT):
                            ...
                            status =
                                mc_ti_read_array(fam_id, wildcard_list[j], (void **)&label_temp_int, &num_items_read);
                            break;
                        ...
                        case (M_INT8):
                            ...
                            status =
                                mc_ti_read_array(fam_id, wildcard_list[j], (void **)&label_temp_long, &num_items_read);
                            break;
                    } /* end switch */
        ...
        for ( con_count = 0; con_count < count; con_count++ )
        {
            ...
            if ( !found )
            {
                ...
                mc_query_family(fam_id, CLASS_SUPERCLASS, (void *)&args, out_names[con_count],
                                (void *)&labels_superclass);
                last_label->next = create_label(out_names[con_count], nproc, labels_superclass, 0, 1, 0);
                working_label = last_label->next;
            ...
            {
                working_label->labels[working_label->offset_per_processor[proc] + j] =
                    working_label->offset_per_processor[proc] + j + 1;
            }
        }
        for ( i = 0; i < count; i++ )
        {
...
*/
void create_new_sorter(int proc, int position, int label, struct sorter *target, int index)
{
...
int load_ti_nprocs(Mili_analysis *in_db)
{
    ...
    num_entries = htable_search_wildcard(fam->ti_param_table, 0, FALSE, "nproc", "NULL", "NULL", wildcard_list);
...
int get_mat_list_for_proc(Mili_analysis **in_db, int proc, short **mat_list)
{
    ...
    {
        for ( class = 0; class < num_sclasses; class ++)
        {
            ...
            {
                status = mc_get_class_info(fam_id, i, superclass, j, short_name, long_name, &obj_qty);

                nodes = (int *)malloc(obj_qty * conn_qty[class] * sizeof(int));
                mats = (int *)malloc(obj_qty * sizeof(int));
                parts = (int *)malloc(obj_qty * sizeof(int));

                status = mc_load_conns(fam_id, i, short_name, nodes, mats, parts);
                for ( k = 0; k < obj_qty; k++ )
                {
                    temp_mat[mats[k]] = 1;
                }
