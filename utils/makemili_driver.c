...
int file_select(struct dirent *entry)
{
    ...
    if ( (strncmp(plotfile_name, entry->d_name, strlen(plotfile_name)) == 0) && (entry->d_name[ptr - 1] == 'A') &&
         (entry->d_name[ptr - 2] >= '0' && entry->d_name[ptr - 2] <= '9') && !(entry->d_name[ptr - 2] == '_') &&
 ...
 * for files.
 */
int find_pad_count(char *input_file_name)
{
    int pad = 0;
    int found = 0;
    ...
    while ( (dirp = readdir(dir)) != NULL )
    {
        ...
        if ( strstr(dirp->d_name, plotfile_name) != NULL && dirp->d_name[last_pos] == 'A' &&
 ...
 * @return  int
 */
int processSingleFile(char *file_name, int close, int *fam_id)
{
    Famid famid;
    int status;
...
int write_multi_processor_files(int padding)
{
    int a_file_count, i;
