 ...
 */
int qty_connects[M_QTY_SUPERCLASS] = {0, 1, 2, 3, 3, 4, 4, 5, 6, 8, 0, 0};
...
void set_timesteps(Mili_analysis *in_db, Mili_analysis *out_db, int *start_state, int *stop_state)
{
    ...
    else
    {
        ...
        while (
            (is_equal(in_fam->state_map[j].time, time) ||
             in_fam->state_map[j].time <= out_fam->state_map[out_fam->state_qty - 1].time && j < in_fam->state_qty) )
        {
            j++;
        }
        j++;
...
int get_max_state(Mili_analysis *analy)
{
    ...
    rval = mc_query_family(analy->db_ident, QTY_STATES, NULL, NULL, &qty_states);
...
#ifdef TIME_TRACKER
        char tracker_str[256], walltime_str[20], version[64], problem_str[128];
