c $Id: partre_c.f,v 1.1 2021/07/23 16:03:03 rhathaw Exp $
...
c product endorsement purposes.
      ...
      program partre_c
c.....Intended to be executed after partre/partre_b sequence to exercise
c.....Mili restart features.
...
c.....MUST open the family
      ...
      call mf_open( 'prest', 'data', 'AaPs', fid, stat )
      if ( stat .ne. 0 ) then
...
c.....Do a couple queries just to see if they work
      call mf_query_family( fid, m_qty_states, m_dont_care, 
     +                      m_blank, qty_states, stat )
      call mf_print_error( 'mf_query_family', stat )
    
      bound_states(2) = 1 !note the inverted order
      bound_states(1) = qty_states
      call mf_query_family( fid, m_series_times, bound_states, 
     +                      m_blank, st_times, stat )
      call mf_print_error( 'mf_query_family', stat )

c.....Truncate by restart on state index
c     call mf_restart_at_state( fid, 2, 0, stat )
c     call mf_print_error( 'mf_restart_at_state', stat )

c.....Write ten records, each with one call.
c      time = 0.0
c      call mf_new_state( fid, srec_id, time, stat )
c      call mf_print_error( 'mf_new_state', stat )
c      
c      call mf_wrt_stream( fid, m_real, 54, state_record1, stat )
c      call mf_print_error( 'mf_wrt_stream', stat )
c      
c      time = 1.0
c      call mf_new_state( fid, srec_id, time, stat )
c      call mf_print_error( 'mf_new_state', stat )
c      
c      call mf_wrt_stream( fid, m_real, 54, state_record2, stat )
c      call mf_print_error( 'mf_wrt_stream', stat )
c    
...
c      call mf_print_error( 'mf_new_state', stat )
c      
c      call mf_wrt_stream( fid, m_real, 54, state_record3, stat )
c      call mf_print_error( 'mf_wrt_stream', stat )
c     
      call mf_query_family( fid, m_srec_fmt_id, 1, m_blank, 
     +                      srec_id, stat )
      call mf_print_error( 'mf_query_family', stat )
...
c     time = 3.0
c     call mf_new_state( fid, srec_id, time, suffix, st_idx, stat )
c     call mf_print_error( 'mf_new_state', stat )
c     
c     call mf_wrt_stream( fid, m_real, 54, state_record4, stat )
c     call mf_print_error( 'mf_wrt_stream', stat )
...
c.....Test m_next_state_loc query
     ...
     +                      m_blank, oiargs, stat )
      call mf_print_error( 'mf_query_family', stat )
      print 200, iiargs(1), iiargs(2), oiargs(1), oiargs(2) 
      

c.....Force file partition with append caused by restart call
...
c.....Test m_next_state_loc query
     ...
     +                      m_blank, oiargs, stat )
      call mf_print_error( 'mf_query_family', stat )
      print 300, 3, 0, oiargs(1), oiargs(2) 
...
100   format( 'After open, state at (', I2, ', ', I2, 
     +        '), next state at (', I2, ', ', I2, ')' )
200   format( 'After state at (', I2, ', ', I2, '), next state at (', 
     +        I2, ', ', I2, ')' )
300   format( 'After restart call to (', I2, ', ', I2, 
     +        '), next state at (', I2, ', ', I2, ')' )
