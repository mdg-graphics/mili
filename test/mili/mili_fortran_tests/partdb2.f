...
c.....Initialize object-ordered state records
      real state_record1(15)
      data state_record1 /
     +    -0.50, 1.25,  1250.0, 50.0, 90.0,  
     +    -0.50, 5.00,   900.0, 50.0, 85.0, 
     +    -0.50, 8.75,   790.0, 50.0, 75.0/
      real state_record2(15)
      data state_record2 /
     +    1.10, 1.25,  1250.0, 50.0, 90.0,  
     +    0.925, 5.00,   900.0, 50.0, 85.0, 
     +    0.83, 8.75,   790.0, 50.0, 75.0/
      real state_record3(15)
      data state_record3 /
     +    2.20, 1.25,  1250.0, 50.0, 90.0,  
     +    1.85, 5.00,   900.0, 50.0, 85.0, 
     +    1.66, 8.75,   790.0, 50.0, 75.0/
      real state_record4(15)
      data state_record4 /
     +    3.30, 1.25,  1250.0, 50.0, 90.0,  
     +    2.775, 5.00,   900.0, 50.0, 85.0, 
     +    2.49, 8.75,   790.0, 50.0, 75.0/
      real state_record5(15)
      data state_record5 /
     +    4.40, 1.25,  1250.0, 50.0, 90.0,  
     +    3.70, 5.00,   900.0, 50.0, 85.0, 
     +    3.32, 8.75,   790.0, 50.0, 75.0/
      real state_record6(15)
      data state_record6 /
     +    5.50, 1.25,  1250.0, 50.0, 90.0,  
     +    4.625, 5.00,   900.0, 50.0, 85.0, 
     +    4.15, 8.75,   790.0, 50.0, 75.0/
      real state_record7(15)
      data state_record7 /
     +    6.60, 1.25,  1250.0, 50.0, 90.0,  
     +    5.55, 5.00,   900.0, 50.0, 85.0, 
     +    4.98, 8.75,   790.0, 50.0, 75.0/
      real state_record8(15)
      data state_record8 /
     +    7.70, 1.25,  1250.0, 50.0, 90.0,  
     +    6.475, 5.00,   900.0, 50.0, 85.0, 
     +    5.81, 8.75,   790.0, 50.0, 75.0/
      real state_record9(15)
      data state_record9 /
     +    8.80, 1.25,  1250.0, 50.0, 90.0,  
     +    7.40, 5.00,   900.0, 50.0, 85.0, 
     +    6.64, 8.75,   790.0, 50.0, 75.0/
      real state_record10(15)
      data state_record10 /
     +    9.90, 1.25,  1250.0, 50.0, 90.0,  
     +    8.325, 5.00,   900.0, 50.0, 85.0, 
     +    7.47, 8.75,   790.0, 50.0, 75.0/
      real state_record11(15)
      data state_record11 /
     +    11.00, 1.25,  1250.0, 50.0, 90.0,  
     +    9.25, 5.00,   900.0, 50.0, 85.0, 
     +    8.30, 8.75,   790.0, 50.0, 75.0/
...
c.....for this name
      character*(m_max_name_len) part_short(6), part_long(6)
      data 
     +    part_short /
     +        'partpos', 'temp', 'ke', 'pe', 'ux', 'uy'/ 
     +    part_long /
...
c.....name to distinguish from other M_UNIT classes.
     ...
     +    nodal_l, quads_s, quads_l, global_s, global_l,
     +    material_s, material_l
      data particle_s, particle_l, nodal_s, nodal_l, quads_s, quads_l,
     +    global_s, global_l, material_s, material_l/
     +        'part', 'Particle', 'node', 'Node', 'quad', 'Quad',
...
c.....Nodes 1:4
     ...
     +    0.0, 0.0,  10.0, 0.0,  10.0, 10.0,  0.0, 10.0/
      integer n_labels(4)
      data n_labels / 1, 2, 3, 4 /

c.....Quad 1:1 connectivities; 4 nodes, 1 material, 1 part
      integer quad_1_1(6,1)
      data quad_1_1 /
     +    1, 2, 3, 4, 1, 1/
      integer quad_labels(1)
      data quad_labels / 1 /
...
c.....MUST open the family
      ...
      call mf_open( 'partdb2.plt', '.', 'AwPs', fid, stat )
      if ( stat .ne. 0 ) then
...
c.....Write a title
      call mf_wrt_string( fid, 'title', "2D Particle db", 
     +                    stat )
...
c              is completely independent of formatting the state record.
      ...
      call mf_def_vec_svar( fid, m_real, 2, part_short(1), 
...
c.....Define a node class.
      ...
      call mf_def_nodes( fid, mesh_id, nodal_s, 1, 4, node_1_4, stat )
      call mf_print_error( 'mf_def_nodes', stat )

      call mf_def_node_labels(fid, mesh_id, nodal_s, 4, n_labels, stat)
...
c.....Define a quad class
      ...
      call mf_def_conn_seq_labels( fid, mesh_id, quads_s, 1, 1, 
     +                             quad_labels, quad_1_1, stat )
...
c.....Define a particle class
     ...
     +                   particle_l, stat )
      ...
      call mf_def_class_idents( fid, mesh_id, particle_s, 1, 3, 
     +                          stat )
...
c.....Write ten records, each with one call.
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record1, stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record2, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record3, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record4, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record5, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record6, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record7, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record8, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record9, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record10, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 15, state_record11, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
