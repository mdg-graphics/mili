c $Id: shellrw.f,v 1.1 2021/07/23 16:03:03 rhathaw Exp $
...
c product endorsement purposes.
      ...
      program shellrw
c.....Tests re-write on nodal coordinates.  Modifies initial coordinates
c.....of selected nodes which can be seen in "md" output and by looking
c.....at Y Displacement at state zero using Griz4.
c.....State records have not been modified to be consistent with re-written
c.....initial positions.
...
c.....Modified coordinates for nodes 2, 4, 6, and 8 for re-write.
      real node_2_new(3,1)
      data node_2_new/ 1.0, -.5, 0.0/

      real node_6_new(3,1)
      data node_6_new/ 1.0, 1.5, 0.0/

      real node_4_new(3,1)
      data node_4_new/ 6.0, -.5, 0.0/

      real node_8_new(3,1)
      data node_8_new/ 6.0, 1.5, 0.0/
...
c.....MUST open the family
      ...
      call mf_open( 'shrw', 'data', 'AwPs', fid, stat )
      if ( stat .ne. 0 ) then
...
c.....Write a title
     ...
     +                    "Node def coord's re-write test", stat )
...
c.....First node re-write, node 2 (re-write to currently open non-state file)
      call mf_def_nodes( fid, mesh_id, nodal_s, 2, 2, node_2_new, stat )
      call mf_print_error( 'mf_def_nodes re-write', stat )
    
c.....Flush non-state to force file partition
...
c.....First node re-write, node 4 (re-write to closed non-state file)
      call mf_def_nodes( fid, mesh_id, nodal_s, 4, 4, node_4_new, stat )
      call mf_print_error( 'mf_def_nodes re-write', stat )
 
c.....Finish writing initial node defs
      call mf_def_nodes( fid, mesh_id, nodal_s, 5, 8, node_5_8, stat )
      call mf_print_error( 'mf_def_nodes', stat )
...
c.....First node re-write node 6 (re-write to 2nd open non-state file)
      call mf_def_nodes( fid, mesh_id, nodal_s, 6, 6, node_6_new, stat )
      call mf_print_error( 'mf_def_nodes re-write', stat )
...
c.....First node re-write, node 8 (re-write to 2nd closed file)
      call mf_def_nodes( fid, mesh_id, nodal_s, 8, 8, node_8_new, stat )
      call mf_print_error( 'mf_def_nodes re-write', stat )
