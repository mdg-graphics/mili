c $Id: nodev1.f,v 1.1 2021/07/23 16:03:03 rhathaw Exp $
...
c.....This app creates a db with these constraints:
c       1) A single srec format;
c       2) a single non-proper (with multiple blocks) Nodal subrecord 
c          in the srec format;
c       3) a single scalar state variable bound to the Nodal subrecord.
c     The mesh defined does have hex elements to provide faces for
c     rendering the nodal results, but no data is written for them.
c     Also, state variables definitions are left in for multiple classes
c     for future growth.
...
c.....Fill-in data to cover all nodes (but still out of order), if bound
c     to the subrecord by appending blocks [2,3] then [6,7], would be:
c     state_record1: 0, 0, 0, 0
c     state_record2: 200, 0, 200, 0
c     state_record3: 200, 200, 200, 200
c     state_record4: 200, 200, 200, 200
c     state_record5: 200, 200, 200, 200
...
c.....Nodal state variables
      character*(m_max_name_len) nodal_short(14), nodal_long(14)
      data 
     +    nodal_short /
     +        'nodpos', 'nodvel', 'nodacc', 'ux', 'uy', 'uz', 
     +        'vx', 'vy', 'vz', 'ax', 'ay', 'az', 'temp', 'goo' /,
     +    nodal_long /
     +        'Node Position', 'Node Velocity', 'Node Acceleration', 
     +        'X Position', 'Y Position', 'Z Position',
     +        'X Velocity', 'Y Velocity', 'Z Velocity',
     +        'X Acceleration', 'Y Acceleration', 'Z Acceleration',
     +        'Temperature', 'Gooeyness' /
...
c.....MUST open the family
      ...
      call mf_open( 'nv1o', 'data', 'ws', fid, stat )
      if ( stat .ne. 0 ) then
...
c.....Write a title
      call mf_wrt_string( fid, 'title', "Node db test 1", 
     +                    stat )
...
c              is completely independent of formatting the state record.
     ...
     +                      nodal_long(10), stat )
      ...
      call mf_def_svars( fid, 2, svar_types, nodal_short(13), 
     +                   nodal_long(13), stat )
...
c....+                    obj_blocks, stat )
c     call mf_def_subrec( fid, srec_id, hexs_l, m_object_ordered, 2, 
c    +                    hex_short(1), hexs_s, m_block_obj_fmt, 1, 
c    +                    obj_blocks, stat )
...
c.....Write five records, each with one call.
      ...
      call mf_wrt_stream( fid, m_real, 12, state_record1, stat )
      ...
      call mf_wrt_stream( fid, m_real, 12, state_record2, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 12, state_record3, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 12, state_record4, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 12, state_record5, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
