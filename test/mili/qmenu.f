c $Id: qmenu.f,v 1.1 2021/07/23 16:03:03 rhathaw Exp $
...
c product endorsement purposes.
      ...
      program qmenutest
c.....This program generates a loooooong primal submenu for the Shell class.
...
c                                                                Quad vars
     +    7 * 0.0, 50 * 0.0, 7 * 0.0, 50 * 0.0, 7 * 0.0, 50 * 0.0 /
      real state_record2(227)
      data state_record2 /
c                                                           Node Positions
...
c.....Quad state variables
      character*(m_max_name_len) quad_short(58), quad_long(58)
      data
     +    quad_short / 
     +        'stress', 'eps', 
...
c.....as the largest block of svars that will be defined in one call
      integer svar_types(51)
      data svar_types / 51*m_real /
...
c.....MUST open the family
      ...
      call mf_open( 'qmenuo', 'data', 'AwPs', fid, stat )
      if ( stat .ne. 0 ) then
...
c.....Write a title
      call mf_wrt_string( fid, 'title', "Long result menu test (Shell)",
     +                    stat )
...
c              is completely independent of formatting the state record.
     ...
     +                      quad_long(1), quad_short(53), 
     +                      quad_long(53), stat )
      call mf_print_error( 'mf_def_vec_svar', stat )
    
      call mf_def_svars( fid, 51, svar_types, quad_short(2), 
     +                   quad_long(2), stat )
      call mf_print_error( 'mf_def_svars', stat )
...
c.....Quad data, Shell class
      obj_blocks(1,1) = 1
      obj_blocks(2,1) = 3
      call mf_def_subrec( fid, srec_id, quads_l, m_object_ordered, 52, 
...
c.....Write five records, each with one call.
      ...
      call mf_wrt_stream( fid, m_real, 227, state_record1, stat )
      ...
      call mf_wrt_stream( fid, m_real, 227, state_record2, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 227, state_record3, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 227, state_record4, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
      ...
      call mf_wrt_stream( fid, m_real, 227, state_record5, stat )
      call mf_print_error( 'mf_wrt_stream', stat )
